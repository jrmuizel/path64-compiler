...
showfp_(char *message, long len) {
  ...
  __asm__("fldcw %0" : : "m" (*&(our_env.__control_word)));
  fprintf(stderr, "\n\t\t\tINV/OVF/ZDV/UNF/INX/Denorm\n");
  fprintf(stderr, "X87 Status:\t\t%d%d%d%d%d%d\n",
    !!(our_env.__status_word & 0x01),
    !!(our_env.__status_word & 0x08),
    !!(our_env.__status_word & 0x04),
    !!(our_env.__status_word & 0x10),
    !!(our_env.__status_word & 0x20),
    !!(our_env.__status_word & 0x02));
  fprintf(stderr,
    "X87 Mask (0=>trap):\t%d%d%d%d%d%d\n",
    !!(our_env.__control_word & 0x01),
    !!(our_env.__control_word & 0x08),
    !!(our_env.__control_word & 0x04),
    !!(our_env.__control_word & 0x10),
    !!(our_env.__control_word & 0x20),
    !!(our_env.__control_word & 0x02));
  static char *rounding[] = {
    "nearest", "down", "up", "zero"
    };
  fprintf(stderr, "X87 Rounding: %s\n",
    rounding[(our_env.__control_word >> 10) & 0x3]);
  fprintf(stderr,
