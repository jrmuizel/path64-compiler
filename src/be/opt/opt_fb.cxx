...
{
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Add_node(%" PRIdPTR ")\n", nx_new );
...
{
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Add_edge(%" PRIdPTR " --> %" PRIdPTR ")\n", nx_src, nx_dst );
...
{
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Remove_edge(ex %" PRIdPTR ")\n", ex );
...
{
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Change_edge_freq(ex %" PRIdPTR ")\n", ex );
...
OPT_FEEDBACK::Set_edge_dest( IDTYPE ex, IDTYPE nx_dst_new ) {
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Set_edge_dest(ex %" PRIdPTR ", nx_dst %" PRIdPTR ")\n",
	     ex, nx_dst_new );
...
OPT_FEEDBACK::Emit_feedback( WN *wn, BB_NODE *bb ) const
{
  ...
  if ( _trace ) {
    fprintf( TFile, "OPT_FEEDBACK::Emit_feedback bb->Id() = %3" PRIdPTR ", opr = %s:\n",
...
OPT_FEEDBACK::Delete_edge( IDTYPE nx_src, IDTYPE nx_dst ) {
  if ( _trace )
    fprintf( TFile, "OPT_FEEDBACK::Delete_edge(%" PRIdPTR " --> %" PRIdPTR ")\n",
	     nx_src, nx_dst );
  ...
  if (! match)
    DevWarn ("OPT_FEEDBACK::Delete_edge: edge (%" PRIdPTR " --> %" PRIdPTR ") not found",
	     nx_src, nx_dst);
}
...
{
  if ( _trace )
    fprintf( TFile, "OPT_FEEDBACK::Move_edge_dest(%" PRIdPTR " --> %" PRIdPTR ", %" PRIdPTR ")\n",
...
{
  if ( _trace )
    fprintf( TFile, "OPT_FEEDBACK::Move_incoming_edges_dest(%" PRIdPTR ", %" PRIdPTR ")\n",
...
{
  if ( _trace )
    fprintf( TFile, "OPT_FEEDBACK::Merge_outgoing_paths(%" PRIdPTR ", %" PRIdPTR ")\n",
	     nx_src, nx_dst_new );
...
OPT_FEEDBACK::Delete_node( IDTYPE nx ) {
  if ( _trace )
    fprintf( TFile, "OPT_FEEDBACK::Delete_node(%" PRIdPTR ")\n", nx );
...
{
  if ( _trace )
    fprintf( TFile, "OPT_FEEDBACK::Split_edge(%" PRIdPTR " --> %" PRIdPTR " --> %" PRIdPTR ")\n",
...
{
  if ( _trace )
    fprintf( TFile, "OPT_FEEDBACK::Split_node(nx_old %" PRIdPTR ", nx_new %" PRIdPTR ")\n",
	     nx_old, nx_new );
...
{
  if ( _trace )
    fprintf( TFile, "OPT_FEEDBACK::Clone_edge(%" PRIdPTR " --> %" PRIdPTR ", %" PRIdPTR " --> %" PRIdPTR ")\n",
			  ...
			  map<vertex_id, vertex_id>& nx_old_to_new )
{
  ...
  if ( _trace ) {
    ...
    for ( map<IDTYPE, FB_FREQ>::iterator iv = freq_transfer_node.begin();
      ...
      fprintf( TFile, " %" PRIdPTR "_", (*iv).first );
    ...
    for ( map<IDTYPE, FB_FREQ>::iterator ie = freq_transfer_edge.begin();
      ...
      IDTYPE ex = (*ie).first;
      const OPT_FB_EDGE& edge = _fb_opt_edges[ex];
      fprintf( TFile, " (%" PRIdPTR ",%" PRIdPTR ")_", edge.source, edge.destination );
...
OPT_FEEDBACK::Verify( CFG *cfg, const char *const phase )
{
  ...
  for ( IDTYPE ex = 1; ex < _fb_opt_edges.size(); ++ex ) {
	...
	balanced = false;
      if ( _trace ) {
	fprintf( TFile, "  Edge[%" PRIdPTR "] has incoming frequency == ", ex );
  ...
  for ( IDTYPE nx = 1; nx < _fb_opt_nodes.size(); ++nx ) {
    ...
    if ( node.freq_total_in != freq_total ) {  // change to Is_True later
      balanced = false;
      DevWarn( "OPT_FEEDBACK::Verify found node[%" PRIdPTR "]"
    ...
    if ( node.in_out_same &&
	 ...
	 node.freq_total_out.Known() ) {
      ...
      balanced = false;
      if ( _trace ) {
	fprintf( TFile, "  Node[%" PRIdPTR "] is unbalanced: incoming == ", nx );
  ...
  for ( BB_NODE *bb = cfg->First_bb(); bb != NULL; bb = bb->Next() ) {
	...
	valid = false;
	if ( _trace )
	  fprintf( TFile, "  CFG edge (bb%" PRIdPTR " --> bb%" PRIdPTR ") missing feedback!\n",
		   bb->Id(), succ->Id() );
      }
    }
  }
