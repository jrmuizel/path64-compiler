...
DU_MANAGER::Collect_BB_id(WN_MAP wn_to_cr_map, WN *wn)
{
  ...
  if (stmt && (WN_operator(wn) != OPR_FUNC_ENTRY || 
    ...
    bs->last_stmt = wn;
    if (Tracing())
      fprintf(TFile, "stmt (map_id %d ) in BB%" PRIuPTR "\n", WN_map_id(wn), Get_bb_id(wn));
  }
...
DU_MANAGER::Collect_CFG(CFG *cfg)
{
  ...
  if (Tracing()) {
    ...
    fprintf(TFile, "entry bb is %" PRIuPTR ".  exit bb is %" PRIuPTR "\n", _entry_bb, _exit_bb);
    for (i = 1; i < cfg->Total_bb_count(); i++) {
      fprintf(TFile, "%3d -> ", i);
      if (_bb_summary[i].succ) {
	for (IDTYPE *p = _bb_summary[i].succ; *p != 0; p++)
	  fprintf(TFile, "%" PRIuPTR " ", *p);
      }
      if (_bb_summary[i].cd) 
	fprintf(TFile, "\t\tCD%" PRIuPTR ":%d ", _bb_summary[i].cd, _bb_summary[i].which_edge);
