...
UPDATE<TRANSFORM, CACHE, VERSION>::Process_CR_no_repeat(CODEREP *cr, bool is_mu, STMTREP *stmt, BB_NODE *bb)
{
  ...
  if (simp) {
    if (_trace) {
      fprintf(TFile, "UPDATE<%s>: BB%" PRIdPTR "\n", _trans->Name(), bb->Id());
      cr->Print(10, TFile);
      simp->Print(10, TFile);
    }
    return simp;
  ...
  case CK_IVAR: 
	...
	Process_CR(cr->Mload_size(), false, stmt, bb) : NULL;
      CODEREP *index = (cr->Opr() == OPR_ILOADX) ? 
	Process_CR(cr->Index(), false, stmt, bb) : NULL;

      CODEREP *mu = NULL;
      if (cr->Ivar_mu_node())
	mu = Process_CR(cr->Ivar_mu_node()->OPND(), true, stmt, bb);
      
      if (ilod_base || mload_size || index || mu) {
...
UPDATE<TRANSFORM, CACHE, VERSION>::Process_SR(STMTREP *stmt, BB_NODE *bb)
{
  ...
  case OPR_ISTOREX: {
    ...
    if (cr) 
      stmt->Lhs()->Set_istr_base(cr);
    CODEREP *index = (CODEREP *)stmt->Lhs()->Index();
    cr = Process_CR(index, false, stmt, bb);
    if (cr)
      stmt->Lhs()->Set_index(cr);
    break;
  }
  }
}
