			     ...
			     CODEREP *old_temp, CODEREP *multiplier ) const
{
  ...
  if ( Tracing() ) {
    fprintf( TFile, "CSE::Generate_injury_repair in BB:%" PRIdPTR "\n",
	     injury->Bb()->Id() );
...
#endif
  if ( Tracing() ) {
    fprintf(TFile, "Repair_injury_phi_phi_opnd: phi-pred in BB%" PRIdPTR "\n",
	    use->Bb()->Id());
