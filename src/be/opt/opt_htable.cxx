...
CODEMAP::Add_const(MTYPE typ, INT64 val)
{
  ...
  cr = Hash_Const(cr);
#if !defined( KEY) // this is bad for performance
...
#if defined( KEY) // bug 14605: force to signed because generating negate
		   ...
		   COPYPROP *copyprop)
{
  ...
  if (ccr->Tree() == NULL && kid1.Tree() == NULL) {
    ccr->Set_scale(ccr->Scale() * kid1.Scale());
#if defined( KEY) // bug 2846: check for need to truncate the folded result
  ...
  case OPR_XPRAGMA:
    ...
    if (WN_pragma(Wn()) == WN_PRAGMA_COPYIN_BOUND) {
      ...
      CODEREP *cr = htable->Add_expr(WN_kid0(Wn()),
                                     opt_stab, this,  &proped, copyprop);
      Set_rhs(cr);
#if defined( KEY) // since its value may consist of MAX, prevent its copy propagation
...
#if defined( KEY) // bug 5131: mark mp-shared variables defined in MP region 
