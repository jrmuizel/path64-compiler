...
#if defined( KEY) && !defined(TARG_ST) // bug 3621
...
CODEREP::Print(INT indent, FILE *fp) const
{
  ...
  if (Kind() == CK_IVAR) {
    ...
    if (Ivar_mu_node() && Ivar_mu_node()->OPND())  // Check if the MU has been deleted.
      fprintf(fp, "%" PRIdPTR "/cr%d", Ivar_mu_node()->OPND()->Aux_id(), 
...
CODEREP::Print_node(INT32 indent, FILE *fp) const
{
  ...
  case CK_OP:
    ...
    case OPR_COMPOSE_BITS:
      ...
      break;
    case OPR_ARRAY:
      fprintf(fp, " %d %" SCNd64 "", Num_dim(), Elm_siz());
      break;
    case OPR_INTRINSIC_CALL:
    case OPR_INTRINSIC_OP:
...
#if defined( KEY) && !defined(TARG_ST)
    case OPR_PURE_CALL_OP:
      fprintf(fp, " %" PRIdPTR , Call_op_aux_id());
      break;
#endif
    }
    break;
  case CK_IVAR:
    ...
    if (Opr() == OPR_ILOADX ||
        Opr() == OPR_MLOAD ||
        Opr() == OPR_PARM)
      sprintf(buf, "%s ty=%" SCNd64 " ", OPCODE_name(Op()), Ilod_ty());
    else
      sprintf(buf,"    %s%s%s %d ty=%" SCNd64 " ",
  ...
  case CK_LDA:
    fprintf(fp, ">");	// mark line visually as htable dump
    for (i = 0; i < indent; i++) fprintf(fp, " ");
    fprintf(fp, "LDA %s sym%" PRIdPTR " %d", MTYPE_name(Dtyp()),Lda_aux_id(),Offset());
    break;
  case CK_VAR:
    fprintf(fp, ">");	// mark line visually as htable dump
    for (i = 0; i < indent; i++) fprintf(fp, " ");
    ...
    fprintf(fp, " %s %s sym%" PRIdPTR "v%" PRIdPTR " %" PRId32 " ty=%" SCNd64 " ", MTYPE_name(Dsctyp()),
	    MTYPE_name(Dtyp()), Aux_id(), Version(), Offset(), Lod_ty());
    break;
  case CK_CONST:
    fprintf(fp, ">");	// mark line visually as htable dump
    for (i = 0; i < indent; i++) fprintf(fp, " ");
    fprintf(fp, "LDC %s %" SCNd64 "", MTYPE_name(Dtyp()), Const_val());
    break;
  case CK_RCONST:
    fprintf(fp, ">");	// mark line visually as htable dump
    for (i = 0; i < indent; i++) fprintf(fp, " ");
...
CODEREP::Print_str(BOOL name_format) const
{
  ...
  case CK_IVAR:
    if (name_format) {
      if (Opr() == OPR_MLOAD ||
	  Opr() == OPR_PARM) {
	sprintf(buf,"%s %d ty=%" SCNd64 " ", OPCODE_name(Op()),
                Offset(), Ilod_ty());
      }
      else if (Opr() == OPR_ILOADX) {
	sprintf(buf,"%s%s%s ty=%" SCNd64 " ",
  ...
  case CK_LDA:
    if (name_format)
      sprintf(buf,"LDA sym%" PRIdPTR " %d",Lda_aux_id(),Offset());
    break;
  case CK_VAR:
    if (name_format) {
      if (! Bit_field_valid())
      ...
      sprintf(buf,"sym%" PRIdPTR "v%" PRIdPTR " %d ty=%" SCNd64 " ",Aux_id(),Version(),Offset(), Lod_ty());
    }
    break;
  case CK_CONST:
    if (name_format)
      sprintf(buf,"LDC %" SCNd64 "",Const_val());
    break;
  case CK_RCONST:
    if (name_format)
...
#if defined( KEY) && !defined(TARG_ST) // bug 4518
...
CODEMAP::Add_const(MTYPE typ, INT64 val)
{
  ...
  cr = Hash_Const(cr);
#if !defined( KEY) || defined(TARG_ST) // this is bad for performance
...
#if defined( KEY) && !defined(TARG_ST) // bug 5557
...
#if defined( KEY) && !defined(TARG_ST)// bug 14605: force to signed because generating negate
		   ...
		   COPYPROP *copyprop)
{
  ...
  if (ccr->Tree() == NULL && kid1.Tree() == NULL) {
    ccr->Set_scale(ccr->Scale() * kid1.Scale());
#if defined( KEY) && !defined(TARG_ST) // bug 2846: check for need to truncate the folded result
  ...
  if ( Is_flag_set(CF_MADEUP_TYPE) || Dsctyp() == MTYPE_UNKNOWN ) {
    ...
    if (to_dsctyp != MTYPE_BS && Dsctyp() != MTYPE_BS) {
      if (MTYPE_size_min( Dsctyp() ) != MTYPE_size_min( to_dsctyp )) {
        ...
        DevWarn("CODEREP::Var_type_conversion: (fixed) loads of different size share aux_id %" PRIdPTR ,Aux_id());
...
#if defined( KEY) && !defined(TARG_ST) // bug 3054
	  if (retv->Kind() == CK_CONST) {
#endif
	  retv->DecUsecnt();
	  ccr->Set_tree(NULL);
  ...
  case OPR_XPRAGMA:
    ...
    if (WN_pragma(Wn()) == WN_PRAGMA_COPYIN_BOUND) {
      ...
      CODEREP *cr = htable->Add_expr(WN_kid0(Wn()),
                                     opt_stab, this,  &proped, copyprop);
      Set_rhs(cr);
#if defined( KEY) && !defined(TARG_ST) // since its value may consist of MAX, prevent its copy propagation
...
#if defined( KEY) && !defined(TARG_ST) // bug 5131: mark mp-shared variables defined in MP region 
...
STMTREP::Print(FILE *fp) const
{
  ...
  if (Has_mu()) {
	...
	if (mnode->OPND())  // Check if the MU has been deleted.
	  fprintf(fp, "%" PRIdPTR "/cr%d ", mnode->Aux_id(), mnode->OPND()->Coderep_id());
    }
    fprintf(fp, ">\n"); 
  }
  ...
  if (Has_chi()) {
    ...
    fprintf(fp,"chi <");
    FOR_ALL_NODE(cnode, chi_iter, Init(Chi_list())) {
      fprintf(fp, "%" PRIdPTR "", cnode->Aux_id());
...
STMTREP::Print_node(FILE *fp) const
{
  ...
  case OPR_STID:
    fprintf(fp, ">");	// mark line visually as htable dump
    fprintf(fp, "%s %s %s sym%" PRIdPTR "v%" PRIdPTR "", OPERATOR_name(_opr),
...
STMTREP::Print_str(BOOL name_format)
{
  ...
  switch (_opr) {
    case OPR_STID:
    case OPR_STBITS:
      sprintf(buf,"%s sym%" PRIdPTR "v%" PRIdPTR "",
...
CODEMAP::Insert_var_phi(CODEREP *new_lhs, BB_NODE *bb)
{
  ...
  FOR_ALL_ELEM (bb_phi, df_iter, Init(bb->Dom_frontier())) {
	...
	phi->Set_live();
      ...
      Is_Trace(Tracing(),
	   (TFile, "CODEMAP: insert var phi at BB%" PRIdPTR " for sym%" PRIdPTR "\n", bb->Id(),
	    aux));
