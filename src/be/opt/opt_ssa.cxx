...
#if !defined( KEY) // move deletion to emit phase because htable.cxx needs to mark
	    // with DONT_PROP flag
...
PHI_NODE::Print(INT32 in_degree, FILE *fp)  const
{
  if (! Live())
    ...
    CODEREP *cr = RESULT();
    fprintf(fp, "   sym%" PRIdPTR "v%" PRIdPTR "<cr%d> <- phi(", cr->Aux_id(),
	    cr->Version(), cr->Coderep_id());
    for (INT32 i = 0; i < Size(); i++) {
      cr = OPND(i);
      if (cr != NULL) {
        fprintf(fp, "sym%" PRIdPTR "v%" PRIdPTR "<cr%d>", cr->Aux_id(), cr->Version(),
	        cr->Coderep_id());
      }
      else {
	fprintf(fp, "<null>");
...
PHI_NODE::PRINT(INT32 in_degree, FILE *fp) const
{
  ...
  if (Dce_dead()) 
    fprintf(fp, "(dce-dead)");
  fprintf(fp, "   sym%" PRIdPTR "v%" PRIdPTR " <- phi(", Aux_id(), Result());
  for (INT32 i = 0; i < in_degree; i++) {
    fprintf(fp, "sym%" PRIdPTR "v%" PRIdPTR "", Aux_id(), Opnd(i));
    if ( i == in_degree-1 )
      fprintf(fp, ")\n" );
    else
      fprintf(fp, "," );
