...
#if defined( KEY) && !defined(TARG_ST) // bug 9114
...
#if defined( KEY) && !defined(TARG_ST) // bug 9114 : this code moved from inside _select_and_sort_valnums
...
#if !defined( KEY) || defined(TARG_ST) // bug 9114 : this code moved to caller
      VALNUM_TO_EXPR_LIST vn_to_exprid(*_vn, _lpool);
#endif
			  ...
			  CODEREP *rhs)
{
   ...
   Is_Trace(_tracing,
	    (TFile,
	     "----> VALNUM_FRE::_save_to_temp in bb%" PRIdPTR "\n", in_bb->Id()));
...
VALNUM_FRE::_ivc_classify(BB_NODE *loop_header, VN_IVC &vn_ivc)
{
   ...
   FOR_ALL_ELEM(phi, phi_iter, Init(loop_header->Phi_list()))
   {
	     ...
	     _is_real_var(phi_result))          // Has a memory location
	 {
	    ...
	    {
#if defined( KEY) && !defined(TARG_ST) // bug 9114 : without this, could falsely identify class for the
...
VALNUM_FRE::_ivc_coalesce(BB_NODE *loop_header, VN_IVC &vn_ivc)
{
   ...
   for (INT32 i = 0; i < vn_ivc.num_eqclasses(); i++)
   {
	 ...
	 if (_tracing)
	 {
	    fprintf(TFile, "====> IVC for BB %" PRIdPTR " <====\n", loop_header->Id());
...
VALNUM_FRE::_insert_valnum_phi(EXP_WORKLST *worklist)
{
   ...
   OPT_POOL_Push(_lpool, -1);
   {
	 ...
	 FOR_ALL_ELEM (bb_phi, bb_iter, Init(bb_orig->Pred()))
         {
	    Is_Trace(_tracing, (TFile, "------ Enter Phi-pred %" PRIdPTR " ------\n",
				bb_phi->Id()));
	    phi_list.Union1D(bb_phi->Dom_dfs_id());
	 }
      }
	 ...
	 Is_Trace(_tracing,
		  (TFile,"------ Generate Phi-pred occurrence %" PRIdPTR " ------\n", 
		   bb_orig->Id()));
