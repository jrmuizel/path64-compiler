...
void OPT_STAB::Simplify_Pointer_Ver(VER_ID ver, POINTS_TO *ai)
{
  ...
  case PHI_STMT:
	...
	FOR_ALL_ELEM(pred, bb_iter, Init(bb->Pred())) {
	  i++;
	  fprintf(TFile, "   phi opnd %d  BB %" PRIdPTR "  ver %" PRIdPTR ": ",
	...
	fprintf(TFile, "phi result ver %" PRIdPTR " BB %" PRIdPTR " aux %" PRIdPTR ": ", ver, bb->Id(), 
  ...
  if ( Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG)) {
    fprintf(TFile, "ver %" PRIdPTR " aux %" PRIdPTR ": ", ver, Ver_stab_entry(ver)->Aux_id());
    ai->Print(TFile);
  }
  CHECK_POINTS_TO(ai);
}
...
void OPT_STAB::Analyze_Base_Flow_Free(POINTS_TO *pt, WN *wn)
{
  ...
  case OPR_MSTORE:
    ...
    break;
  case OPR_ISTOREX:
    Simplify_Pointer(WN_kid2(wn), pt);
    break;
  }
...
void OPT_STAB::Analyze_Base_Flow_Sensitive(POINTS_TO *pt, WN *wn)
{
  ...
  case OPR_MLOAD:
	...
	if (WN_operator(wn) == OPR_ILDBITS)
	  pt->Set_bit_ofst_size(ai.Bit_Ofst() + WN_bit_offset(wn), 
	  			WN_bit_size(wn));
	else pt->Set_bit_ofst_size(ai.Bit_Ofst(), ai.Bit_Size());
	pt->Set_attr(ai.Attr());
	pt->Shift_ofst(WN_offset(wn));
	pt->Lower_to_base(wn);
	...
	if (ST_sclass(st) != SCLASS_REG) {
	  ...
	  if (ST_is_temp_var(st) &&
	    ...
	    if (def) {
	      ...
	      st = Find_Based_Pointer(WN_kid0(def),depth);
	    }
	    if (!st || ST_is_temp_var(st)) {
	      // Failure, so return the original ST
	      *depth = 0;
	      st = orig_st;
	    }
	  } else
...
OPT_STAB::REGION_add_to_bound(RID *rid, AUX_ID aux_id, BOOL empty)
{
  ...
  if (st && ST_class(st) == CLASS_PREG) { // add a preg
    ...
    Is_Trace(_rgn_trace && ret,(TFile,"===== OPT_STAB::REGION_add_to_bound "
		"(RGN %d), adding preg aux_id %" PRIdPTR "\n",RID_id(rid),aux_id));
  } else {
...
OPT_STAB::Allocate_vsym(WN * memop_wn, POINTS_TO *memop_pt)
{
  ...
  if (Update_From_Restricted_Map(memop_wn, &scratch_pt)) {
    ...
    if (scratch_pt.Unique_pt() ||
      ...
      if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG)) {
        fprintf(TFile, "Allocate_vsym: Returning aux_id %" PRIdPTR " as unique "
                "based on\n", var);
					...
					INT32 num_parms)
{
  ...
  for (idx = BS_Choose( alias_set );
	...
	how = READ;
	if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG)) {
	  fprintf(TFile, "<alias> Remove the chi node with aux id %" PRIdPTR
	...
	if (how == READ)
	  ...
	  how = WRITE;
	if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG))
	  fprintf(TFile, "<alias> Remove the mu node with aux id %" PRIdPTR 
...
#if defined( KEY) //Bug 607 && bug 1672 and bug 5022
  ...
  if (!OPCODE_is_black_box(opc)) {
	...
	if (sym_pt->Ip_alias_class() == OPTIMISTIC_AC_ID) {
	  ...
	  if ((sym_pt->Ip_alias_class() != PESSIMISTIC_AC_ID) &&
	    ...
	    DevWarn("Transfer_alias_class_to_occ_and_aux: Inconsistent IP alias class: %" PRIdPTR " vs. %u",
		   sym_pt->Ip_alias_class(), WN_MAP32_Get(WN_MAP_ALIAS_CLASS, wn));
	...
	if (vsym_pt->Unique_pt()) {
	  ...
	  if (alias_class != OPTIMISTIC_AC_ID &&
	    ...
	    vsym_pt->Set_local();
	    if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG)) {
	      fprintf(TFile, "Xfer_ac: Aux ID %" PRIdPTR " is local unique_pt vsym\n",
		      occ->Aux_id());
	    }
...
OPT_STAB::Compute_FSA_stmt_or_expr(WN *wn)
{
  ...
  if (OPERATOR_is_scalar_iload (opr) || opr == OPR_MLOAD) {
    ...
    if (occ->Points_to()->Based_sym() == NULL)
      Analyze_Based_Pointer(occ->Points_to(), 
			    opr != OPR_ILOADX ? WN_kid0(wn) : WN_kid1(wn));
  }
  ...
  if (OPERATOR_is_scalar_istore (opr) || opr == OPR_MSTORE) {
    ...
    if (occ->Points_to()->Based_sym() == NULL) {
      Analyze_Based_Pointer(occ->Points_to(), 
      			    opr != OPR_ISTOREX ? WN_kid1(wn) : WN_kid2(wn));
      if (occ->Points_to()->Based_sym() != NULL) 
	need_update_chi = TRUE;
    }
    ...
    if (need_update_chi) {
      ...
      while (cnode != NULL) {
	AUX_ID v = cnode->Aux_id();
#if defined( KEY) // work around bug 7421: return_vsym aliases with a fixed global var
	if (v == Return_vsym() && 
	...
	if (aux_stab[v].Stype() != VT_UNIQUE_VSYM &&
	  ...
	  if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG))
	    fprintf(TFile, "<alias> Remove the chi node that definus %" PRIdPTR ".\n",
		    cnode->Result());
...
OPT_STAB::Update_alias_set_with_virtual_var(void)
{
  ...
  if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG)) {
    fprintf(TFile, "Return vsym is %" PRIdPTR "\n", Return_vsym());
    fprintf(TFile, "\nAddr_saved bitset:  ");
