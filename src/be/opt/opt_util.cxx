...
static void Opt_tlog2( char *, uint64_t, char * );
static int  Opt_tlog_trace( void );

// TLOG interface for reporting optimizations
extern "C" void 
Opt_tlog( char *keyword, uint64_t srcpos, const char *fmt, ...)
{
  va_list vp;
  char msg_buf[MAX_WARN_LEN];
  int len;
...
static void Opt_tlog2( char *keyword, uint64_t srcpos, char *msg )
{
...
INT Report_statistics()
{
  ...
  if (Get_Trace(TKIND_INFO, TINFO_TIME)) {
	...
	peak_times[i] = p;
      fprintf(TFile,
	      "%8d %6.2f%% (cum %8d %5.2f, peak %5.2f%%) ms %5" SCNd64 "k mem in %s",
