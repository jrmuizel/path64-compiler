  ...
  while (idx && aux_stab[idx].St() != NULL) {
    ...
    case VT_LDA_SCALAR:
      ...
      kind_match = ((is_scalar && 
		     ((MTYPE_is_dynamic(mtype) && aux_stab[idx].Mtype() == mtype) ||
		      (!MTYPE_is_dynamic(mtype) && aux_stab[idx].Mclass() == mclass)))
		    || is_virtual);
#else
...
#if defined( KEY) && !defined(TARG_ST) // bug 12321
	aux_stab[idx].St() == st &&
#endif
...
#if defined( KEY) && !defined(TARG_ST) // bug 5401 and 5267
...
{
  ST *st;
#if defined( KEY) && !defined(TARG_ST) // bug 1523: preopt in ipl cannot use pregs due to exception handling
  ...
  sym->Set_st(st);
#if defined( KEY) && !defined(TARG_ST) // due to about change, st is no longer always preg
...
OPT_STAB::Make_st_group(void)
{
  ...
  FOR_ALL_NODE(idx, aux_stab_iter, Init()) {
	...
	for (INT32 j = 0; j < count; j++) {
      ...
      INT32 count_var = 0;
      for (INT32 j = 0; j < count; j++) {
#if !defined( KEY) || defined(TARG_ST) // cannot remove virtual variables because Lower_to_extract_compose 
...
OPT_STAB::Remap_aux_synonym(WN *wn)
{
  ...
  if ( OPERATOR_has_aux(opr)) {
    ...
    if (Completely_replaced(Aux_stab_entry(idx))) {
      ...
      WN_set_aux(wn, syn);
      if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG)) 
	fprintf(TFile, "remap auxid %" PRIdPTR " to %" PRIdPTR "\n", idx, syn);
...
#if !defined( KEY) || defined(TARG_ST) // this cause the variable to become read-only, then srdata, which
...
OPT_STAB::Create(COMP_UNIT *cu, REGION_LEVEL rgn_level)
{
  ...
  // initialize _pu_has_trampoline
  if (PU_c_lang(Get_Current_PU()) && PU_uplevel(Get_Current_PU())) {
	...
	PU &nested_pu = Pu_Table[ST_pu(&St_Table[st_idx])];
	if (PU_need_trampoline(nested_pu)) {
	  _pu_has_trampoline = TRUE;
	  break;
	}
...
#if defined( KEY) && !defined( TARG_ST) // taken from wn_mp.cxx
...
OPT_STAB::Identify_vsym(WN *memop_wn)
{
  ...
  if (Update_From_Restricted_Map(memop_wn, &scratch_pt)) {
	...
	Is_True(Aux_stab_entry(var)->Stype() == VT_UNIQUE_VSYM ||
      ...
      if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG)) {
	fprintf(TFile, "Identify_vsym: Returning aux_id %" PRIdPTR " as unique "
		"based on\n", var);
  ...
  if (opr == OPR_ILOADX)
    addr_wn = WN_kid1(memop_wn);
  else if (opr == OPR_ISTOREX)
    addr_wn = WN_kid2(memop_wn);
...
OPT_STAB::Remap_ver_synonym(WN *wn)
{
  ...
  if (WN_has_ver(wn)) {
	...
	s = Ver_stab_entry(s)->Synonym();
      WN_set_ver(wn, s);
      if (Get_Trace(TP_GLOBOPT, ALIAS_DUMP_FLAG)) 
	fprintf(TFile, "remap verid %" PRIdPTR " to %" PRIdPTR "\n", idx, s);
    }
  }
...
#else
#ifdef KEY
  fprintf(fp, "sym%" PRIdPTR "v%" PRIdPTR " <- chi( sym%" PRIdPTR "v%" PRIdPTR " ) %s\n", Aux_id(), Result(), Aux_id(), Opnd(), Live() ? "LIVE" : "NOT LIVE");
#else
...
{
#if defined( KEY) && !defined( TARG_ST)
  fprintf(fp, " sym%" PRIdPTR "v%" PRIdPTR " ", Aux_id(), Opnd());
#else
...
void OPT_STAB::Print_aux_entry(AUX_ID i, FILE *fp)
{
  ...
  case VT_SPECIAL_VSYM:
    ...
    if (aux_stab[i].St() != NULL) {
      fprintf(fp, "VAR   st=%s base=%s byte ofst=%" SCNd64 " byte size=%" SCNd64 ""
  ...
  case VT_OTHER:
    fprintf(fp, "SYMBL st=%s base=%s ofst=%" SCNd64 "\n",
	    aux_stab[i].St_name(), aux_stab[i].Base_name(),
...
{
  fprintf(fp, " vers %" PRIdPTR ": sym=%3" PRIdPTR " ver=%3d ", ver_id, Aux_id(), Version());
  ...
  case CHI_STMT:
    ...
    fprintf(fp, " use=%" PRIdPTR , Chi()->Opnd());
    break;
  default:
    break;
  }
