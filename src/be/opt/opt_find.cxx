...
CODEMAP::Fix_zero_version(PHI_NODE *phi, INT opnd_idx, bool allow_real_or_no_def)
{
  ...
  Is_Trace(Tracing(),
	   (TFile, "CODEMAP::Fix_zero_version aux-id %" PRIdPTR ", bb %" PRIdPTR ", opnd %d\n",
  ...
  if (!allow_real_or_no_def) {
	...
	!def_stmt.Is_stmt_node()) {
      Is_Trace(Tracing(),
	       (TFile, "CODEMAP::Fix_zero_version: cannot fix aux-id %" PRIdPTR ", bb %" PRIdPTR ", opnd %d\n",
		phi->Aux_id(), phi->Bb()->Id(), opnd_idx));
      return;
    }
  }
...
CODEMAP::Fix_zero_version(CHI_NODE *chi, STMTREP *stmt)
{
  ...
  Is_Trace(Tracing(),
	   (TFile, "CODEMAP::Fix_zero_version mu-node aux-id %" PRIdPTR ", bb %" PRIdPTR "\n",
