  ...
  FOR_ALL_NODE(occ, occ_iter, Init()) {
	...
	if (def == NULL || !def->Bb()->Postdominates(occ->Bb())) {
	  ...
	  Is_Trace(etable->Tracing(), 
		   (TFile, "SPRE: delete one operand at BB%" PRIdPTR "\n", occ->Bb()->Id()));
	  occ->Set_delete_comp();
	...
	FOR_ALL_ELEM(pred_bb, pred_bb_iter, Init(occ_bb->Pred())) {
	  ...
	  if (phi != NULL) {
		...
		Is_Trace(etable->Tracing(), 
			 (TFile, "SPRE: insert one operand at BB%" PRIdPTR "\n", occ_bb->Id()));
	      }
	      else {
...
ETABLE::SPRE_rename_var(CODEREP *cr, BOOL allow_zero_ver)
{
  ...
  if (Opt_stab()->NULL_coderep(auxid)) {
    ...
    Is_Trace(Tracing(),
	     (TFile, "SPRE_rename_ver:  pushing zero version cr onto stack for auxid %" PRIdPTR ".\n", auxid));
