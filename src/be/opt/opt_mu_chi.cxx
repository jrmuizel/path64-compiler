  ...
  while (1) {
	...
	FOR_ALL_NODE( cnode, chi_iter, Init(chi_list)) {
	  if (cnode->Aux_id() == vsym_id) {
	    ...
	    cnode->Set_live(TRUE);
	    if (trace) {
	      fprintf(TFile, "Update_iload_vsym: set aux_id %" PRIdPTR " chi live.\n",
		      cnode->Aux_id());
	    }
	...
	FOR_ALL_ELEM (phi, phi_iter, Init(bb->Phi_list())) {
	  if (phi->Aux_id() == vsym_id) {
	    ...
	    phi->Set_live();
	    if (trace) {
	      fprintf(TFile, "Update_iload_vsym: set aux_id %" PRIdPTR " phi at BB%" PRIdPTR " to live.\n",
		      phi->Aux_id(), bb->Id());
	    }
