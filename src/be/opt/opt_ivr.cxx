...
#ifdef KEY
      // must not be vector type
      if (MTYPE_is_vector(incr->Dtyp())) continue;
#endif
...
# endif
      ...
      if (_trace) {
	fprintf(TFile, "IVR: generate primary IV with aux-id %" PRIdPTR "\n", new_temp);
	fprintf(TFile, "IVR: insert phi at BB%" PRIdPTR ", init at BB%" PRIdPTR ", incr at BB%" PRIdPTR "\n",
...
#ifdef TARG_X8664
  BOOL div_64bit_ok = TRUE;
  // bug 14816: 64-bit divide under -m32 needs library function
  // support, and hence should not be generated for freestanding
  // environment
  if (OPT_Freestanding && Is_Target_32bit() &&
      tripcount_type == MTYPE_I8) {
    div_64bit_ok = FALSE;
  }
#endif
				      ...
				      IV_CAND *primary) 
{
  ...
  if (_trace) {
    if (found) 
      fprintf(TFile, "Trip IV is %" PRIdPTR ".\n", trip_cand->Var()->Aux_id());
    else
...
#if defined( KEY) // bug 13728: if there is wraparound, do not continue
...
IVR::Convert_all_ivs(BB_LOOP *loop)
{
  ...
  if (_trace) {
    if (trip_count != NULL)  {
      if (trip_count->Kind() == CK_CONST)
	fprintf(TFile, "IVR: exit trip count is %" SCNd64 "\n",
  ...
  if (primary && trip_iv && ivr_generated_primary) {
    ...
    Set_Preg_Name((PREG_NUM) Opt_stab()->St_ofst(primary_id),
		  ST_name(trip_st));
    if (_trace)
      fprintf(TFile, "IVR: change preg %" SCNd64 " index name to %s.\n", 
  ...
  if (Phase() == MAINOPT_PHASE &&
    ...
    if (_trace) 
      fprintf(TFile,
	      "IVR: loop executes one -- removing loop back edge at BB%" PRIdPTR "\n", 
	      test->Id());
  ...
  for (iv_cand_iter = iv_cand_container.begin(); 
    ...
    if (MTYPE_size_min(secondary->Dtype()) <
      ...
      if (_trace) 
	fprintf(TFile, "IVR: skip secondary IV sym%" PRIdPTR " because secondary"
    ...
    if (secondary->Dtype() == MTYPE_U4) {
      if (_trace) 
	fprintf(TFile, "IVR: skip secondary IV sym%" PRIdPTR " because of type MTYPE_U4\n"
		, secondary->Var()->Aux_id());
      continue;
    }
#endif
	...
	(trip_iv == NULL ||
	 (MTYPE_size_min(trip_iv->Dtype())
	  > MTYPE_size_min(primary->Dtype())))) {
      if (_trace) 
	fprintf(TFile, "IVR: skip secondary IV sym%" PRIdPTR " because secondary IV"
    ...
    if (Opt_stab()->Aux_stab_entry(aux_id)->Dont_replace_iv()) {
      if (_trace)
	fprintf(TFile,
		"IVR: skip secondary IV sym%" PRIdPTR " because of Dont_replace_iv.\n", 
		secondary->Var()->Aux_id());
      continue;
    }
    ...
    if (secondary->Init_value() != NULL) {
      ...
      if (_trace) 
	fprintf(TFile, "IVR skip secondary IV sym%" PRIdPTR " because it cannot"
		" find init value.\n", secondary->Var()->Aux_id());
...
IVR::Process_one_loop(BB_LOOP *loop)
{
  ...
  if ( !WOPT_Enable_IVR_Outermost_Loop_Parallel_Region &&
    ...
    {
      Is_Trace(Trace(),
	       (TFile, "IVR::Process_one_loop: skip loop at BB:%" PRIdPTR ,
		loop->Header()->Id()) );
      return has_mp_do;
    }
