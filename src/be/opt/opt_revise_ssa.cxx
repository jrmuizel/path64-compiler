...
OPT_REVISE_SSA::Find_scalars_from_lowering_bitfld_cr(CODEREP *cr)
{
  ...
  case CK_VAR: 
    if (cr->Bit_field_valid()) {
      ...
      _has_bitfield = TRUE;
      if (_tracing) 
	fprintf(TFile, "replace %" PRIdPTR " by extract of %" PRIdPTR "\n", cr->Aux_id(), idx); 
    }
#endif
    return;
  case CK_IVAR:
	...
	case OPR_STID: {
	  ...
	  if (lhs->Dsctyp() != MTYPE_M && MTYPE_byte_size(lhs->Dsctyp()) < 4 &&
	    ...
	    _symbols_to_revise->Union1D(lhs->Aux_id()); 
      	    ...
      	    _has_bitfield = TRUE;
	    if (_tracing) 
	      fprintf(TFile, "replace %" PRIdPTR " by compose of %" PRIdPTR "\n", lhs->Aux_id(), idx); 
	  }
	  break;
	}
#endif
...
OPT_REVISE_SSA::Find_scalars_from_lda_iloads(CODEREP *cr)
{
  ...
  case CK_IVAR: {
	...
	cr->Opr() == OPR_PARM)
      return;
#if defined( KEY) // to limit compilation time explosion (bug 2081)
...
OPT_REVISE_SSA::Update_phis(BB_NODE *bb)
{
  ...
  FOR_ALL_ELEM (phi, phi_iter, Init(bb->Phi_list())) {
	...
	if (MTYPE_is_unsigned(sym->Mtype()))
      ...
      MTYPE desc = rtype;
#if defined( KEY) // promote to register type size
...
OPT_REVISE_SSA::Fold_lda_iloads(CODEREP *cr)
{
  ...
  case CK_IVAR:
    ...
    dsctyp = cr->Dsctyp();
#ifdef KEY
    // 15099: Convert: U8U4LDID --> U8U4CVT(U4U4LDID)
    //                 I8I4LDID --> I8I4CVT(I4I4LDID)
    if ((  dtyp == MTYPE_U8 ||   dtyp == MTYPE_I8) &&
	(dsctyp == MTYPE_U4 || dsctyp == MTYPE_I4)) {
    ...
    if (x->Field_id() == 0 && dsctyp != MTYPE_M && 
	TY_size(x->Lod_ty()) != MTYPE_byte_size(dsctyp))
      x->Set_lod_ty(MTYPE_To_TY(dsctyp));
#endif
    if (dsctyp == MTYPE_BS) // cannot use offset from opt_stab
...
#ifdef KEY
    // 15099: Insert CVTL if necessary for correctness
    if (need_cvt == NEED_CVT) {
      CODEREP *cvt_cr = Alloc_stack_cr(0);
      cvt_cr->Init_expr(opc, x);
