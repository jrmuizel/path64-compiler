...
ANL_VARLIST::_Binary_Search(INT32 id, mUINT32 from, mUINT32 till)
{
   ...
   else
   {
      ...
      const INT32   id2 = _vlist[halfway]->Id();
...
ANL_VARLIST::Find(ST *st)
{
   ...
   if (idx < _vlist.size() && id == _vlist[idx]->Id())
      found = _vlist[idx];
   else
      found = NULL;
...
ANL_VARLIST::Find_or_Insert(ST *st)
{
   ...
   else
   {
      ...
      if (idx >= _vlist.size())
      {
	 _vlist.push_back(var);
	 added=TRUE;
	 found = _vlist[_vlist.size()-1];
      }
      else if (id < _vlist[idx]->Id())
      {
	 _vlist.insert(_vlist.begin()+idx,var);
	 added=TRUE;
	 found = _vlist[idx];
      }
      else
      {
	 _vlist.insert(_vlist.begin()+idx+1,var);
	 added=TRUE;
	 found = _vlist[idx+1];
      }
      if (!added)
