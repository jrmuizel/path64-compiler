...
/////////////////////////////////////
{
  if ( TN_Is_Allocatable(tn) && lrange_mgr.Get(tn) == NULL ) {
    ...
    #else
    if (Get_Trace(TP_SWPIPE, GRA_FLAG)) {
    #endif
...
/////////////////////////////////////
{
  ...
  FOR_ALL_ISA_REGISTER_CLASS( rc ) {
    ...
    LRANGE** vec_paired =
      TYPE_MEM_POOL_ALLOC_N(LRANGE*,&MEM_local_nz_pool, REGISTER_MAX + 1);
    memset(vec_paired, 0, (REGISTER_MAX + 1) * sizeof(LRANGE*));
...
/////////////////////////////////////
{
  if ( TN_is_dedicated(tn)
#if defined( KEY) && !defined(TARG_ST)
       // Check for local TNs allocated by SWP.
       || (!TN_is_global_reg(tn) &&
	   TN_register(tn) != REGISTER_UNDEFINED)
#endif
      ) {
...
#if defined( KEY) && !defined(TARG_ST)
      // For GTNs with assigned registers (such as incoming parameters), track
      // the liveness of the assigned register.
      if (TN_register(op_tn) != REGISTER_UNDEFINED) {
	TN *dedicated_tn = Build_Dedicated_TN(TN_register_class(op_tn),
					      TN_register(op_tn), 0);
	GRA_PREF_LIVE *dedicated_gpl =
	  (GRA_PREF_LIVE*) hTN_MAP_Get(live_data, dedicated_tn);
	if (!dedicated_gpl) {
	  dedicated_gpl = gra_pref_mgr.LIVE_Create(&MEM_local_nz_pool);
	  hTN_MAP_Set(live_data, dedicated_tn, dedicated_gpl);
	}
	if (dedicated_gpl->Num_Defs() == 0) {
	  dedicated_gpl->Exposed_Use_Set(TRUE);
	}
      }
#endif
    }
...
/////////////////////////////////////
{
  ...
  for (op_iter.Init(gbb); ! op_iter.Done(); op_iter.Step()) {
	...
	if (OP_uniq_res(op, i)) {
#else
	if (OP_uniq_res(op) && !OP_cond_def (op)) {
#endif
	  remove_after = TRUE;
	} 
