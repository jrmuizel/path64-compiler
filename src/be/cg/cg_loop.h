 ...
 *	Requires: bb == CG_LOOP_prolog || bb == CG_LOOP_epilog.
 *	Add a existing backpatch to the <bb>.
 *
 ...
 *	Requires: bb == CG_LOOP_prolog || bb == CG_LOOP_epilog
 *	Remove all backpatches from <bb>.
 *
...
inline INT
OP_omega (const OP *op, INT opnd) {
  Is_True (_CG_LOOP_info(op) != 0, 
           ("OP_omega: Omega map isn't initialized."));

    return _CG_LOOP_info(op)->omega[opnd];
}

inline void Set_OP_omega(OP *op, UINT8 opnd, UINT8 omega)
{
  _CG_LOOP_INFO *info = _CG_LOOP_info(op);
  Is_True (info != 0, ("Set_OP_omega: Omega map isn't initialized"));
...
void CG_LOOP_Backpatch_Trace(BB *bb, CG_LOOP_BACKPATCH *bp, FILE *f = TFile);
...
void CG_LOOP_Init_Op(OP *op);
#ifdef KEY
void CG_LOOP_Copy_Loop_Info (OP *op_dest, const OP *op_src);
#endif
void CG_LOOP_Init_OPS(OPS *ops);
...
#ifdef KEY
  BB         *post_remainder_loophead;
  UINT32     post_ntimes;	// ntimes for post-epilogue remainder loop
  VECTOR     backpatch_post_remainder_TNs;	// Add these TNs if generating
						// post-remainder loop.
#endif
...
public:
...
#ifdef KEY
  void  Set_epilog_end(BB *bb)	{ epilog_end = bb; }
#endif
...
#ifdef KEY
  BB  *Post_remainder_loophead() const { return post_remainder_loophead; }
  UINT32 Post_ntimes()		{ return post_ntimes; }
  VECTOR Backpatch_post_remainder_TNs()	{ return backpatch_post_remainder_TNs;}
#endif
  ...
  void Verify();
#ifdef KEY
  void Determine_Post_Ntimes();
  void Backpatch_Post_Remainder_Add(TN *);
#endif
