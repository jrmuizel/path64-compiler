				  ...
				  TYPE_ID mtype, OPS *ops)
{
  ...
  if (!is_double && CG_idivbyconst_opt) {
    UINT32 signed_boundary = ((UINT32)INT_MAX) + 1U;  //0x8000000
    if (is_signed) {
      UINT32 d = labs((long)denom_val); //!is_double, so casts to long and UINT32 are safe
      UINT l = log2(d);
      UINT s, a;
      UINT64 m;
      ...
      j = (signed_boundary % d);
      k = (((UINT64)(1)) << (32 + l)) / (signed_boundary - j);
	...
	if( denom_val < 0 ) 
      ...
      return TRUE;
    } else if ((UINT32)denom_val < (UINT32)signed_boundary) { // !is_signed and special case
      UINT32 l, n, t, d, m, s, a, r;
      UINT64 m_low, m_high, j, k;

      n = 0;
      t = d = (UINT32)denom_val;
	...
	n++;
      ...
      l = log2(t) + 1;
      j = (((UINT32)(UINT_MAX)) % ((UINT32)(t)));  //0xffffffff % t
      k = (((UINT64)(1)) << (32 + l)) / (UINT_MAX - j);
