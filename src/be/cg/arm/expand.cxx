...
)
{
  ...
  Expand_Immediate (tmp, src, MTYPE_signed(mtype), ops);
...
)
{
  ...
  if (TN_is_constant(src2)) {
	...
	Expand_Immediate(tmp, src2, MTYPE_signed(mtype), ops);
        src2 = tmp;
	new_opcode = TOP_add_r_r_npc;
      }
    }
...
)
{
  ...
  if (TN_has_value(src1)) {
    Expand_Immediate(tmp, src1, MTYPE_signed(mtype), ops);
    src1 = tmp;
...
)
{
  ...
  if (TN_Has_Value(src)) {
    if (TN_Value(src) == 0) { 
      Expand_Immediate(dest, Get_Zero_TN(4), MTYPE_signed(MTYPE_I4), ops);
    } else {
      Expand_Immediate(dest, Gen_Literal_TN(1, 4), MTYPE_signed(MTYPE_I4), ops);
    }
    return;
  }
...
)
{
  ...
  if (imm == 0) {
    Expand_Immediate(dest, Gen_Literal_TN(0, 4), MTYPE_signed(MTYPE_I4), ops);
    return TRUE;
