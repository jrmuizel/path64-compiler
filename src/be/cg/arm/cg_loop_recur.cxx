...
)
{
  ...
  for (p = first; p != last; p++) {
    ...
    if (trace) {
      fprintf(TFile, "<recur> TN%d replaces the %"SCNdPTR"-th occurrence of TN%d\n",
...
public:
  ...
  RECUR_OP_DESC(BB *body, BB *epilog, OP *operation, CG_LOOP_DEF& tn_def, 
    ...
    if (action != RECUR_NONE && action != RECUR_BACK_SUB_VARIANT) {
      ...
      for (new_omega = std::max(1, CG_LOOP_recurrence_min_omega); new_omega < illegal_omega_value; new_omega++) {
  ...
  INT ii) : mindist(v.size(), vector<INT>(v.size(), NEG_INF))
{
  ...
  for (i = 0; i < v.size(); i++) {
	...
	Is_True(succ == v[SWP_index(succ)].op, ("MinDIST: wrong SWP_index."));
      ...
      mindist[start][i] = 0;
      mindist[i][stop] = 0;
      mindist[i][branch] = std::max(mindist[i][branch], 0);
    }
  }
