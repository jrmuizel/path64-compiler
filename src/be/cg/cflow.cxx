...
Format_Succ(BB *bb, INT isucc)
{
  ...
  if (nsuccs == 0) {
    ...
    if (offset != 0) {
      len += sprintf(buf + len, "+%" SCNd64 "", offset);
    }
    if (freqs_computed) {
...
Convert_Goto_To_Return ( BB *bp )
{
  ...
  if ( CFLOW_Trace_Branch ) {
    #pragma mips_frequency_hint NEVER
    fprintf(TFile, "changing BB:%d from GOTO-BB:%d+%" SCNd64 " to RETURN\n",
		   BB_id(bp),
		   BB_id(targ),
		   offset);
  }
...
CFLOW_Initialize(void)
{
  ...
  if (CGTARG_Branch_Always_Predicted_Taken ()) {
    ...
    br_taken_cost = fixed + (INT32) ((1.0 - factor) * taken + 
                                     factor * mispredict);
    br_fall_cost  = fixed + (INT32) (factor * mispredict);
  }
#else
  br_taken_cost = fixed + taken;
  br_fall_cost = fixed;
...
// 14813: Labels can have offsets (to support branch delay slot).
static void
Build_Long_Goto(TN *tn_label, OPS *ops)
{
  // Build this sequence:
  ...
  LABEL_IDX label_idx = TN_label(tn_label);
  ...
  INT64 offset = TN_offset(tn_label);
  if (MTYPE_byte_size(Pointer_Mtype) == 8) {	// 64-bit address
    ...
    TN *target_tn = Gen_Symbol_TN(st, offset, TN_RELOC_GOT_DISP);
    Build_OP(TOP_ld, tmp_reg, GP_TN, target_tn, ops);
  } else {					// 32-bit address
    TN *target_hi_tn = Gen_Symbol_TN(st, offset, TN_RELOC_HIGH16);
    TN *target_lo_tn = Gen_Symbol_TN(st, offset, TN_RELOC_LOW16);
...
CFLOW_Fixup_Long_Branches()
{
  ...
  for (bb = REGION_First_BB; bb; bb = BB_next(bb)) {
	...
	// 14813: Find target branch label (including offset)
	TN *tn_label = NULL;
	OP *op_branch = BB_branch_op(bb);
	if (op_branch) {
	  for (INT i = 0; i < OP_opnds(op_branch); ++i) {
	    if (TN_is_label(OP_opnd(op_branch, i))) {
	      tn_label = OP_opnd(op_branch, i);
	      break;
	    }
	  }
	}
	if (tn_label == NULL) {
	  tn_label = Gen_Label_TN(Gen_Label_For_BB(targ_bb), 0);
	}
