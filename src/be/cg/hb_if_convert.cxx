  ...
  if (pred_tn && pred_tn != True_TN) {
      ...
      default:
	{
	  if (OP_load(op)) {
		...
		if (TN_register_class(pred_tn) == ISA_REGISTER_CLASS_fcc)
		  Build_OP(TOP_movf, OP_result(op, 0), res_tn, pred_tn, 
                           OP_result(op, 0), &new_ops);
		else
		  Build_OP(TOP_movz, OP_result(op, 0), res_tn, pred_tn,
                           OP_result(op, 0), &new_ops);
	      } else {
		if (TN_register_class(pred_tn) == ISA_REGISTER_CLASS_fcc)
		  Build_OP(TN_size(res_tn)==4?TOP_movf_s:TOP_movf_d, 
			   OP_result(op, 0), res_tn, pred_tn, OP_result(op, 0), &new_ops);
		else
		...
		switch(OP_code(op)) {
		case TOP_movn:
		  Build_OP(TOP_movz, OP_result(op, 0), OP_opnd(op, 0), 
                           tmp_tn, OP_result(op, 0), &new_ops);		
		  break;
		case TOP_movn_d:
