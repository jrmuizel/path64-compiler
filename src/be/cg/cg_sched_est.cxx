...
Calc_Latency_To(CG_SCHED_EST *se, OP *op)
{
  ...
  if (old_entry != LATENCY_IGNORE) {
    ...
    memset(found_pred_for_opnd, 0, op_opnds * sizeof(mBOOL));
...
Avg_Cost_Path(BB_SET *region, BB *entry, BB_MAP ests, SCHED_EST_TYPE type)
{
  ...
  FOR_ALL_BB_SET_members(region, cur_bb) {
    ...
    // Disable adjusting bias factor in presence of SWP. This is to reduce
    // side-effects of enabling SWP.
    BOOL Local_Enable_SWP = Enable_SWP;
    Enable_SWP = FALSE;
#endif
