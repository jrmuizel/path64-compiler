...
)
{
 ...
 #if defined(BUILD_OS_DARWIN)
        *buf = vstr_concat(*buf, underscorify(ST_name(st)));
...
#ifdef TARG_X8664
    vstr_sprintf (buf, vstr_len(*buf),  (hexfmt ? "0x%" SCNx64 "" : "%" SCNd64 ""), value );
#else
#ifdef TARG_ST
...
//         mask as an operand. So, I need this hack ...
  ...
  */
  if (Assembly || Lai_Code )
      fprintf (Output_File, "\t%s", ASM_CMNT);
#endif
  if (OP_scycle(op) >= 0)
  ...
  INT32	repeat)		/* Repeat count */
{
  ...
  for ( i = 0; i < repeat; i++ ) {
	...
	fprintf (Output_File, "\t%s\t", 
		(scn_ofst % address_size) == 0 ? 
		AS_ADDRESS : AS_ADDRESS_UNALIGNED);
	fputs (LABEL_name(lab), Output_File);
	if (lab_ofst != 0)
		fprintf (Output_File, " %+" SCNd64 , lab_ofst);
	fputc ('\n', Output_File);
    } 
    if (Object_Code) {
  ...
  )
{
  ...
  for ( i = 0; i < repeat; i++ ) {
    if (Assembly) {
      ...
      fputs (LABEL_name(lab1), Output_File);
      fputc ('-', Output_File);
...
#if !defined(TARG_MIPS) && !defined(TARG_MVP) && !defined(TARG_ST)
#ifdef TARG_X8664
...
Process_Initos_And_Literals (SYMTAB_IDX stab)
{
  ...
  for (st_iter = st_list.begin(); st_iter != st_list.end(); ++st_iter) {
    ...
    if (st_inito_entry != st_inito_map.end()) {
      ...
      if (Emit_Global_Data) {
	fprintf(Asm_File, "\t%s\t", AS_GLOBAL);
	EMT_Write_Qualified_Tcon_Name(Asm_File, st);
	fprintf(Output_File, "\n");
	// [CL] output as hidden/internal so that we are close to the
	// export_local meaning
#ifdef AS_INTERNAL
	if (ST_export(st) == EXPORT_LOCAL_INTERNAL) {
	  fprintf (Asm_File, "\t%s\t", AS_INTERNAL);
	  EMT_Write_Qualified_Tcon_Name(Asm_File, st);
	  fprintf(Asm_File, "\n");
	} else
#endif
...
#  ifdef AS_MOVEABLE
	...
	moveable = TRUE;
      ...
      fprintf (Asm_File, ", %s", AS_TYPE_OBJECT);
      if (moveable) {
	fprintf (Asm_File, ", %s", AS_MOVEABLE);
      }
#  ifdef AS_USED
      /* TB: add gnu used  attibute when needed */
      if (ST_is_used(st)) {
	fprintf (Asm_File, ", %s", AS_USED);
      }
#endif
      fprintf (Asm_File, "\n");
#  else //AS_MOVEABLE
...
#else //Not TARG_ST
#ifdef MERGED_CODE
      fprintf(Asm_File, "%s_UNNAMED_CONST_%d:\n", Local_Label_Prefix, ST_tcon(st));
#endif
#endif
  ...
  FOREACH_SYMBOL (CURRENT_SYMTAB, sym, i) {
    ...
    if (ST_sclass(sym) == SCLASS_FORMAL) {
      ...
      */
      ...
      fprintf (Lai_File, "\t%s\t.input %s at %s, %ld\n", 
...
//         fprintf ( Asm_File, "\n\t%s Performance optimization level: %d\n", ASM_CMNT, PU_perf_opt(Get_Current_PU()) );
 ...
 if (Lai_Code) {
  ...
  FOREACH_SYMBOL (CURRENT_SYMTAB, sym, i) {
	...
	if (Has_Base_Block(sym)) {
	  Base_Symbol_And_Offset(sym, &base, &ofst);
	  fprintf (Lai_File, "\t%s\t.auto %s at %s, %ld\n", 
                        ASM_CMNT_LINE, ST_name(sym), ST_name(base), ofst);
	}
      }
    }
