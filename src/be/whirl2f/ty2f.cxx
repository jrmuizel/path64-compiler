		 ...
		 STAB_OFFSET  fld_ofst)
{
   ...
   Append_Token_Special(tokens, '(');
   Append_Token_String(tokens, Number_as_String(fld_ofst+1, "%" SCNd64 ""));
   Append_Token_Special(tokens, ')');
   Append_Token_Special(tokens, ',');
...
TY2F_scalar(TOKEN_BUFFER decl_tokens, TY_IDX ty_idx)
{
   ...
   if (TY_size(ty) > 0)
   {
	 ...
	 kind_spec = Concat3_Strings("(",Number_as_String(kind_type, "%" SCNd64 ""),")");
	 ...
	 Prepend_Token_String(
			      ...
			      Concat3_Strings(base_name,
					      "*", 
					      Number_as_String(TY_size(ty), "%" SCNd64 "")));
      }
   }
   else
   {
			...
			STAB_OFFSET  arr_ofst)
{
  ...
  if (TY_Is_Character_String(arr_ty_idx))
	...
	*/
      Append_Token_String(tokens, Number_as_String(arr_ofst+1, "%" SCNd64 ""));
      Append_Token_Special(tokens, ':');
      Append_Token_String(tokens, Number_as_String(arr_ofst+1, "%" SCNd64 ""));
    }
	...
	if (arr_ofst == 0)
	  {
	    Prepend_Token_String(idx_tokens, Number_as_String(1LL, "%" SCNd64 ""));
	  }
	else if (ARB_const_stride(arb)) /* Constant stride */
	  {
	    idx = arr_ofst/ARB_stride_val(arb) + 1;
	    Prepend_Token_String(idx_tokens, Number_as_String(idx, "%" SCNd64 ""));
