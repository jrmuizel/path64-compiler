...
public:
  Linenum_Pusher(INT64 new_line_number) {
    if (new_line_number <= 0)
      Fail_FmtAssertion(
        "Linenum_Pusher::Linenum_Pusher() : invalid line number %" SCNd64 "",
        new_line_number);
...
Create_Copyfunc(ST *struct_st)
{
  ...
  sprintf ( st_name, "__%s_%" PRIuPTR , construct_type_str, ST_st_idx(struct_st));
                   ...
                   WN *ldid_lock)
{
  ...
  char st_name[32];
  sprintf ( st_name, "__mp_cpprv_%" PRIuPTR , ST_st_idx(fld_st) );
    ...
    if (v->vtype == VAR_REDUCTION_SCALAR ||
      ...
      if ( (v->reduction_opr == OPR_CAND || v->reduction_opr == OPR_CIOR) &&
           (v->mtype == MTYPE_F4 || v->mtype == MTYPE_F8 ||
	          v->mtype == MTYPE_FQ || v->mtype == MTYPE_F16) ) {
      ...
      if ((( restype == MTYPE_I4 || restype == MTYPE_I8 ||
           ...
           (( v->mtype == MTYPE_I4 || v->mtype == MTYPE_I8 ||
             ...
             ( restype == MTYPE_F4 || restype == MTYPE_F8 ||
	             restype == MTYPE_FQ || restype == MTYPE_F16 )))
