...
SUMMARY_FORMAL::Print ( FILE* fp ) const
{
    ...
    fprintf ( fp, "ty_idx = 0x%" SCNx64 , Get_ty());
    if (Is_ref_parm ())
...
SUMMARY_ACTUAL::Print (FILE *f, INT32 id) const
{
    ...
    fprintf (f, " ty_idx = 0x%" SCNx64 ", ", Get_ty());
...
{ 
    ...
    case VALUE_INT_CONST:
	fprintf (f, "%" SCNd64 "", Get_int_const_value ());
	return;
	...
	fputs ("GLOBAL ", f);
        if (Is_global_st_idx()) { 
	  fprintf(f, "ST_IDX = %" PRIdPTR , Get_global_st_idx ());
        } else { 
	  if (symbol)
		  ...
		  fprintf (f, "ST_IDX = 0x%" PRIxPTR , Get_global_st_idx ());
	  }
        } 
	return;
...
SUMMARY_EXPR::Print (FILE *f) const
{
	...
	if (Get_kid () == 0) {
		...
		fprintf (f, " 0x%" SCNx64 "", Get_const_value ());
	} else {
	    fprintf (f, " 0x%" SCNx64 "", Get_const_value ());
	    Print_node (f);
	}
...
SUMMARY_COMMON_SHAPE::Print ( FILE *fp, INT32 id ) const
{
  fprintf ( fp, "COMMON_SHAPE[%d]:  ", id ); 
    ...
    if ( Is_kind_scalar() ) {
      fprintf ( fp, " scalar, element size = %d, offset = %" SCNd64 ", symbol_id = %d   \n", Get_element_size(), Get_offset(), Get_symbol_index());
    }
  ...
  fprintf ( fp, "FLD ACCESS [%d]: name:%s  ", id,Get_ty_name() ); 
	for(i=0; i<max_hot_num;i++){
		if(Get_hot_fld_id(i)==0) /*fld_id=1,2,...*/
			break;
      	fprintf ( fp, " field_id= %d, count = %" SCNd64 "  \n", Get_hot_fld_id(i),Get_hot_fld(i));
	}
