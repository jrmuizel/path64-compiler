...
GetName(char *common_name, INT64 offset)
{
  ...
  sprintf(Split_Name, "%s.%" SCNi64 , common_name, offset);
  ++Name_Count;
  return Split_Name;
}
...
#ifdef KEY
  // bug 12005: return the log (base 2) of the alignment, and if the
  // alignment is not a power of 2 then first convert it to the next
  // higher power of 2. This is because TY_align expects alignment
  // to be a power of 2. Arbitrary alignment derived from size can
  // cause random alignment settings.
  return Get_log_base2 (align);
#else
  return align;
#endif
}
