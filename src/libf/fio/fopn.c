...
/*
 *	Return value:
 *		SUPPRESS_REPEATS_UNSET, SUPPRESS_REPEATS_NO (0), or
 *		SUPPRESS_REPEATS_YES (1)
 *
 *	Side effects:
 *		initialize save_suppress_repeats variable to avoid calling
 *		getenv repeatedly
 */
suppress_repeats_t _Suppress_repeats() {
  static suppress_repeats_t save_suppress_repeats = SUPPRESS_REPEATS_UNKNOWN;
  if (save_suppress_repeats == SUPPRESS_REPEATS_UNKNOWN) {
    char *temp = getenv("FTN_SUPPRESS_REPEATS");
    save_suppress_repeats = temp ?
      (*temp == 'Y' || *temp == 'y') :
      SUPPRESS_REPEATS_UNSET;
  }
  return save_suppress_repeats;
}
#endif /* KEY Bug 2559 */
...
#ifdef KEY /* Bug 2559 */
	{
	/* When env var FTN_SUPPRESS_REPEATS is not set, default to whatever
	 * "assign" said, or to true. */
	suppress_repeats_t tmp = _Suppress_repeats();
	if (tmp != SUPPRESS_REPEATS_UNSET) {
		cup->ufrptcnt = tmp;
	}
	}
#endif /* KEY Bug 2559 */
