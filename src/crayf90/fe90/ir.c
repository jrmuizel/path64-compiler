...
|*	old_opnd							      *|
|*	new_opnd							      *|
|*	new_line	If nonzero, replace source line numbers with this     *|
|*	new_col		If new_line is nonzero, replace source columns with   *|
|*			this						      *|
|*									      *|
|* Output parameters:							      *|
|*	NONE								      *|
      ...
      switch(OPND_FLD((*old_opnd))) {
         ...
         case IR_Tbl_Idx :
            ...
            if (IR_OPR(idx) == Stmt_Expansion_Opr) {
               ...
               copy_subtree(&opnd_o, &opnd_n, new_line, new_col);
...
#ifdef KEY /* Bug 7969 */
               if (new_line) {
	         IR_LINE_NUM(new_root) = new_line;
	         IR_COL_NUM(new_root) = new_col;
	       }
#endif /* Bug 7969 */
...
#ifdef KEY /* Bug 7969 */
	       fld_type ftl = IR_FLD_L(new_root);
	       if (new_line && (ftl == AT_Tbl_Idx || ftl == CN_Tbl_Idx ||
	         ftl == SB_Tbl_Idx)) {
	         IR_LINE_NUM_L(new_root) = new_line;
	         IR_COL_NUM_L(new_root) = new_col;
	       }
#endif /* Bug 7969 */
...
#ifdef KEY /* Bug 7969 */
	       fld_type ftr = IR_FLD_R(new_root);
	       if (new_line && (ftr == AT_Tbl_Idx || ftr == CN_Tbl_Idx ||
	         ftr == SB_Tbl_Idx)) {
	         IR_LINE_NUM_R(new_root) = new_line;
	         IR_COL_NUM_R(new_root) = new_col;
	       }
#endif /* Bug 7969 */
            }
            break;
...
#ifdef KEY /* Bug 7969 */
	    if (new_line) {
	      IL_LINE_NUM(new_root) = new_line;
	      IL_COL_NUM(new_root) = new_col;
	    }
#endif /* Bug 7969 */
...
#ifdef KEY /* Bug 7969 */
               if (new_line) {
	         IL_LINE_NUM(list_idx) = new_line;
	         IL_COL_NUM(list_idx) = new_col;
	       }
#endif /* Bug 7969 */
...

{
   ...
   case IL_Tbl_Idx:
      ...
      copy_subtree(&loc_opnd, &new_opnd, 0, 0);
   ...
   *new_start_sh_idx = NULL_IDX;
   ...
   while (sh_idx != SH_NEXT_IDX(end_sh_idx)) {
      ...
      if (IR_OPR(SH_IR_IDX(sh_idx)) != Init_Opr &&
         ...
         copy_subtree(&opnd_o, &opnd_n, 0, 0);
