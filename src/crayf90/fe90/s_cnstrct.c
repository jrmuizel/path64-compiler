...

{
   ...
   if (IR_OPR(ir_idx) != Constant_Struct_Construct_Opr &&
      ...
      copy_subtree(&(exp_desc->char_len), &char_len_opnd, 0, 0);
...

{
   ...
   switch (OPND_FLD((*top_opnd))) {
      ...
      case CN_Tbl_Idx:
	    ...
	    // Originally this was handled just like "case Real:". But when
	    // the compiler is built with -m64, the real and imaginary parts
	    // of Complex_4 are aligned on 64-bit boundaries, yet
	    // num_host_wds is 1. Meaning of num_host_wds is not documented,
	    // so it's not clear whether changing it to 2 would fix many bugs
	    // yet to be found, or cause many bugs. For now, fix it this way.
	    case Complex: {
	       int iterations = num_host_wds[TYP_LINEAR(type_idx)];
	       if (iterations < 2) {
	         iterations = 2;
	       }
...

{
   ...
   if (count) {
      ...
      copy_subtree(&IL_OPND(IR_IDX_R(ir_idx)), &opnd, 0, 0);
...

{
   ...
   if (count) {
      ...
      copy_subtree(&(arg_info_list[info_idx].ed.char_len), &opnd, 0, 0);
      COPY_OPND(IR_OPND_L(ir2_idx), opnd);

      copy_subtree(&IL_OPND(list_idx), &opnd, 0, 0);
