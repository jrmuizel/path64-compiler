 ...
 * behavior.
 *
 * (Note that if we simply want to call a module procedure in the intrinsic
 * module, no entry in the following table is needed or wanted.)
 *
 * The two kinds of entries within the table are:
 *
 *  imt_inline		Use a procedure in the array intrinsic_semantics
 *			(definitions are in s_intrinsic.c) to synthesize IR
 *  imt_extern		Provide a declaration for an external procedure
 *			in the runtime system, so that code in s_call.c will
 *			emit a call to that external procedure
 *  imt_type		Mark a type as "intrinsic" so that it gets handled
 *			specially when converting from the data structures
 *			in the Cray part of the front end to the WHIRL
 *			symbol table in the SGI part of the front end
 *			(basically, for X8664 -m32 we must treat ISO_C_BINDING
 *			types C_PTR and C_FUNPTR as integers so that function
 *			return values are compatible with C "void *", rather
 *			than being passed via the parameter list.)
 *
 * An imt_inline entry must have a value other than Unknown_Intrinsic in the
 * "index" field so as to choose a procedure in intrinsic_semantics. An
 * imt_external entry may set "index" to Unknown_Intrinsic (in which case
 * we will simply emit a call to an external procedure) or it may set
 * "index" to select a procedure from intrinsic_semantics, in which case the
 * front end will first call that procedure and then (unless that procedure
 * intervenes) generate a call to an external procedure. This is useful when
 * there's some work to be done (validity-checking of arguments, for example)
 * before generating the call. It's particularly useful when the module has
 * used "!dir$ ignore_tkr <argname>" to disable generalized validity-checking
 * so that a procedure in intrinsic_semantics can perform more specific
 * checking.
 *
...
{
  /* Only interface, program unit, or type can be intrinsic */
