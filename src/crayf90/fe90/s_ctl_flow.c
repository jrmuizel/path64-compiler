...
# ifdef _SEPARATE_DEALLOCATES
   ...
   while (list_idx) {
      ...
      copy_subtree(&stat_loc_opnd, &stat_loc_opnd, 0, 0);
      ...
      if (OPND_FLD(stat_opnd) != NO_Tbl_Idx) {
         copy_subtree(&stat_opnd, &stat_opnd, 0, 0);
...
#ifdef KEY /* Bug 13662 */
	 /* According to comment in expr_sem() in s_asg_expr.c, "this means we
	  * are in some sort of openmp region rather than a cmic region". */
         if (cdir_switches.parallel_region && dump_flags.open_mp &&
	   OPND_FLD(cdir_switches.first_sh_blk_stk) == IL_Tbl_Idx) {
	   /* This big predicate is also borrowed from expr_sem(). */
	   if (ATD_CLASS(do_var_idx) != Struct_Component &&
	     ATD_CLASS(do_var_idx) != Constant &&
	     ATD_CLASS(do_var_idx) != Compiler_Tmp &&
	     ATD_CLASS(do_var_idx) != CRI__Pointee &&
	     (ATD_CLASS(do_var_idx) != Dummy_Argument ||
	       ! ATD_SF_DARG(do_var_idx)) &&
	       ! cdir_switches.autoscope &&
	       SB_BLK_TYPE(ATD_STOR_BLK_IDX(do_var_idx)) != Threadprivate &&
	       ! ATD_TASK_PRIVATE(do_var_idx) &&
	       ! ATD_TASK_GETFIRST(do_var_idx) &&
	       ! ATD_TASK_LASTLOCAL(do_var_idx) &&
	       ! ATD_TASK_REDUCTION(do_var_idx) &&
	       ! ATD_TASK_LASTTHREAD(do_var_idx) &&
	       ! ATD_TASK_FIRSTPRIVATE(do_var_idx) &&
	       ! ATD_TASK_COPYIN(do_var_idx) &&
	       ! ATD_TASK_LASTPRIVATE(do_var_idx) &&
	       ! ATD_TASK_SHARED(do_var_idx)) {
	     /* OMP 2.5 standard 2.8.1.1 "Sharing Attribute Rules for Variables
	      * Referenced in a Construct":
	      *   "The loop iteration variable in the do-loop of a do or
	      *   parallel do construct is private in that construct".
	      *
	      *   "Variables used as loop iteration variables in sequential
	      *   loops in a parallel construct are private in the parallel
	      *   construct."
	      * This doesn't quite say that a loop iteration variable in an
	      * OMP PARALLEL DO nested inside an OMP PARALLEL will also be
	      * private outside the PARALLEL DO but inside the PARALLEL;
	      * however, two separate customers both expect that to be the
	      * case, so it seems ok just to force it to be private at this
	      * spot. */
	     ATD_TASK_PRIVATE(do_var_idx) = TRUE;
	     ADD_VAR_TO_PRIVATE_LIST(do_var_idx);
	   }
	 }
#endif /* KEY Bug 13662 */
...
# ifdef _HIGH_LEVEL_DO_LOOP_FORM
         ...
         if (OPND_FLD(temp_opnd) == IR_Tbl_Idx) {
            copy_subtree(&temp_opnd, &temp_opnd, 0, 0);
         }
...
static
#endif /* KEY Bug 8502 */
void short_circuit_high_level_if(void)
...
# endif
   ...
   if (list_idx &&
      ...
      copy_subtree(&IL_OPND(IL_NEXT_LIST_IDX(list_idx)), &forall_mask_opnd, 0, 0);
...

{
   ...
   copy_subtree(&mask_opnd, &mask_opnd, 0, 0);
   ...
   while (sh_idx != IR_IDX_L(SH_IR_IDX(active_forall_sh_idx))) {
      if (SH_STMT_TYPE(sh_idx) == Assignment_Stmt) {
         ...
         copy_subtree(&lhs_opnd, &lhs_opnd, 0, 0);
