...

/*
 * Remove predicate of an "if" statement by assigning it to a compiler temp
 * and using the temp as the predicate instead. This is needed by the bounds
 * check code because a subscript might contain an operation like "count" or
 * "all" which is not acceptable to the WHIRL generator when it appears
 * directly as the predicate of an "if".
 *
 * if_sh_idx	sh_idx for "if" statement
 */
static void
isolate_if_predicate(int if_sh_idx)
{
  int save_sh_idx = curr_stmt_sh_idx;
  curr_stmt_sh_idx = if_sh_idx;
   ...
   for (i = 0; i < ATI_NUM_SPECIFICS(gen_idx); i++) {
      ...
      if (!no_func_expansion)   {
         ...
         if (! is_function) {
            ...
            ok = final_arg_work(&tmp_opnd, spec_idx, num_args, NULL,
...
# endif
         ...
         copy_subtree(left_opnd, &opnd, 0, 0);
...
#ifdef KEY /* Bug 7969 */
         gen_runtime_bounds(ir_idx, FALSE);
#else /* KEY Bug 7969 */
...
#ifdef KEY /* Bug 7969 */
         gen_runtime_substring(ir_idx, FALSE);
#else /* KEY Bug 7969 */
...
#ifdef KEY /* Bug 7969 */
/*
 * Generate runtime check for substring bounds
 * substring_idx	IL_Tbl_Idx for Substring_Opr
 * even_if_const	if true, generate check even if it could have been
 *			checked at compile time
 */
void gen_runtime_substring(int substring_idx, boolean even_if_const)
#else /* KEY Bug 7969 */
...
#ifdef KEY /* Bug 7969 */
   /* Avoid duplicates due to calls in both substring_opr_handler and
    * gen_runtime_checks */
   if (IR_BOUNDS_DONE(substring_idx)) {
     return;
   }
#endif /* KEY Bug 7969 */
...
#ifdef KEY /* Bug 7969 */
/*
 * Generate runtime check for subscript bounds
 * substring_idx	IL_Tbl_Idx for Subcript_Opr or Section_Subscript_Opr
 * even_if_const	if true, generate check even if it could have been
 *			checked at compile time
 */
void gen_runtime_bounds(int sub_idx, boolean even_if_const)
#else /* KEY Bug 7969 */
...
# endif
   ...
   while (list_idx != NULL_IDX) {
      ...
      if (ATD_IM_A_DOPE(attr_idx)) {
         ...
         copy_subtree(&lb_opnd, &opnd2, 0, 0);
...
#ifdef KEY /* Bug 7969 */
	 /* For final upper bound of assumed-size, use nil pointer to tell
	  * gen_[r]bounds_check_call() not to test against it. */
	 if (BD_ARRAY_CLASS(bd_idx) == Assumed_Size && dim == BD_RANK(bd_idx)) {
	    p_ub_opnd = 0;
	 }
	else
#endif /* KEY Bug 7969 */
      ...
      if (IL_FLD(list_idx) == IR_Tbl_Idx &&
         ...
         COPY_OPND(start_opnd, IL_OPND(list_idx2));
         copy_subtree(&start_opnd, &start_opnd, 0, 0);

         list_idx2 = IL_NEXT_LIST_IDX(list_idx2);
         COPY_OPND(end_opnd, IL_OPND(list_idx2));
         copy_subtree(&end_opnd, &end_opnd, 0, 0);

         list_idx2 = IL_NEXT_LIST_IDX(list_idx2);
         COPY_OPND(inc_opnd, IL_OPND(list_idx2));
         copy_subtree(&inc_opnd, &inc_opnd, 0, 0);
...

{
   ...
   gen_if_stmt(&cond_opnd, 
               ...
               col);
#ifdef KEY /* Bug 8502 */
   isolate_if_predicate(if_sh_idx);
#endif /* KEY Bug 8502 */
...

{
   ...
   /* subscript > ub */
#ifdef KEY /* Bug 7969 */
   /* Skip comparison against ub when it is null for last dimension of an
    * assumed-size array */
   if (ub_opnd == 0) {
     or_idx = lt_idx;
     ub_opnd = lb_opnd;
   ...
   if (glb_tbl_idx[Bounds_Attr_Idx] == NULL_IDX) {
      glb_tbl_idx[Bounds_Attr_Idx] = create_lib_entry_attr(
                                                   ...
                                                   col);
	/* Bug COMPILER-8762 */
	ATP_PURE(glb_tbl_idx[Bounds_Attr_Idx]) = TRUE;
   }
...

{
...
#ifdef KEY /* Bug 7969 */
   /* Assumed-size final upper bound: treat as if it were same as lower */
   if (ub_opnd == 0) {
     ub_opnd = lb_opnd;
   }
#endif /* KEY Bug 7969 */
   ...
   if (glb_tbl_idx[Rbounds_Attr_Idx] == NULL_IDX) {
      glb_tbl_idx[Rbounds_Attr_Idx] = create_lib_entry_attr(
                                                   ...
                                                   col);
	/* Bug COMPILER-8762 */
	ATP_PURE(glb_tbl_idx[Rbounds_Attr_Idx]) = TRUE;
   }
...
/*
 * even_if_const	Generate full runtime check even if it could have been
...

{
   ...
   if (glb_tbl_idx[Sbounds_Attr_Idx] == NULL_IDX) {
      glb_tbl_idx[Sbounds_Attr_Idx] = create_lib_entry_attr(
                                                   ...
                                                   col);
	/* Bug COMPILER-8762 */
	ATP_PURE(glb_tbl_idx[Sbounds_Attr_Idx]) = TRUE;
   }
      ...
      if (IR_OPR(ir_idx) == Subscript_Opr ||
         ...
         if (ATD_RESHAPE_ARRAY_OPT(attr_idx)) {
            gen_opnd(&opnd, ir_idx, IR_Tbl_Idx, IR_LINE_NUM(ir_idx),
                                                IR_COL_NUM(ir_idx));
            copy_subtree(&opnd, result_opnd, 0, 0);
            ir_idx = OPND_IDX((*result_opnd));
...
# endif
   ...
   copy_subtree(section_opnd, element_opnd, 0, 0);
...
#ifdef KEY /* Bug 8502 */
/* Return sh_idx for "if" statement */
int
#else /* KEY Bug 8502 */
void
#endif /* KEY Bug 8502 */
gen_if_stmt(opnd_type	*cond_opnd,
...
#ifdef KEY /* Bug 7969 */
/*
 * sub_idx	IR_Tbl_Idx for {Whole_,Section_,}Subscript_Opr
 * ignore_cdir_bounds	Ignore cdir_switches.bounds (set to make FE check
 *			constant bounds at compile time even in absence of -C
 *			or -ffortran-bounds-check)
 */
boolean	needs_bounds_check(int sub_idx, boolean ignore_cdir_bounds)
#else /* KEY Bug 7969 */
...

{
...
#ifdef KEY /* Bug 7969 */
   /* Skip comparison of upper bound of assumed-size array */
   if (ub_opnd == 0) {
     or_idx1 = lt_idx;
   }
   else {
   ...
   copy_subtree(&xt_opnd, &xt_opnd, 0, 0);
 ...
 * returns	TRUE if the entity is readonly because it has the PROTECTED
 *		attribute and is accessed by use association
 */
boolean
readonly_protected(int attr_idx) {
  return (!AT_IS_DARG(attr_idx)) && AT_PROTECTED(attr_idx) &&
