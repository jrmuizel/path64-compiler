...
#ifdef KEY /* Bug 9645 */
	 /* Evidently const_pool[const_pool_idx] = 0 normally clears the last
	  * word of the constant so that the string is null-terminated, but
	  * when extra_zero_word is true, the null terminator will be
	  * somewhere inside const_pool[const_pool_idx-1] instead. */
         if (extra_zero_word && CN_INT_TO_C(TYP_IDX(type_idx)) > 0) {
