...
fei_arith_con(TYPE type, SLONG *start)
{
  ...
  if (MTYPE_is_integral(bt)) {
    ...
    case MTYPE_C4 :  
      f  = (float *) start ;
      // fe90 constant pool aligns each constant on a "long" boundary; thus
      // there will be padding between real and imag parts when compiler is
      // built -m64
      tcon = Host_To_Targ_Complex_4 ( bt, *f,
        *(float *)(((char *)f) + sizeof(long)));
      break ;
