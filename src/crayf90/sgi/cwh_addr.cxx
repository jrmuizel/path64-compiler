   ...
   switch(cwh_stk_get_class()) {
    ...
    case DEREF_item:
	 ...
	 ty = cwh_types_dope_basic_TY(ty);
	  ...
	  may_be_noncontig) {
#ifdef KEY /* Bug 14150 */
      // Array dummy arg can't have "value" attr (F2003 C527), so no need to
      // check for that here
#endif /* KEY Bug 14150 */
...
#ifdef KEY /* Bug 7969 */
/*
 * The Cray fe90 front end always emitted bounds checks for variables in I/O
 * lists, and set IR_BOUNDS_DONE(sub_idx), which made cvrt_exp_to_pdg() in
 * fe90/i_cvrt.c clear the bound_chk argument which it passed to fei_xxx()
 * functions, which told the code in the SGI part of the front end not to
 * generate redundant WHIRL to check bounds.
 *
 * Now we have changed the front end to emit bounds checks for variables in
 * non-I/O contexts as well. Hopefully that causes cwh_addr_do_bounds_check()
 * and cwh_addr_insert_bounds_check() never to execute. If we ever discover
 * that they do execute (the evidence would be message lib-4964, which is used
 * by the SGI code but not the Cray fe90 code) we should try to generate the
 * checks in the Cray world instead, because message lib-4964 provides less
 * information, and because the following functions contain a bug which
 * erroneously reports an error for an empty slice or substring when an index
 * is out of bounds (the standard says that's not an error.)
 */
#endif /* KEY Bug 7969 */
...
#ifdef KEY /* Bug 14150 */
    } else if (ST_is_value_parm(st) && !ST_auxst_is_rslt_tmp(st)) {
       wn = cwh_addr_ldid(st,off,ts);
#endif /* KEY Bug 14150 */
    } else {
...
#ifdef KEY /* Bug 10468 */
      /* May have Cray pointer to derived type */
      if (ST_class(st)==CLASS_VAR && ST_auxst_is_auto_or_cpointer(st)) {
	/* need to load it through its base */
	wa = cwh_addr_address_ST(st);
	wn = cwh_addr_load_WN(wa,off,ts);
...
#ifdef KEY /* Bug 7722 */
/*
 * Replace first occurrence of old_node with new_node inside tree.
 * tree		Tree in which to search
 * old_node	Node to search for within tree
 * new_node	Node which will take its place
 * returns true if found and replaced
 */
static int
replace_node(WN *tree, WN *old_node, WN *new_node) {
  for (int i = 0; i < WN_kid_count(tree); i += 1) {
    if (WN_kid(tree, i) == old_node) {
      WN_kid(tree, i) = new_node;
      return 1;
    }
    if (replace_node(WN_kid(tree, i), old_node, new_node)) {
      return 1;
    }
  }
  return 0;
...
#ifdef KEY /* Bug 7722 */
    {
    /* For array of char, the node we want to replace is more deeply buried, so
     * search the entire tree */
    int found = replace_node(pa, s1d, wn);
    DevAssert((found), ("cwh_addr_nonc_util failed to replace node"));
    }
#else /* Bug 7722 */
      WN_kid1(pa) = wn;
