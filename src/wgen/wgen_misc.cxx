...
#endif
#ifdef KEY
extern std::vector<gs_t> guard_vars_to_be_initialized;
#endif
...
WGEN_Init (INT argc, char **argv, char **envp )
{
  ...
  if (TARGET_64BIT)
    ABI_Name = const_cast<char*>("n64"); // TARGET_64BIT should be defined somewhere
  else ABI_Name = const_cast<char*>("n32");
#endif
...
#ifdef KEY
  WGEN_Guard_Var_Init ();
  guard_vars_to_be_initialized.clear ();
#endif
...
// KEY: if CALL_STMT is TRUE, it is OK to close the current exception
// region. CALL_STMT == FALSE indicates adding this WN must not result
// in closing the current exception region.
void
