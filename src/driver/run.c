...
run_phase (phases_t phase, char *name, string_list_t *args)
{
    ...
    if (input != NULL && pex_input_file(pex, 0, input) == NULL) {
        ...
        if (ld_libraryn32_path) {
            asprintf(&l32_path, "%s:%s", my_path, ld_libraryn32_path);
        }
    ...
    errmsg = pex_run(pex, PEX_LAST | (save_stderr ? PEX_STDERR_TO_STDOUT : 0),
...
#endif
            case SIGTERM:
                error("%s died due to signal %s", name, strsignal(termsig));
                break;
            default:
                internal_error("%s died due to signal %s", name, 
					           strsignal(termsig));
                break;
        }
