...
// constructor maintains list of all resources.
  : count(count), name(name), id(total++), gname(const_cast<char*>("resource"))
{
  resources[id] = this;
}
...
INSTRUCTION_GROUP::INSTRUCTION_GROUP(const char* name)
    : id(count++),
      ...
      ii_res_req_gname(const_cast<char*>("ii_rr"))
{
  bad_iis[0] = 0;
  bad_iis[1] = 0;
...
std::vector<char*> TOP_SCHED_INFO_MAP::top_sched_info_ptr_map(TOP_count_limit,const_cast<char*>("0"));
std::vector<bool> TOP_SCHED_INFO_MAP::top_sched_info_defined(TOP_count_limit,false);
...
void TOP_SCHED_INFO_MAP::Create_Dummies( void )
{
  ...
  for ( int i = 0; i < (int)TOP_count_limit; ++i ) {
    if ( TOP_is_dummy((TOP)i) ) {
      if ( !dummies ) {
	dummies = new INSTRUCTION_GROUP(const_cast<char*>("Dummy instructions"));
