...
#include "defs.h"
#if !(defined(BUILD_OS_DARWIN) || defined(__sun) || defined(_WIN32))
#include "em_elf.h"
...
static OPTION_DESC Options_INTERNAL[] = {
    ...
    { OVK_NAME, OV_INTERNAL,	FALSE, "lang",			"",
      0, 0, 0,	&IPA_lang,	NULL },
    { OVK_NAME, OV_INTERNAL,	FALSE, "linker",		"",
      0, 0, 0,	&IPA_linker,	NULL },
#endif
...
#ifdef TARG_MIPS
INT32 CG_memmove_inst_count = 32;	/* intrinsic expansion of bzero etc */
INT32 CG_memmove_align_inst_count = 256;
#else
INT32 CG_memmove_inst_count = 16;	/* intrinsic expansion of bzero etc */
INT32 CG_memmove_align_inst_count = 16;
#endif
...
Configure (void)
{
  ...
  // bug 13998 - do this even under -mno-sse2; and for MIPS (14896)
  ...
  /**  NOTE: filename CAN BE NULL */
{
  ...
  if ( OPT_Space ) {
    ...
    if (!CG_memmove_align_inst_count_overridden)
      CG_memmove_align_inst_count = 8;
  ...
  if ( IEEE_Arithmetic > IEEE_ACCURATE ) {
    ...
    // On X8664, facerec fails at -O3 if Recip_Allowed is true
    // 14790: On MIPS, 453.povray fails at -Ofast if Recip_Allowed is true
    if ( ! Recip_Set )
      Recip_Allowed = IEEE_Arithmetic >= IEEE_INEXACT;
#endif
#if !defined(TARG_X8664) && !defined(TARG_MIPS)
    // On X8664, apsi fails at -O3 because Rsqrt_Allowed is true
    if ( ! Rsqrt_Set )
...
Build_Skiplist ( OPTION_LIST *olist )
{
  ...
  for ( ol = olist; ol != NULL; ol = OLIST_next(ol) ) {
    p = OLIST_val(ol);
    if ( p == NULL || p[0] == '\0' ) continue;  // 14880
  ...
  {
    if ( OLIST_val(ol) == NULL || OLIST_val(ol)[0] == '\0' ) {  // 14880
      --count;
      continue;
    }
