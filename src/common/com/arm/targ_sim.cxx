...
Update_Stack_Offsets( TYPE_ID mtype, PLOC *ploc ) {
  ...
  if(PLOC_on_stack(plocc)) {
    ploc->formal_offset = Current_Formal_Offset;
    ploc->upformal_offset = ROUNDUP(Current_UpFormal_Offset + PLOC_lpad(plocc), alignment);
    Current_UpFormal_Offset = ROUNDUP(ploc->upformal_offset + PLOC_size(plocc) + PLOC_rpad(plocc), alignment);
  }
  else {
    ploc->formal_offset = ROUNDUP(Current_Formal_Offset + PLOC_lpad(plocc), alignment);
    Current_Formal_Offset = ROUNDUP(ploc->formal_offset + PLOC_size(plocc) + PLOC_rpad(plocc), alignment);
  }
}
