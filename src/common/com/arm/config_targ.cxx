...
Abi_Name ( TARGET_ABI b)
{
  ...
  switch ( b ) {
    case ABI_ARM_ver1:     return const_cast<char*>("eabi-ver1");
    case ABI_ARM_ver2:      return const_cast<char*>("eabi-ver2");
    default:
      r = bnb[bnb_used].name;
...
Isa_Name ( TARGET_ISA b)
{
  ...
  switch ( b ) {
    case TARGET_ISA_armv5: return const_cast<char*>("armv5");
    case TARGET_ISA_armv6:  return const_cast<char*>("armv6");
    default:
      r = bnb[bnb_used].name;
      bnb_used = (bnb_used + 1) % 4;
      sprintf (r, "ISA_%d", b);
...
Targ_Name ( TARGET_PROCESSOR b)
{
  ...
  switch ( b ) {
    case TARGET_armv5: return const_cast<char*>("armv5");
    case TARGET_armv6:  return const_cast<char*>("armv6");
    default:
      r = bnb[bnb_used].name;
      bnb_used = (bnb_used + 1) % 4;
...
Configure_Target ()
{
  ...
  if (Use_32_Bit_Pointers) {
    AS_ADDRESS =  const_cast<char*>(AS_WORD);
    AS_ADDRESS_UNALIGNED =  const_cast<char*>(AS_WORD);
  } else {
    AS_ADDRESS =  const_cast<char*>(AS_DWORD);
    AS_ADDRESS_UNALIGNED =  const_cast<char*>(AS_DWORD_UNALIGNED);
  }
...
BOOL Check_Asm_Constraints(char* constraint_string, UINT8 mtype)
{
  ...
  const char *mtype_name = MTYPE_name(mtype);
