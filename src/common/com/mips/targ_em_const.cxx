		 ...
		 INTSC loc )	    /* Location (i.e. offset from some base) */
{
	...
	if (CG_emit_non_gas_syntax) {
	  ...
	  Emit_Repeated_Constant ( fl, ((loc % 4) == 0
					? AS_WORD : AS_WORD_UNALIGNED),
				   TCON_v0(tc), rc, 4 );
	  Emit_Repeated_Constant ( fl, ((loc % 4) == 0
					? AS_WORD : AS_WORD_UNALIGNED),
				   TCON_v1(tc), rc, 4 );
	...
	fprintf(fl, "\t%s float[2] %#g %#g\n", ASM_CMNT,
		TCON_R4(tc), TCON_R4(tc));
	rc = 0;
	break;
	}
