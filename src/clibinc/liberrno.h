...
#if defined(_HOST_OS_DARWIN) || defined(BUILD_OS_DARWIN) || defined(__FreeBSD__) || defined(__sun)
#include <errno.h>
...
#ifdef KEY /* F2003 */
/* ISO/IEC 1539-1:2004 13.8.2.5 requires a single value for any EOF, so
 * all of these map to the same number. The old messages and numbers are
 * dead, but the macro symbols are still in use in the library code.
 * The "awk" machinery for generating lib.cat won't allow us to refer to
 * FERDPEOF, unfortunately. */
#define FERDEMPT -(BASE+1)	/* Tried to read an empty file		*/
#define FERDENDR -(BASE+1)	/* Tried to read past endfile record	*/
#define FERDNLEF -(BASE+1)	/* Tried to read past EOF on namelist	*/
#define FERDIEOF -(BASE+1)	/* Tried to read past internal file EOF */
#define FE__RDEMPT -(BASE+2)	/* DEAD MESSAGE               		*/
#define FE__RDENDR -(BASE+3)	/* DEAD MESSAGE                     	*/
#define FE__RDNLEF -(BASE+4)	/* DEAD MESSAGE                      	*/
#define FE__RDIEOF -(BASE+5)	/* DEAD MESSAGE                         */
#else /* KEY F2003 */
...
/* F2003 also requires a single value for EOR, but officially the only
 * EOR occurs with ADVANCE='NO'; otherwise, exceeding the limits of the
 * record constitutes ERR not EOR. */
...
#define FWSTKPHYSMEM (BASE+982)	/* Physical memory: %" SCNd64 " bytes\nNumber of */
...
#define FWSTKBADMAX (BASE+986)	/* Bad maximum stack size limit of %" SCNd64 " */
...
#define FWSTKVSPHYS (BASE+990)	/* Your requested stack size limit is %" SCNd64 "%% */
