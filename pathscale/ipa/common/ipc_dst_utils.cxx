...
#ifdef KEY
UINT16
DST_copy_directories_and_files (DST_TYPE src, DST_TYPE dest, MEM_POOL * p)
{
  DST_IDX dest_dirs = DST_get_include_dirs(dest);
  DST_IDX src_dirs = DST_get_include_dirs(src);
  DST_IDX cur_include_dir;
  DST_IDX last_dir = DST_INVALID_IDX;
  UINT16 prev_include_dirs = 0;

  // traverse to the end of dirlist in dest
  while (dest_dirs != DST_INVALID_IDX)
  {
    last_dir = dest_dirs;
    prev_include_dirs++;
    dest_dirs = DST_INCLUDE_DIR_next(DST_get_include_dir(dest, dest_dirs));
  }

  // copy dirs
  cur_include_dir = last_dir;
  while (src_dirs != DST_INVALID_IDX)
  {
    cur_include_dir =
          DST_copy_include_dir(src, dest, p, cur_include_dir, src_dirs);
    src_dirs = DST_INCLUDE_DIR_next(DST_get_include_dir(src, src_dirs));
  }

  DST_IDX dest_files = DST_get_file_names(dest);
  DST_IDX src_files = DST_get_file_names(src);
  DST_IDX cur_filename;
  DST_IDX last_file = DST_INVALID_IDX;
  UINT16 prev_include_files = 0;

  // traverse to the end of filelist in dest
  while (dest_files != DST_INVALID_IDX)
  {
    last_file = dest_files;
    prev_include_files++;
    dest_files = DST_INCLUDE_DIR_next(DST_get_file_name(dest, dest_files));
  }

  // copy files
  cur_filename = last_file;
  while (src_files != DST_INVALID_IDX) {
    cur_filename = DST_copy_filename(src, dest, p, cur_filename, src_files,
                                     prev_include_dirs);
    src_files = DST_INCLUDE_DIR_next(DST_get_file_name(src, src_files));
  }

  return prev_include_files;
}
#endif
