		  ...
		  SUMMARY_VALUE &return_value) 
{
  if (expr->Has_const_operand () && expr->Get_kid () != operand) {
    ...
    // 15144: Use desc (not rtype) of OP, for incoming integers.
    TYPE_ID mtype = OPCODE_desc(expr->Get_opcode());
    if (! MTYPE_is_integral(mtype)) {
      mtype = expr->Get_mtype ();
    }
    return_value.Set_mtype (mtype);
    return;
  }
  ...
  for (UINT32 i = 0; i < Size; ++i) {
    if (!Top(i) && !Bottom(i)) {
      ...
      for (UINT32 j = 0; j < gvals.Elements(); ++j) {
        if (gvals[j].Value()) {
          fprintf(fp, "    offset = %" SCNd64 ", element_size = %d, value = ",
