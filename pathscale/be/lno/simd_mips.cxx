...
// NOTE: simd_mips.cxx was copied from simd.cxx, and then heavily modified
// to support MIPS paired single (MTYPE_V8F4).  The copy was made because
// simd.cxx contains a lot of x8664 specific code, and I didn't want to
// clutter up the code with a ton of "ifdef TARG_X8664/TARG_MIPS".
// Eventually, the target specific PU should be separated from the more
// general algorithmic PUs.
...
// 12056: Vectorized loop must iter at least once.
static BOOL Too_Few_Iterations(INT64 iters, WN *body)
{
  if (iters < Iteration_Count_Threshold) return TRUE;
...
// Return TRUE for operators which have paired single versions
static BOOL is_vectorizable_op (OPERATOR opr, TYPE_ID rtype, TYPE_ID desc) {
  ...
  case OPR_MIN:
  case OPR_SELECT:
    return (rtype == MTYPE_F4 ? TRUE : FALSE);

  case OPR_EQ: case OPR_NE:
  case OPR_LT: case OPR_GT:
  case OPR_LE: case OPR_GE:
    return (rtype == MTYPE_F4 && MTYPE_is_integral(rtype) ? TRUE : FALSE);
...
static SIMD_OPERAND_KIND simd_operand_kind(WN* wn, WN* loop)
{
  ...
  if (opr == OPR_PARM) {
    if (WN_Parm_By_Reference(wn)) return Reference;
    wn = WN_kid0(wn);
    opr = WN_operator(wn);
  }
  ...
  case OPR_ISTORE: {
    ...
    WN *wn_addr = WN_kid(wn, opr == OPR_ILOAD ? 0 : 1);
    if (opr == OPR_ILOAD && WN_kid_count(wn) != 1) return Complex;
    if (WN_offset(wn) != 0 || WN_operator(wn_addr) != OPR_ARRAY)
      return Complex;

    ACCESS_ARRAY* aa = (ACCESS_ARRAY*) WN_MAP_Get(LNO_Info_Map, wn_addr);
    ...
    for (INT i = 0; i < aa->Num_Vec(); ++i) {
	  ...
	  av->Non_Const_Loops() > loopno) {
	return Complex;
      }
      if (av->Loop_Coeff(loopno) != 0) {
	if (i != aa->Num_Vec() - 1) return Reference;
	if (seen_non_zero) return Complex;  // cannot have two non-zero
	seen_non_zero = TRUE;
      }
    }
    if (! seen_non_zero) return Invariant;
    return Simple;
  }
  }
...
static BOOL Is_Under_Array(WN *wn)
{
  ...
  while (parent && WN_operator(parent) != OPR_DO_LOOP) {
    if (WN_operator(parent) == OPR_ARRAY) return TRUE;
...
static BOOL Simd_Benefit (WN* wn) {
  ...
  if (LNO_Run_Simd == 0) return FALSE;
  if (LNO_Run_Simd == 2) return TRUE;

  if (wn == NULL) return FALSE;

  OPERATOR opr = WN_operator(wn);

  // Bug 5582: If the CVT is for array address calculation and can not be
  // vectorized we can not say CVT is beneficial.  Let others (e.g. alignment)
  // to make decision.
  if (opr == OPR_CVT &&
      (! Is_Under_Array(wn) ||
       is_vectorizable_op(opr, WN_rtype(wn), WN_desc(wn))))
    return TRUE;
  ...
  // Bug 8931 : according to (1), this routine is not just for alignment.
  // It also contains beneficial info.  So, I think if a loop contains a
  // intrinsic that can be vectorized, it is also ok if OPT:fast_math=on
  // or simd=2.
  ...
  if (WN_opcode(wn) == OPC_BLOCK) {
    WN *wn_stmt, *wn_next;
    for (wn_stmt = WN_first(wn); wn_stmt; wn_stmt = wn_next) {
      wn_next = WN_next(wn_stmt);
      if (Simd_Benefit(wn_stmt)) return TRUE;
    }
  } else {
    for (UINT kidno = 0; kidno < WN_kid_count(wn); kidno ++) {
      if (Simd_Benefit(WN_kid(wn, kidno))) return TRUE;
    }
  }
...
static BOOL Unit_Stride_Reference( WN *wn, WN *loop, BOOL in_simd )
{
  BOOL ok = TRUE;
  ...
  if (WN_operator(wn) == OPR_ARRAY &&
    ...
    for (INT i = 0; i < aa->Num_Vec(); ++i) {
      av = aa->Dim(i); //for each dimension
	...
	ok = FALSE;
      // bug 9858 : a(i+(i*(i+1))/2) -- array is too messy and contains the
      // index variable we can not vectorize it due to non-unit stride
      if (av->Too_Messy &&
  ...
  if (WN_opcode(wn) == OPC_BLOCK) {
    for ( WN* kid = WN_first(wn); kid; kid = WN_next(kid) ) {
      if (! Unit_Stride_Reference(kid, loop, in_simd)) return FALSE;
    }
  } else {
    for (UINT kidno = 0; kidno < WN_kid_count(wn); ++kidno) {
...
static BOOL Is_Well_Formed_Simd (WN* wn, WN* loop)
{
  ...
  if (WN_operator(wn) == OPR_ILOAD) {
    return (WN_operator(parent) == OPR_ISTORE ? TRUE : FALSE);
  }
  ...
  if (WN_kid_count(wn) > 1 &&
	...
	WN_operator(WN_kid0(array1)) != OPR_LDA) {
      ...
      // Bug 6649 - vectorize things like struct[index].array[loop_index]
      //            where is base is const + ARRAY
      if (WN_operator(WN_kid0(array1)) != OPR_ADD) return FALSE;
  ...
  if (WN_operator(kid0) == OPR_ILOAD &&
    ...
    if (stmt && WN_operator(stmt) == OPR_STID &&
	Is_Unroll_Statement(stmt, WN_do_body(loop)) &&
	WN_element_size(WN_kid0(kid0)) != MTYPE_byte_size(WN_desc(stmt)))
      return FALSE;
  }
  if (kid1 && WN_operator(kid1) == OPR_ILOAD &&
    ...
    if (stmt && WN_operator(stmt) == OPR_STID &&
	Is_Unroll_Statement(stmt, WN_do_body(loop)) &&
	WN_element_size(WN_kid0(kid1)) != MTYPE_byte_size(WN_desc(stmt)))
      return FALSE;
  }
  ...
  WN* stmt; // bug 2336 - trace up the correct type
  for (stmt = parent; stmt; stmt = LWN_Get_Parent(stmt)) {
    if (OPCODE_is_store(WN_opcode(stmt)) ||
	WN_operator(stmt) == OPR_DO_LOOP) break;
  }
  ...
  if (WN_operator(wn) != OPR_INTRINSIC_OP &&
    ...
    for (INT kid_count = 0; kid_count < WN_kid_count(wn); ++kid_count) {
...
static WN* Find_Do_Body (WN* simd_op)
{
  ...
  WN* parent = LWN_Get_Parent(body);
  while (parent) {
    if (WN_operator(parent) == OPR_DO_LOOP) break;
    body = parent;
    parent = LWN_Get_Parent(parent);
  }
  return body;
   ...
   FF_STMT_LIST& expandable_ref_list)
{
  ...
  if (total_loops >= 1) { // used to be > 1 09/14/2003
    ...
    for (i = 0; i < total_loops - 1; ++i) {
      ...
      for (INT j = 0; j < size; ++j) stmt = WN_next(stmt);
...
typedef enum { V8F4, INVALID } SIMD_KIND;
INT Vec_Unit_Size[2] = { 4, -1 };
...
Find_Simd_Kind ( STACK_OF_WN *vec_simd_ops )
{
  ...
  for (INT i = 0; i < vec_simd_ops->Elements(); ++i) {
    ...
    TYPE_ID type = WN_desc(istore);
    if (type == MTYPE_V) type = WN_rtype(istore);

    if (type == MTYPE_F4) smallest_kind = V8F4;
    else {
...
BOOL Is_Vectorizable_Intrinsic (WN *wn)
{
  return FALSE;  // No paired single MIPS library functions
}


// Returns FALSE if a non-vectorizable OP is encountered
// Push scalar vectorizable OPs onto simd_ops
BOOL Gather_Vectorizable_Ops(WN* wn, SCALAR_REF_STACK* simd_ops,
			     MEM_POOL *pool, WN *loop)
{
  // Check the statements in the block
  if (WN_opcode(wn) == OPC_BLOCK) {
    for (WN* wn_stmt = WN_first(wn); wn_stmt; wn_stmt = WN_next(wn_stmt)) {
      if (! Gather_Vectorizable_Ops(wn_stmt, simd_ops, pool, loop))
	return FALSE;
    }
    return TRUE;
  }
  ...
  if (is_vectorizable_op(opr, rtype, desc)) {
    if ((opr != OPR_INTRINSIC_OP && Is_Well_Formed_Simd(wn, loop)) ||
	(opr == OPR_INTRINSIC_OP && Is_Vectorizable_Intrinsic(wn))) {
  ...
  if (WN_operator(wn) == OPR_STID) {
	...
	while(parent && ! OPCODE_is_load(WN_opcode(parent)) &&
...
//    symbol 'sym' and OPERATOR type 'opr' and push their addresses on the
//    'stack'.
//-----------------------------------------------------------------------

static void Find_Nodes(OPERATOR opr, SYMBOL sym,
		       WN* wn_tree, STACK<WN*>* stack)
{
  if (WN_operator(wn_tree) == opr) {
    SYMBOL newsym = SYMBOL(wn_tree);
    if (newsym == sym) stack->Push(wn_tree);
  }
...
Simd_Replace_With_Constant(WN *copy, SYMBOL sym, WN *cons, TYPE_ID index_type)
{
  ...
  if (WN_operator(copy) == OPR_LDID) {
	...
	if (WN_kid(parent, kid) == copy) break;
      OPCODE intconst_opc
...
BOOL Analyse_Dependencies(WN* innerloop)
{
  ...
  for (stmt = WN_first(body);
       stmt && gather_status != -1; stmt = WN_next(stmt)) {
  ...
  for (EINDEX16 e = sdg->Get_Edge(); e; e = sdg->Get_Next_Edge(e)) {
    ...
    if (LWN_Get_Parent(source) == body || LWN_Get_Parent(sink) == body)
      ...
      dep_g_p->Add_Unique_Edge(stmt_to_vertex->Find(source),
  ...
  for (INT i = 1; i <= total_scc; ++i) scc_size[i] = 0;
...
static BOOL Loop_Has_Asm (WN* loop)
{
  ...
  for ( ; itr != NULL; itr = LWN_WALK_TreeNext(itr)) {
    if (WN_operator(itr->wn) == OPR_ASM_STMT) return TRUE;
  }
  return FALSE;
}
...
static BOOL Contain_Vectorizable_Intrinsic(WN *wn)
{
  ...
  if (WN_opcode(wn) == OPC_BLOCK) {
    for (WN* kid = WN_first(wn); kid; kid = WN_next(kid)) {
      if (Contain_Vectorizable_Intrinsic(kid)) return TRUE;
    }
    return FALSE;
  }
...
{
  if (LNO_Run_Vintr < 2) return FALSE; // only at level 2

  if (Loop_Has_Asm(innerloop)) return FALSE;
...
extern BOOL Is_Vectorizable_Loop (WN* innerloop)
{
  ...
  for (stmt = WN_first(body); stmt; stmt = WN_next(stmt)) {
    if (! Gather_Vectorizable_Ops(stmt, simd_ops,
				  &SIMD_tmp_pool, innerloop)) break;
  }
  BOOL _stop = stmt ? TRUE : FALSE;
...
extern void Mark_Auto_Vectorizable_Loops (WN* wn)
{
  ...
  if (opr == OPR_DO_LOOP) {
    ...
    for (wn_stmt = WN_first(wn); wn_stmt; wn_stmt = wn_next) {
      wn_next = WN_next(wn_stmt);
      Mark_Auto_Vectorizable_Loops(wn_stmt);
    }
  }
  else
    for (UINT kidno = 0; kidno < WN_kid_count(wn); ++kidno) {
...
// To facilitate vectorization, convert all ISTORE of CONST/LDID/ILOAD
// into ISTORE of PAREN of CONST/LDID/ILOAD.  Since PAREN nodes are later
// converted into NOPs, no new instructions are generated.  This enables
// vectorizer to assume PAREN is a vectorizable op and proceed.
static void Simd_Mark_Code (WN* wn)
{
  if ((WN_operator(wn) == OPR_ILOAD &&
	...
	OPCODE paren_opc = OPCODE_make_op(OPR_PAREN, desc, MTYPE_V);
	new_parent = LWN_CreateExp1(paren_opc, WN_kid(parent, 0));
      }
...
// Rename all reduction statements involving the variable st,
// at old_offset, with new offset.
...
static void Rename_Other_Reductions (WN_OFFSET orig_offset, ST *orig_st,
				     WN_OFFSET offset, WN *start, WN *last,
				     TYPE_ID vmtype)
{
  ST *st = WN_st(last);
  for (WN *stmt = start; stmt != last; stmt = WN_next(stmt)) {
...
// If innerloop cannot be vectorized, return FALSE and write an
// explanation message into verbose_msg.
static BOOL Simd_Pre_Analysis(WN *innerloop, char *verbose_msg)
{
  ...
  // If the loop index variable is live at exit, try to finalize it.
  // Skip loop if variable cannot be finalized.
  ...
  if (Do_Loop_Is_Mp(innerloop) && ! Early_MP_Processing) {
    ...
    // Find enclosing parallel region
    ...
    // Skip loop if region is REDUCTION OMP
  ...
  // Bug 9141: Improve simd diagnostics, also split unit stride checking
  // out of gathering simd ops
  ...
  if (! Simd_Benefit(WN_do_body(innerloop))) {
    sprintf(verbose_msg, "Vectorization is not likely to be beneficial"
	    " (try -LNO:simd=2 to vectorize it).");
    return FALSE;
  }
...
static void SA_Version_F90_Loops_For_Contiguous(WN *innerloop)
{
  ...
  if (if_noncontig) {
    WN *stmt_before, *parent_block, *wn_if;
	...
	enclosing_parallel_region
	  = LWN_Get_Parent(LWN_Get_Parent(enclosing_parallel_region));
#endif
      stmt_before = WN_prev(enclosing_parallel_region);
      FmtAssert(stmt_before, ("NYI"));
      parent_block = LWN_Get_Parent(enclosing_parallel_region);
      wn_if = Version_Region(enclosing_parallel_region, innerloop);
    } else {
      stmt_before = WN_prev(innerloop);
      parent_block = LWN_Get_Parent(innerloop);
      wn_if = Version_Loop(innerloop);
    }
    WN_if_test(wn_if) = if_noncontig;
    LWN_Insert_Block_After(parent_block, stmt_before, wn_if);
    LWN_Parentize(wn_if);
  } // end if_noncontig
}
...
static BOOL Simd_Analysis(WN *innerloop, char *verbose_msg)
{
  ...
  WN *stmt, *body = WN_do_body(innerloop);
  needs_scalar_expansion = FALSE;  // temporary global
  // Set global index type
  ...
  for (stmt = WN_first(body);
       stmt && curr_simd_red_manager; stmt = WN_next(stmt)) {
    if (WN_operator(stmt) == OPR_STID &&
	...
	sprintf(verbose_msg, "Vectorization requires scalar"
		" expansion (implemented under -LNO:simd=2).");
	return FALSE;
      }
    }
  }
  ...
  Init_Ref_Stmt_Counter();
  INT32 gather_status = 0;
  for (stmt = WN_first(body); stmt && gather_status != -1;
       stmt = WN_next(stmt)) {
    gather_status = New_Gather_References(stmt, writes, reads, stack1,
					  scalar_writes, scalar_reads,
					  params, &SIMD_default_pool);
  ...
  // it stores dependence relations between SCCs
  SCC_DIRECTED_GRAPH16 *ac_g
    = dep_g_p->Acyclic_Condensation(&SIMD_default_pool);

  VINDEX16 total_scc = dep_g_p->Get_Scc_Count();

  // scc[i] is a list of statemens in i-th SCC
  FF_STMT_LIST *scc
    = CXX_NEW_ARRAY(FF_STMT_LIST, total_scc + 1, &SIMD_default_pool);
  ...
  UINT_DYN_ARRAY* new_loops
    = simd_fis_merge_scc_to_form_new_loop(total_scc, scc, scc_size,
					  innerloop, ac_g);
...
static INT Simd_Align_Best_Peel(STACK_OF_WN *vec_simd_ops,
...
// Return TRUE if the upper bound is not known at compile time
static BOOL Simd_Align_UB_Variable(WN *innerloop)
{
  WN* end = WN_end(innerloop);
  SYMBOL loop_index(WN_index(innerloop));
  if (WN_kid_count(end) != 2) return TRUE;
  if (WN_operator(WN_kid0(end)) == OPR_LDID &&
      loop_index == SYMBOL(WN_kid0(end))) {
    if (WN_operator(WN_kid1(end)) != OPR_INTCONST) return TRUE;
  } else if (WN_operator(WN_kid1(end)) == OPR_LDID &&
	     loop_index == SYMBOL(WN_kid1(end))) {
    if (WN_operator(WN_kid0(end)) != OPR_INTCONST) return TRUE;
  }
  return FALSE;
}
			       ...
			       WN *innerloop, BOOL is_store)
{
  ...
  if (! const_lb) {
	...
	copy = WN_CreateIntconst(intconst_opc,
      ...
      Simd_Replace_With_Constant(copy, symbol,
				 WN_kid0(WN_start(innerloop)), index_type);
  }
  ...
  if (WN_operator(WN_array_base(array0)) == OPR_LDID) // maybe different
    ...
    alignment = -2;
	   ...
	   Vec_Unit_Size[simd_kind] != MTYPE_byte_size(WN_desc(load_store)))
    alignment = -2;
  else {
    if (aa0->Dim(aa0->Num_Vec() - 1)->
	Loop_Coeff(Do_Loop_Depth(innerloop)) == -1) {
      WN *wn = WN_CreateIntconst(OPCODE_make_op(OPR_INTCONST,
						index_type, MTYPE_V),
				 (16 / ABS(WN_element_size(array0))) - 1);
      TYPE_ID sub_mtype = Mtype_TransferSign(MTYPE_I4, index_type);
      copy = LWN_CreateExp2(OPCODE_make_op(OPR_SUB, sub_mtype, MTYPE_V),
			    copy, wn);
    }
	...
	(TY_kind(ST_type(st)) == KIND_POINTER && !Align_Unsafe &&
	 (!ST_pt_to_unique_mem(st) || ST_is_temp_var(st) ||
      ...
      alignment = -2;
	     ...
	     ST_sclass(st) == SCLASS_FORMAL_REF)
      alignment = -2;
    else if (base_st->sym_class == CLASS_BLOCK && alignment < 0) // Bug 2322
...
{
  TY_IDX ty_load_store
    = TY_pointed(is_load ? WN_load_addr_ty(load_store) : WN_ty(load_store));
  TY_IDX ty_idx = 0;
  TY &ty = New_TY (ty_idx);
...
static void Simd_Update_Index_Def_Use(WN *loop,WN *depend_loop,
				      WN *what, SYMBOL sym)
{
  DOLOOP_STACK sym_stack(&LNO_local_pool);
...
static void Simd_Update_Loop_Info(WN *loop, WN *orig_loop, DO_LOOP_INFO *dli,
				  BOOL set_unimportant)
{
...
static INT Simd_Count_Good_Vector(STACK_OF_WN *vec_simd_ops,
				  SIMD_KIND *simd_op_kind)
{
  INT good_vector = 0;
  for (INT i = 0; i < vec_simd_ops->Elements(); ++i) {
...
static void
Simd_Handle_Negative_Coefficient( WN *parent,/*shffle's parent*/
				  ...
				  BOOL no_shuffle )
{
  ...
  INT incr = 8 / ABS(WN_element_size(array));
  ACCESS_ARRAY* aa = (ACCESS_ARRAY *) WN_MAP_Get(LNO_Info_Map, array);
  if (aa->Dim(aa->Num_Vec() - 1)->Loop_Coeff(Do_Loop_Depth(loop)) == -1) {
    TYPE_ID vector_type = MTYPE_V8F4;
    WN *opnd = LWN_Get_Parent(array);
    if (ABS(WN_element_size(array)) != 4 || ! MTYPE_is_float(WN_desc(opnd))) {
      FmtAssert(FALSE, ("Simd_Handle_Negative_Coefficient: NYI"));
    }
    TYPE_ID index_type = WN_rtype(WN_end(loop));
    OPCODE intconst_opc = OPCODE_make_op(OPR_INTCONST, index_type, MTYPE_V);
    TYPE_ID sub_mtype = Mtype_TransferSign(MTYPE_I4, index_type);
    OPCODE sub_opc = OPCODE_make_op(OPR_SUB, sub_mtype, MTYPE_V);
    ...
    LWN_Parentize(array); // up to now is fine, we just create array
    if (no_shuffle == FALSE) {
...
static void Simd_Add_Shuffle_For_Negative_Coefficient(WN* simd_op, WN *loop)
{
  ...
  for (INT kid = 0; kid < WN_kid_count(simd_op); kid ++) {
    ...
    if (WN_operator(opnd) == OPR_ILOAD &&
	WN_operator(WN_kid0(opnd)) == OPR_ARRAY)
      Simd_Handle_Negative_Coefficient(simd_op, kid, WN_kid0(opnd),
				       loop, FALSE);
  }
  //handle parent
  WN *parent = LWN_Get_Parent(simd_op);
				    ...
				    WN *simd_op) //const_wn's parent
{
  FmtAssert(const_wn && (WN_operator(const_wn) == OPR_INTCONST ||
			 WN_operator(const_wn) == OPR_CONST),
  ...
  switch (type) {
  case MTYPE_F4: case MTYPE_V8F4:
    WN_set_rtype(const_wn, MTYPE_V8F4);
    break;
  }
...
static WN *Simd_Vectorize_Invariants(WN *inv_wn, WN *istore, WN *simd_op)
{
  TYPE_ID type = WN_desc(istore);
  if (type == MTYPE_V) type = WN_rtype(istore);
  ...
  if (type == MTYPE_V8F4 || type == MTYPE_F4) {
    OPCODE opc = OPCODE_make_op(OPR_REPLICATE, MTYPE_V8F4, MTYPE_F4);
    inv_wn = LWN_CreateExp1(opc, inv_wn);
  }
  return inv_wn;
}
				  ...
				  WN *innerloop, TYPE_ID index_type)
{
  ...
  for (INT k = 1, sum = add_to_base; k < unroll_times; ++k) {
	...
	Simd_Update_Copy_Array_Index(iload_copy, origA,
				     add_to_base, index_type);
      }
    ...
    Simd_Update_Copy_Array_Index(WN_kid1(copy), WN_kid1(istore),
				 add_to_base, index_type);
...
static BOOL Simd_Good_Reduction_Load(WN *innerloop, WN *load)
{
  ...
  FmtAssert(stmt && (curr_simd_red_manager || WN_operator(stmt) != OPR_STID),
	    ("NYI"));
  if (WN_operator(stmt) == OPR_STID &&
      curr_simd_red_manager->Which_Reduction(stmt) != RED_NONE &&
					   ...
					   TYPE_ID vmtype, INT kid)
{
  ...
  if (WN_operator(simd_op) == OPR_ADD ||
    ...
    TYPE_ID rep_mtype = Mtype_TransferSign(MTYPE_I4, desc);
  ...
  if (! Do_Loop_Is_Mp(innerloop))
	...
	!= WN_PRAGMA_PARALLEL_DO)
      LWN_Insert_Block_Before(LWN_Get_Parent(enclosing_parallel_region),
  ...
  if (MTYPE_is_integral(desc) && MTYPE_byte_size(desc) < 4)
    desc = MTYPE_I4;
  WN* reduce = LWN_CreateExp2(OPCODE_make_op(s_opr, WN_rtype(simd_op),
					     WN_desc(simd_op)),
...
// static WN *Simd_Vectorize_Induction_Variables(...)
// Induction variables are not paired single.
					   ...
					   BOOL *invarkid)
{
  if (WN_operator(simd_op) != OPR_CVT && WN_operator(simd_op) != OPR_TRUNC) {
    ...
    if (OPCODE_is_compare(WN_opcode(simd_op))) {
      WN_set_rtype(simd_op, MTYPE_V8I4);
    ...
    case MTYPE_I4: vec_desc = MTYPE_V8I4; break;
    case MTYPE_F4: vec_desc = MTYPE_V8F4; break;
    default: FmtAssert(FALSE, ("NYI"));
    }
    switch(WN_rtype(simd_op)) {
    case MTYPE_F8:
      vec_rtype = MTYPE_V8F4;
      if (vec_desc == MTYPE_V8I4) // bug 7334
	vec_desc = MTYPE_V8I4;
      else vec_desc = MTYPE_V8F4;
      break;
    case MTYPE_F4: vec_rtype = MTYPE_V8F4; break;
    case MTYPE_I4: vec_rtype = MTYPE_V8I4; break;
    default: FmtAssert(FALSE, ("NYI"));
    }
...
static void Simd_Finalize_Loops(WN *innerloop, WN *remainderloop,
				INT vect, WN *reduction_node)
{
  SYMBOL symbol(WN_index(innerloop));
  OPCODE intconst_opc = OPCODE_make_op(OPR_INTCONST, index_type, MTYPE_V);
  OPCODE add_opc = OPCODE_make_op(OPR_ADD, index_type, MTYPE_V);
  ...
  OPCODE new_cmp_opc = OPCODE_make_op(OPR_LE, WN_rtype(WN_end(innerloop)),
  ...
  if (opr == OPR_GE || opr == OPR_GT) {
    ...
    BOOL save_simp_state = WN_Simplifier_Enable(FALSE);
    WN_end(innerloop)
      = LWN_CreateExp2(new_cmp_opc, WN_kid1(WN_end(innerloop)),
  ...
  if (WN_kid0(loop_end) == tmp) {
	...
	if ((WN_const_val(tmp1) - WN_const_val(tmp2) + 1)%vect == 0)
      ...
      WN* tmp1 = WN_Simplify_Tree(LWN_Copy_Tree(WN_kid1(loop_end)));
      if (WN_operator(tmp1) == OPR_INTCONST) {
	if ((WN_const_val(tmp1) - WN_const_val(WN_kid0(loop_start)) + 1)
	    % vect == 0)
	  rloop_needed = FALSE;
      }
    }
    ...
    WN *wn;
    // old loop end
    wn = LWN_CreateExp2(sub_opc, WN_kid1(loop_end_tmp),
			WN_kid0(loop_start));
    wn = LWN_CreateExp2(add_opc, wn, WN_CreateIntconst(intconst_opc, 1));
    wn = LWN_CreateExp2(div_opc, wn, WN_CreateIntconst(intconst_opc, vect));
    wn = LWN_CreateExp2(mpy_opc, wn, WN_CreateIntconst(intconst_opc, vect));
    wn = LWN_CreateExp2(add_opc, wn, WN_CreateIntconst(intconst_opc, -1));
    WN_kid1(loop_end) = LWN_CreateExp2(add_opc, wn, WN_kid0(loop_start_tmp));

    // Adjust loop lower bound for serial remainder loop (if any)
    wn = LWN_CreateExp2(sub_opc, WN_kid1(loop_end_tmp_rloop),
			WN_kid0(loop_start_rloop));
    wn = LWN_CreateExp2(add_opc, wn, WN_CreateIntconst(intconst_opc, 1));
    wn = LWN_CreateExp2(div_opc, wn, WN_CreateIntconst(intconst_opc, vect));
    wn = LWN_CreateExp2(mpy_opc, wn, WN_CreateIntconst(intconst_opc, vect));
    WN *start = WN_start(remainderloop);
    WN_kid0(start) = LWN_CreateExp2(add_opc, wn,
    ...
    // old loop end
    WN *wn = LWN_CreateExp2(sub_opc, WN_kid0(loop_end_tmp),
			    WN_kid0(loop_start));
    wn = LWN_CreateExp2(add_opc, wn, WN_CreateIntconst(intconst_opc, 1));
    wn = LWN_CreateExp2(div_opc, wn, WN_CreateIntconst(intconst_opc, vect));
    wn = LWN_CreateExp2(mpy_opc, wn, WN_CreateIntconst(intconst_opc, vect));
    ...
    // Adjust loop lower bound for serial remainder loop (if any)
    wn = LWN_CreateExp2(sub_opc, WN_kid0(loop_end_tmp_rloop),
			WN_kid0(loop_start_rloop));
    wn = LWN_CreateExp2(add_opc, wn, WN_CreateIntconst(intconst_opc, 1));
    wn = LWN_CreateExp2(div_opc, wn, WN_CreateIntconst(intconst_opc, vect));
    wn = LWN_CreateExp2(mpy_opc, wn, WN_CreateIntconst(intconst_opc, vect));
    WN *start = WN_start(remainderloop);
    WN_kid0(start) = LWN_CreateExp2(add_opc, wn, WN_kid0(loop_start_rloop_tmp));
  }
  ...
  if (rloop_needed) {
	...
	enclosing_parallel_region =
	  LWN_Get_Parent(enclosing_parallel_region);
      FmtAssert(enclosing_parallel_region,
		("Simd: MP loop has no enclosing region!"));
	...
	region  = WN_CreateRegion (REGION_KIND_MP, body,
				   pragmas, exits, -1, 0);
	WN* pragma =
	  WN_CreatePragma(WN_PRAGMA_SINGLE_PROCESS_BEGIN, (ST_IDX)NULL, 0, 0);
	...
	if (region_pragma && WN_pragma(region_pragma) == WN_PRAGMA_NOWAIT) {
	  WN* nowait_pragma =
	    WN_CreatePragma (WN_PRAGMA_NOWAIT, (ST_IDX) NULL, 0, 0);
    ...
    // Remainder loop is not needed, dependences should be removed first
...
static BOOL Simd(WN* innerloop)
{
  INT good_vector = 0;

  // Pre_analysis filters out loops that cannot be vectorized
  ...
  // Note: Debug purpose only, may not be remapped to the source code
  Last_Vectorizable_Loop_Id += 1;
  ...
  MEM_POOL_Push(&SIMD_default_pool);
  {
	...
	for (INT k = 0; k < 3; ++k)
      ...
      for (INT k = 0; k < 3; ++k) {
	if (load_store[k] == NULL ||
	    simd_operand_invariant[k][second_indx] == 1)
	...
	continue;
      ...
      if (best_peel == 0)
	// Align iloads and istores
	Simd_Align_Array_References(vec_simd_ops, simd_op_kind,
	...
	if (simd_operand_invariant
	    [kid][vec_simd_ops->Elements() - i - 1] == 1 ||
	  ...
	  if (simd_operand_invariant
		...
		WN_operator(inv_node) == OPR_INTCONST) { // constant
	      ...
	      if (WN_operator(inv_node) == OPR_INTCONST &&
		  WN_const_val(inv_node) == 2 &&
		  WN_opcode(simd_op) == OPC_I4MPY) {
		inv_node = LWN_Copy_Tree(WN_kid(simd_op, 1 - kid),
					 TRUE, LNO_Info_Map);
		LWN_Copy_Def_Use(WN_kid(simd_op, 1 - kid), inv_node, Du_Mgr);
		LWN_Copy_Frequency_Tree(inv_node, WN_kid(simd_op, 1 - kid));
		WN_set_opcode(simd_op,
			      OPCODE_make_op(OPR_ADD, WN_rtype(simd_op),
					     MTYPE_V));
	    ...
	    if (SYMBOL(operand) == SYMBOL(WN_index(innerloop))) {
	      //induction variable
	      FmtAssert(FALSE, ("NYI"));
	    } else //all other ldid
...
// Invoke Simd on each elegible innermost DO_LOOP;
// Set Simd_Align if any Simd returns TRUE.
...
void Simd_Phase(WN* func_nd)
{
  ...
  if (debug) {
    fprintf(TFile, "%sLNO: \"WHIRL tree before simd phase\"\n", DBar);
  ...
  if (debug) {
    fprintf(TFile, "%sLNO: \"WHIRL tree after simd phase\"\n", DBar);
    fdump_tree(TFile, func_nd);
  }
