...
static BOOL Is_Shared_Or_Reduction_In_Prallel_Region(WN* scalar_ref)
{
  ...
  while (wn) {
    ...
    while (next_wn) {
	  ...
	  if (WN_st(scalar_ref)==WN_st(next_wn))
	    return TRUE;
       // bug 14739: a "shared" variable could be the default, otherwise
       // it must be in the shared pragma list. 
        }else  if ((WN_PRAGMA_ID)WN_pragma(next_wn)==WN_PRAGMA_LOCAL)
           if (WN_st(scalar_ref)==WN_st(next_wn))
    ...
    if(default_shared)
       return TRUE;
    wn=LWN_Get_Parent(mp_region); //parent for nested parallel regions
  }
  return FALSE;
}
#endif
