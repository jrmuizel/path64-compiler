...
{
   if (val1 != -1)
      Append_Token_String(tokens, Number_as_String(val1, "%" SCNd64 ""));

   if (val2 != -1)
   {
      Append_Token_Special(tokens, ',');
      Append_Token_String(tokens, Number_as_String(val2, "%" SCNd64 ""));
   }
} /* Append_Arg_Numbers */
                           ...
                           INT32        size)
{
   ...
   if (PF_GET_STRIDE_1L(pflag) > 0)
   {
      if (PF_GET_STRIDE_2L(pflag) > 0)
      {
         Append_Token_String(tokens, 
            ...
            Concat2_Strings(Number_as_String(PF_GET_STRIDE_1L(pflag), "%" SCNd64 ""),
            Concat2_Strings(",",
   ...
   if (size > 0)
   {
      Append_Token_Special(tokens, ',');
      Append_Token_String(tokens, 
         Concat2_Strings("size=", Number_as_String(size, "%" SCNd64 "")));
   }
