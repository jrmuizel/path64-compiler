...
LRANGE_MGR::Create_Complement( TN* tn )
{
  ...
  // Earlier phases such as SWP may have done partial register assignment.  In
  // that case, GRA must give the assigned register to the TN before giving
  // that register to other TNs.  Do this by raising the lrange's priority.
...
LRANGE::Allowed_Registers( GRA_REGION* region )
{
  ...
  case LRANGE_TYPE_COMPLEMENT:
    ...
    if (rc == GRA_avoid_register_class)
      allowed = REGISTER_SET_Difference1(allowed, GRA_avoid_register);
#endif
...
LRANGE::Reclaimable_Registers( GRA_REGION* region )
{
  ...
  case LRANGE_TYPE_COMPLEMENT:
    ...
    // The user wants GRA to avoid certain registers.  Bug 12625.
    if (rc == GRA_avoid_register_class)
      reclaimable = REGISTER_SET_Difference1(reclaimable, GRA_avoid_register);
#endif
