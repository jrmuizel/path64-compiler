...
Expand_Power_Of_2_Divide (TN *result, TN *numer, INT64 dvsr, TYPE_ID mtype, OPS *ops)
{
  ...
  if (MTYPE_is_unsigned(mtype)) {
    ...
    Expand_Add (t1, Gen_Literal_TN(absdvsr-1, is_double ? 8 : 4), 
		numer, mtype, ops);
    Build_OP(TOP_movz, t1, numer, p1, t1, ops);
				  ...
				  TYPE_ID mtype, OPS *ops)
{
  ...
  if (is_signed) {
     ...
     * and set p1 to 1 iff the numerator is negative:
     */
    TN *abs_tn = Build_TN_Of_Mtype (mtype);
    p1 = Build_TN_Of_Mtype (mtype);
    Build_OP (is_double ? TOP_dsrl32 : TOP_srl,
	      p1, numer_tn, Gen_Literal_TN(31, 4), ops);
    Build_OP (is_double ? TOP_dsubu : TOP_subu,
	      abs_tn, Zero_TN, numer_tn, ops);
    Build_OP (TOP_movz, abs_tn, numer_tn, p1, abs_tn, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

    /* Generate a multiply upper:
     */
    mult_tn = Build_TN_Of_Mtype (mtype);
    Expand_High_Multiply (mult_tn, abs_tn, d_tn,
    ...
    if (n > 0) {
      ...
      TOP top = is_double ? ((n > 32) ? TOP_dsra32 : TOP_dsra) : TOP_sra;
      Build_OP (top, shift_tn, mult_tn, Gen_Literal_TN ((n-1)%32, 4), ops);
    } else {
      shift_tn = mult_tn;
    }
     ...
     */
    Build_OP (is_double ? TOP_dsubu : TOP_subu,
	      result, Zero_TN, shift_tn, ops);
    Build_OP (denom_val < 0 ? TOP_movn : TOP_movz,
	      result, shift_tn, p1, result, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  } /* end Signed */
...
Expand_Power_Of_2_Rem (TN *result, TN *src1, INT64 src2_val, TYPE_ID mtype, OPS *ops)
{
  ...
  if (MTYPE_is_signed(mtype)) {
    ...
    Build_OP(TOP_movz, tmp1,  src1, p1, tmp1, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
    ...
    Build_OP(TOP_movz, result, tmp2, p1, result, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  } else {
...
Expand_Divide (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (CGEXP_cvrt_int_div_to_fdiv) {
   if (Is_Target_Mips5kf() || Is_Target_twc9a()) {
    if (MTYPE_is_size_double(mtype)) {
      ...
      UINT success = 1;
      if (const_src2 && TN_is_constant(src2)) {
	success = 0;
...
Expand_Mod (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  Build_OP(TOP_movn, tmp3, src2, p1, tmp3, ops);
  Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  Build_OP(TOP_movz, tmp3, Zero_TN, p1, tmp3, ops);
  Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  if (is_double)
...
Expand_DivRem(TN *result, TN *result2, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (const_src1 && const_src2) {
	...
	src1 = Expand_Immediate_Into_Register(Gen_Literal_TN(src1_val, tn_size),
					      is_double, ops);
	Build_OP(is_double ? TOP_ddiv : TOP_div, Hilo_TN(), src1, src2, ops);
	Build_OP(TOP_teq, src2, Zero_TN, Gen_Literal_TN(7, 4), ops);
	Build_OP(TOP_mflo, result, Hilo_TN(), ops);
