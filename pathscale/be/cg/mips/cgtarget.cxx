 ...
 * Module: cgtarget.cxx
 * $Revision: 1.18 $
 * $Date: 05/03/04 17:59:11-08:00 $
 * $Author: gautam@jacinth.keyresearch $
 * $Source: be/cg/mips/SCCS/s.cgtarget.cxx $
 *
 * Description:
 *
...
void CGTARG_Compute_Branch_Parameters(INT32 *mispredict, INT32 *fixed, INT32 *brtaken, double *factor)
{
  ...
  *factor = 0.0;
  ...
  if (Is_Target_R10K ()) {
    *mispredict= 7; *fixed= 1; *brtaken= 1; *factor = 1.00;
  }
  else if (Is_Target_Mips5kf ()) {
    *mispredict= 2; *fixed= 2; *brtaken= 0; *factor = 1.00;
  }
  else if (Is_Target_twc9a ()) {
    *mispredict= 7; *fixed= 2; *brtaken= 0; *factor = 0.01;
  }
  else
  {
  ...
  TN **tn2)
{
  ...
  *tn2 = OP_opnd(br, 1);
  ...
  BOOL is_64bit = (TN_size(*tn1) == 8);

  switch (OP_code(br))
  {
  case TOP_beq:
    variant = is_64bit ? V_BR_I8EQ : V_BR_I4EQ;
    break;

  case TOP_bne:
    variant = is_64bit ? V_BR_I8NE : V_BR_I4NE;
    break;

  case TOP_bgez:
    variant = is_64bit ? V_BR_I8GE : V_BR_I4GE;
    *tn2 = Gen_Literal_TN(0, is_64bit ? 8 : 4);
    break;
    
  case TOP_bltz:
    variant = is_64bit ? V_BR_I8LT : V_BR_I4LT;
    *tn2 = Gen_Literal_TN(0, is_64bit ? 8 : 4);
    break;

  case TOP_bgtz:
    variant = is_64bit ? V_BR_I8GT : V_BR_I4GT;
    *tn2 = Gen_Literal_TN(0, is_64bit ? 8 : 4);
    break;
    
  case TOP_blez:
    variant = is_64bit ? V_BR_I8LE : V_BR_I4LE;
    *tn2 = Gen_Literal_TN(0, is_64bit ? 8 : 4);
    break;
...
void CGTARG_Initialize(void)
{
  ...
  CGTARG_Invert_Table[TOP_add_ps]   = TOP_sub_ps;
  CGTARG_Invert_Table[TOP_madd_s]   = TOP_nmadd_s;
  CGTARG_Invert_Table[TOP_madd_d]   = TOP_nmadd_d;
  CGTARG_Invert_Table[TOP_madd_ps]   = TOP_nmadd_ps;
  ...
  CGTARG_Invert_Table[TOP_sub_ps]   = TOP_add_ps;
  CGTARG_Invert_Table[TOP_nmadd_s]   = TOP_madd_s;
  CGTARG_Invert_Table[TOP_nmadd_d]   = TOP_madd_d;
  CGTARG_Invert_Table[TOP_nmadd_ps]   = TOP_madd_ps;
  ...
  CGTARG_Invert_Table[TOP_movf_ps] = TOP_movt_ps;
  CGTARG_Invert_Table[TOP_movt_ps] = TOP_movf_ps;
  CGTARG_Invert_Table[TOP_movz_ps] = TOP_movn_ps;
  CGTARG_Invert_Table[TOP_movn_ps] = TOP_movz_ps;
...
INT CGTARG_Copy_Operand(OP *op)
{
  ...
  case TOP_sext:
    // Bug 13497: Special case: sll always sign extends lower 32 bits
    // and may be explicitly used for sign extension.
...
/* ====================================================================
 *
 * CGTARG_Can_Fit_Displacement_In_Branch_Instruction
 *
 * See interface description
 *
 * ====================================================================
...
{
  // DISP is in bytes.
  return ISA_LC_Value_In_Class (disp / 4, LC_simm16);
}
#endif
                           ...
                           TYPE_ID /* unused */)
{
  ...
  static const char* immediates = "inIKLMNPO";
  ...
  else if (strchr("gmpR", *constraint)) // SiCortex 8768
  {
	...
	TYPE_ID desc, TN **tn, const char **name)
{
	...
	*name = Get_TN_Assembly_Name(*tn);
	if (*constraint == 'm' || *constraint == 'R') {
	    	sprintf(asm_constraint_name[asm_constraint_index], "0(%s)", 
			*name);
	} else {
...
void
CGTARG_Process_Asm_lh_constraints (TN ** tns, OPS * ops)
{
  if (tns[0]) // LO
    Build_OP(TOP_mflo, tns[0], Hilo_TN(), ops);

  if (tns[1]) // HI
    Build_OP(TOP_mfhi, tns[1], Hilo_TN(), ops);
}
...
// Return TRUE if OP does double multiply.
BOOL
OP_is_like_muld (OP *op)
{
  if ((OP_fmul(op) ||
       OP_madd(op)) &&
      OP_result_size(op, 0) == 64)
    return TRUE;
  return FALSE;
}

// Return TRUE if OP disrupts the M2 pipeline stage of an earlier double
// multiply.
BOOL
OP_disrupts_muld (OP *op)
{
...
// Return FALSE if OP cannot be placed in the branch delay slot for hardware
// reasons.
BOOL
CGTARG_Is_Delay_Slot_Op (OP *op)
{
  // Avoid fp stall and fp rounding hardware bugs.
  switch (CG_muld) {
    case 0:		// No workaround, do nothing.
      break;
    case 1:
    case 2:
    case 3:
      // Don't place mul.d in delay slot.  Bug 13027.
      if (OP_is_like_muld(op))
	return FALSE;
      break;
  }

  // Workaround fp bugs.  Bug 13481.
  if (CG_hw_round > 0 ||
      CG_hw_stall > 0) {
    if (OP_fp_round_hazard(op) ||		// fp rounding hazard
	OP_fp_unpredictable_latency(op) ||	// WAW fp stall hazard
	OP_code(op) == TOP_mfc1 ||		// WAR fp stall hazard
	OP_code(op) == TOP_dmfc1)		// WAR fp stall hazard
      return FALSE;
  }
