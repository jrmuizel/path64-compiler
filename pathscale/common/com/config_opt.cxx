...
INT32 OPT_Madd_Height = 5;
INT32 OPT_Madd_Chains = 4;
# endif
...
#ifdef TARG_MIPS
INT32  OPT_Threshold_To_Memlib = 0;
BOOL   OPT_Enable_Memlib_Aggressive = TRUE; /* transform unknown trip counts */
#else
INT32  OPT_Threshold_To_Memlib = 256;    /* threshold to transform to mem calls */
BOOL   OPT_Enable_Memlib_Aggressive = FALSE; /* transform unknown trip counts */
#endif
...
BOOL OPT_Freestanding = FALSE; // -ffreestanding?
BOOL OPT_Reassoc_For_Cse = FALSE; // Reassociation to maximize CSE opportunities
#endif	// KEY
...
static OPTION_DESC Options_OPT[] = {
  ...
  { OVK_BOOL,	OV_INTERNAL,	TRUE, "freestanding", "",
    0, 0, 0,	&OPT_Freestanding, NULL,
    "Compilation takes place in a freestanding environment" },

  { OVK_BOOL, OV_VISIBLE,     FALSE, "reassoc_for_cse",      "reassoc",
  ...
  { OVK_BOOL,   OV_INTERNAL,    FALSE, "memlib_aggr",   "memlib_aggr",
    0, 0, 0,    &OPT_Enable_Memlib_Aggressive, NULL,
    "Transform memory copy/set loops with unknown trip count into library calls" },
  ...
  { OVK_INT32,  OV_VISIBLE,    TRUE, "madd_height",    "",
    5, 1, INT32_MAX,    &OPT_Madd_Height, NULL,
    "Maximum length of MADD chain allowed before transforming it to sum of shorter MADD chains"},
  { OVK_INT32,  OV_VISIBLE,    TRUE, "madd_chains",    "",
    4, 1, INT32_MAX,    &OPT_Madd_Chains, NULL,
    "Maximum number of chains produced when breaking up overly long MADD chain"},
#endif
...
#ifdef KEY
#ifdef TARG_MIPS
  {OVK_INT32,   OV_VISIBLE,	TRUE,     "memmove_align_count",  "memmove_align",
    256, 0, INT32_MAX,   &CG_memmove_align_inst_count,&CG_memmove_align_inst_count_overridden,
    "Maximum size of inline expansion of memory intrinsics" },
#else
  {OVK_INT32,   OV_VISIBLE,	TRUE,     "memmove_align_count",  "memmove_align",
    16, 0, INT32_MAX,   &CG_memmove_align_inst_count,&CG_memmove_align_inst_count_overridden,
    "Maximum size of inline expansion of well-aligned memory intrinsics" },
#endif
#endif
