...
#include <ctype.h>
#include <string.h>
#include "W_ar.h"         /* for support of -INLINE:library= */
#include "W_errno.h"  /* for EBADF */
#include "defs.h"
#include "config.h"
...
{
#ifdef TARG_ST
  // [CL] handle renaming of static functions (so that they
  // have unique names
  char* is_unique = strstr(name, "..");
  if (is_unique) {
    char *new_name = (char *) alloca (is_unique-name+1);
    strncpy(new_name, name, is_unique-name);
    new_name[is_unique-name] = '\0';
    return User_Specified_Info(new_name, user_inline_request);
  }
#endif
...
#ifdef TARG_ST
  // [CL] handle the case where the user does not
  // provide a value for options never,must,....
  if (args) {
#endif
  do {
    char *endc;
