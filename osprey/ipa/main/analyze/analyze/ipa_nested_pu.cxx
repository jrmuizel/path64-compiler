...
typedef __GNUEXT::hash_map<PU_IDX, PU_IDX> IPA_NESTED_PU_PARENT_MAP;

static IPA_NESTED_PU_PARENT_MAP* nested_pu_parent_map;

#ifdef TARG_ST
// [SC]: Must recurse on child to find its children too.
static void
Build_Parent_Child_Relations_1 (PU_Info *parent)
{
    while (parent) {
	    ...
	    Build_Parent_Child_Relations_1 (child);
...
#ifdef TARG_ST
    Build_Parent_Child_Relations_1 (parent);
#else
    while (parent) {
	PU_Info* child = PU_Info_child(parent);
	while (child) {
...
#ifdef TARG_ST
    // [SC]: Correct text of assertion.
    Is_True(parent_pu_idx > PU_IDX_ZERO && parent_pu_idx < PU_Table_Size(),
          ("Get_Parent_Of_Nested_PU: bad pu index %d, not in range %d < idx < %d",
           PU_IDX_ZERO, parent_pu_idx, PU_Table_Size()));
#else
    Is_True(parent_pu_idx > PU_IDX_ZERO && parent_pu_idx < PU_Table_Size(),
