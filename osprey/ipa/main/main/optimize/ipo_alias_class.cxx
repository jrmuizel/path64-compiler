				  ...
				  const BOOL                trace)
{
  ...
  if (trace) {
    fprintf(TFile, "Pending(%u) += Pending(%u)\n", Id(), that.Id());
  }
				       ...
				       const BOOL                   trace)
{
  ...
  // classes.
      // [HK] fix to get the right allocator
//         vector<IP_ALIAS_CLASS_MEMBER * >::iterator arg;
//         vector<IP_ALIAS_CLASS_MEMBER * >::iterator that_arg;
  vector<IP_ALIAS_CLASS_MEMBER *, mempool_allocator<IP_ALIAS_CLASS_MEMBER *> >::iterator arg;
  vector<IP_ALIAS_CLASS_MEMBER *, mempool_allocator<IP_ALIAS_CLASS_MEMBER *> >::iterator that_arg;
...
//         vector<IP_ALIAS_CLASS_MEMBER * >::iterator start, stop;
  vector<IP_ALIAS_CLASS_MEMBER *, mempool_allocator<IP_ALIAS_CLASS_MEMBER *> >::iterator start, stop;
...
#ifdef TARG_ST
      // [CL] Accept call to alias
      if ((ST_base_idx(St_Table[callee_st_idx]) != callee_st_idx) &&
	  ST_emit_symbol(WN_st(call_wn)))
      {
...
#ifdef TARG_ST
// [CG 2004/11/16] This code must match be/opt/opt_alias_class.cxx
...
BOOL
IP_ALIAS_CLASSIFICATION::Stmt_stores_asm_output(const WN *const stmt)
{
  ...
  if (opr == OPR_LDID || opr == OPR_LDA) {
    if ((ST_sclass(WN_st(load)) == SCLASS_REG) &&
	WN_offset(load) < 0) {
      return TRUE;
    }
  }
...
BOOL
IP_ALIAS_CLASSIFICATION::Stmt_Uses_asm_output(const WN *const stmt)
{
  return FALSE;
}

WN *
IP_ALIAS_CLASSIFICATION::Handle_asm(WN *asm_wn)
{
  ...
  Is_True(!Stmt_Uses_asm_output(stmt),
	  ("IP_ALIAS_CLASSIFICATION: General use of asm output is illegal"));
...
IP_ALIAS_CLASSIFICATION::Classify_memops(WN *entry_wn)
{
  ...
  if (Tracing()) {
    fprintf(TFile, "Current_Map_Tab == 0x%p\n", Current_Map_Tab);
