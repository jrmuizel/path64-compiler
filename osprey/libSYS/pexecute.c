...
#if defined(IN_GCC) || HAVE_CONFIG_H
#include "config.h"
#endif
...
#if defined(IN_GCC) || !HAVE_LIBIBERTY_H
#if 0
#include "gansidecl.h"
#endif
...
/* LAO Team add-on : Microsoft C spawn functions also split arguments with space!!. */
     ...
     char **argvec;
{
  ...
  for (i = 0; argvec[i] != 0; i++)
    {
      int newlen, len, j, newj;
      char *temp, *newtemp;

      temp = argvec[i];
      len = strlen (temp);

      new_argv[i] = xmalloc(len*2+3); /* max final size. */
      newtemp = new_argv[i];

      for (newlen = 0, newj= 0, j = 0; j < len; j++)
        {
          if (temp[j] == '"')
            {
              newtemp[newj] =  '\\';
              newtemp[newj+1] =  temp[j];
              newlen++;
              newj += 2;
            }
	  else
      ...
      /* LAO Team add-on */
      for (j = 0; j < newlen; j++) {
	if (isspace(newtemp[j])) {
	  /* Need to quote it. */
	  memmove(&newtemp[1], &newtemp[0], newlen);
	  newtemp[0] = '\"';
	  newtemp[newlen+1] = '\"';
	  newtemp[newlen+2] = '\0';
	  newlen += 2;
