...
#ifdef TARG_ST
      /* This is specific to the SGI usage of the GCC Fe. We must propagate
	 restrict information to the Whirl Symbol table. In this usage of the
	 fe, the flag_strict_aliasing is not certainly set, since GCC optimizations 
	 ARE DISABLED , so me must do it in all valid usage cases. The very place
	 of TREE_RESTRICT definition is tree.h since it is shared with tree_symtab.cxx.
	  Additionally, we are not certain that the alias set computed by GCC
	  are of any use to SGI compiler*/
	  TREE_RESTRICT(decl) = 1 ;
#endif /* TARG_ST */
    }
}
