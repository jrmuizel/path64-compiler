...
/* WFE pragmas are defined in wfe_pragmas.def, refer to this file
   for documentation.
*/
#include "wfe_pragmas.h"

/* Pragma parsing state. */
static int pragma_last_lex;
static tree pragma_last_tree;
static int
get_pragma_lex(void)
{
  if (pragma_last_lex == 0) {
    pragma_last_lex = c_lex(&pragma_last_tree);
  }
  return pragma_last_lex;
}

static int
match_pragma_lex(int lex)
{
  return get_pragma_lex() == lex;
}

static int
parse_pragma_lex(int lex)
{
...
static int
match_pragma_arg(const char *name, char arg)
{
  switch (arg) {
  case 'I':
...
parse_pragma_arg(tree *list, const char *name, char arg)
{
  if (match_pragma_arg(name, arg)) {
    if (*list == NULL) *list = build_tree_list(NULL_TREE, pragma_last_tree);
    else *list = chainon(*list,build_tree_list(NULL_TREE, pragma_last_tree));
    pragma_last_lex = 0;
    pragma_last_tree = 0;
    return 1;
  }
...
parse_pragma_args(tree *args, const char *name, const char *args_spec)
{
  ...
  while (*p) {
	...
	if (match_pragma_arg(name, *p)) {
	  if (!parse_pragma_arg(&list, name, *p)) return 0;
	}
	p++;
      } else if (*(p+1) == '*') {
	if (match_pragma_arg(name, *p)) {
	  ...
	  while (match_pragma_lex(CPP_COMMA)) {
	    if (!parse_pragma_lex(CPP_COMMA)) return 0;
	    if (!parse_pragma_arg(&list, name, *p)) return 0;
	  }
	}
	p++;
...
{ \
  ...
  if (in_code && current_function_decl == NULL) { \
    warning ("pragma not expected out of function scope at '#pragma %s' - ignored", name); \
    return; \
  } \
  if (!in_code && current_function_decl != NULL) { \
    warning ("pragma not expected in function scope at '#pragma %s' - ignored", name); \
    return; \
  } \
  if (parse_pragma_args (&pragma_args, name, args_spec)) { \
    tree pragma_stmt; \
    pragma_stmt = build_nt(PRAGMA_STMT, \
			   build_int_2(id, 0), pragma_args); \
    if (in_code) add_stmt(pragma_stmt); \
    else WFE_Expand_Pragma(pragma_stmt); \
  } else { \
    warning("malformed pragma - ignored. Usage is: %s", usage); \
  } \
}
...
/*
 * Called at expansion time for code scope pragmas.
 */
void
expand_pragma_stmt (stmt)
     tree stmt;
{
...
#ifdef HANDLE_WFE_PRAGMAS
#define DECL_WFE_PRAGMA(id,name,wn_scope,wn_id,nargs,args_spec,usage) \
  if (*name != '\0') \
    cpp_register_pragma (parse_in, 0, name, handle_##id);
