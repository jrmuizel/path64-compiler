...
BE_EXPORTED extern WN_MAP RID_map;
BE_EXPORTED extern MEM_POOL REGION_mem_pool;

/* latest region id, defined in common/com/wn.c */
BE_EXPORTED extern INT32 New_Region_Id(void);
BE_EXPORTED extern INT32 Last_Region_Id(void);

/* Replace the old WN tree with a new one and update the RID pointers */
BE_EXPORTED extern void REGION_new_wn(WN *, WN *);

/* Make sure RID and WN map are consistent */
BE_EXPORTED extern BOOL REGION_consistency_check(WN *);

/* Update alias information for glue code */
BE_EXPORTED extern void REGION_update_alias_info(WN *, struct ALIAS_MANAGER *);

/* Get the RID from the WN map */
BE_EXPORTED extern RID *REGION_get_rid(const WN *);

/* Given a WN *, find the RID tree root WN which is the PU */
BE_EXPORTED extern WN *REGION_find_pu(WN *);

/* Given a RID that will be processed to black, mark the root RID */
BE_EXPORTED extern BOOL REGION_has_black_regions(RID *);

/* Set the region_level for this RID and all its children */
BE_EXPORTED extern void REGION_set_level(RID *, REGION_LEVEL);

/* count the exits in the exit block of a region */
BE_EXPORTED extern BOOL REGION_count_exits(WN *);

/* recount exits, fix up any boundary sets that have changed */
BE_EXPORTED extern void REGION_fix_up_exits(RID *, WN *);

/* look at all exit block between two rids and find a given label number */
BE_EXPORTED extern BOOL REGION_scan_exits(WN *, INT32);
...
BE_EXPORTED extern RID *RID_Create(INT, INT, WN *);

/* Given two RIDs make the first the kid of the second */
BE_EXPORTED extern void RID_Add_kid(RID *, RID *);

/* Remove a RID from the RID tree */
BE_EXPORTED extern void RID_unlink(RID *);

/* Replace old_rid with new_rid */
BE_EXPORTED extern void RID_replace(RID *old_rid, RID *new_rid);

/* delete a RID from the tree */
BE_EXPORTED extern void RID_Delete(WN_MAP_TAB *, WN *);
BE_EXPORTED extern void RID_Delete2(RID *);

/* clone a RID and connect into tree */
BE_EXPORTED extern void REGION_clone(WN *, WN *, WN *);

/* reattach a RID and WN, update the level and last preg */
BE_EXPORTED extern void REGION_emit(RID *, WN *, INT32, INT32, INT64);

/* copy in and out sets from one rid to another */
BE_EXPORTED extern void RID_copy_sets(RID *, RID *);

/* find the RID level for Preopt (it's called so many places) */
BE_EXPORTED extern REGION_LEVEL RID_preopt_level(INT);

/* search a boundary set for a given PREG */
BE_EXPORTED extern BOOL REGION_search_preg_set(PREG_LIST *, PREG_NUM);

/* create a POINTS_TO for the wn given and add it to the list given */
BE_EXPORTED extern void REGION_add_wn_points_to(POINTS_TO_SET **, WN *,
				 struct ALIAS_MANAGER *);

/* add a POINTS_TO to the list given, similar to REGION_add_wn_points_to */
BE_EXPORTED extern void REGION_add_points_to(POINTS_TO_SET **, POINTS_TO *,
				 struct ALIAS_MANAGER *);

/* allocate from a specific pool and copy a points_to into it */
BE_EXPORTED extern POINTS_TO *Points_to_copy(POINTS_TO *, MEM_POOL *);

OPT_EXPORTED extern void Get_symbol_info_for_cvt_io(POINTS_TO *, WN *);
			/* in opt_sym.cxx */
BE_EXPORTED extern POINTS_TO *Points_to(struct ALIAS_MANAGER *, WN *);
			/* in opt_alias_mgr.cxx */

/* add a PREG to either the pregs_in or pregs_out lists (pregs_quad also) */
BE_EXPORTED extern BOOL REGION_add_preg_in(RID *rid, PREG_NUM pr, TYPE_ID quad);
BE_EXPORTED extern BOOL REGION_add_preg_out(RID *rid, INT32 which_set, PREG_NUM pr,
				TYPE_ID quad);

/* remove a PREG from either the pregs_in or ALL the pregs_out sets */
/* does quad also */
BE_EXPORTED extern BOOL REGION_remove_preg(RID *rid, PREG_NUM pr, BOOL outset);

/* add a label after the region and the OPC_REGION_EXIT at the end */
BE_EXPORTED extern WN *REGION_add_exit(WN *, WN *, WN *);

/* delete a region exit from the exit block and decrement RID_num_exits */
BE_EXPORTED extern void REGION_delete_exit(RID *, INT32, WN *, BOOL);

/* Find the options pragma for the PU or region */
BE_EXPORTED extern char *REGION_get_options_string(WN *);

/* tell if a region is an EH region based on WHIRL kind */
BE_EXPORTED extern BOOL REGION_is_EH(WN *);

/* tell if a region is an MP region based on WHIRL pragmas */
BE_EXPORTED extern BOOL REGION_is_mp(WN *);

/* tell if a region is an EH region based on WHIRL kind */
BE_EXPORTED extern BOOL REGION_is_EH(WN *);

/* tell if a call is a fake call (inside the pragma part of an EH region) */
BE_EXPORTED extern BOOL WN_Fake_Call_EH_Region(WN *,WN_MAP);


/* convert from RID_TYPE_* to REGION_KIND_* (common/com/wn_core.h) */
BE_EXPORTED extern REGION_KIND REGION_type_to_kind(RID *);

/* convert from REGION_KIND_* to RID_TYPE_* */
BE_EXPORTED extern void REGION_kind_to_type(WN *, RID *);

/* set REGION_RETURN bit on current RID and propagate up to parent */
BE_EXPORTED extern void REGION_propagate_return(RID *);

/* return the text for the level this RID is compiled to */
BE_EXPORTED extern char *RID_level_str(RID *rid);

/* return text for RID type */
BE_EXPORTED extern char *RID_type_str(RID_TYPE type);

/* Print: pregs_in, pregs_out, pregs_quad,	*/
/* used_in, def_in_live_out */
BE_EXPORTED extern void RID_set_print(FILE *, RID *);
/* this is defined in opt_alias_analysis.cxx and exported from wopt.so */
OPT_EXPORTED extern void Print_points_to(FILE *, POINTS_TO *);

/* Print the given region_id to file */
BE_EXPORTED extern void RID_Fprint(FILE *, RID *);

/* Print the tree of RIDs rooted at rid to TFile */
BE_EXPORTED extern void RID_Tree_Print(FILE *, RID *);

/* Print the tree of RIDs rooted at the RID of the given WN to TFile */
BE_EXPORTED extern void RID_WN_Tree_Print(FILE *, WN *);
