...
BE_EXPORTED extern INT REGION_Initialize(WN *, BOOL);

/* Pop the REGION_mem_pool */
BE_EXPORTED extern void REGION_Finalize(void);
...
BE_EXPORTED extern void REGION_CS_ITER_init( REGION_CS_ITER *, WN * );
BE_EXPORTED extern BOOL REGION_CS_NoEarlierSub_While( REGION_CS_ITER * );
BE_EXPORTED extern void REGION_CS_NoEarlierSub_Next ( REGION_CS_ITER * );
BE_EXPORTED extern void REGION_CS_NoEarlierSub_First( REGION_CS_ITER *, WN *, RID_TYPE);
BE_EXPORTED extern void REGION_CS_print( REGION_CS_ITER * );

/* Remove a region from the PU and mark location for insertion later */
BE_EXPORTED extern WN  *REGION_remove_and_mark(WN *, REGION_CS_ITER *);
/* Insert new region at mark left by REGION_remove_and_mark */
BE_EXPORTED extern void REGION_replace_from_mark(WN *, REGION_CS_ITER *);

/* These two phases are called by preopt (wodriver.c) for :p (lno off)	*/
/* and by lno (lnodriver.c) for :l (lno on)				*/
BE_EXPORTED extern WN *Rail(WN *);
BE_EXPORTED extern void Region_Bound_Info(WN *, struct DU_MANAGER *, 
...
BE_EXPORTED extern void Generate_region_boundaries(WN *, struct ALIAS_MANAGER *);
