...
#ifdef TARG_ST
PF_UGS *PF_LOOPNODE::Get_UGS (WN *ref_addr) {
  PF_UGS *pf_ugs;
  ST_IDX ref_st;
  WN *base = ref_addr;
  int i;
  ...
  if (opr == OPR_ARRAY) {
    ...
    opr = WN_operator(base);
    
    if (opr == OPR_ADD || opr == OPR_SUB) {
      if (WN_operator(WN_kid1(base)) == OPR_INTCONST)
	base = WN_kid0(base);
      else if (WN_operator(WN_kid0(base)) == OPR_INTCONST)
	base = WN_kid1(base);
      opr = WN_operator(base);
    }
  }
  ...
  ref_st = WN_st_idx(base);

  PF_BASE_ARRAY *pf_base;
  for (i=0; i <NumBases(); i++) {
    pf_base = Get_BaseArray(i);
    if (pf_base->Get_Symbol()->St()->st_idx == ref_st)
      return pf_base->Get_UGS(ref_addr);
  }
