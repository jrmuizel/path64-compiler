...
//-----------------------------------------------------------------------
// NAME: Matching_Load_Store
// FUNCTION: Returns TRUE if 'wn_load' and 'wn_store' are reading and
// writing to the same location.
//-----------------------------------------------------------------------

static BOOL Matching_Load_Store(WN* wn_load, 
			    WN* wn_store)
{
  
  OPERATOR opr_load = WN_operator(wn_load);
  OPERATOR opr_store = WN_operator(wn_store);
  if (((opr_load == OPR_LDID) && (opr_store != OPR_STID)) ||
      ((opr_load == OPR_ILOAD) && (opr_store != OPR_ISTORE)))
    return FALSE;
  if (WN_desc(wn_load) != WN_desc(wn_store))
    return FALSE;
  if (opr_load == OPR_LDID)
    return SYMBOL(wn_load) == SYMBOL(wn_store); 
  if (opr_load == OPR_ILOAD) {
    WN* wn_array_load = WN_kid0(wn_load); 
    if (WN_operator(wn_array_load) != OPR_ARRAY)
      return FALSE; 
    WN* wn_array_store = WN_kid1(wn_store); 
    if (WN_operator(wn_array_store) != OPR_ARRAY)
      return FALSE; 
    WN* wn_base_load = WN_array_base(wn_load); 
    WN* wn_base_store = WN_array_base(wn_store); 
    ST* st_base_load(Get_ST_Base(wn_base_load)); 
    ST* st_base_store(Get_ST_Base(wn_base_store)); 
    BOOL same_base = (st_base_load == NULL || st_base_store == NULL)
      ? st_base_load == st_base_store
      : ST_base(st_base_load) == ST_base(st_base_store)
      && ST_ofst(st_base_load) == ST_ofst(st_base_store);
    if (!same_base) 
      return FALSE; 
    ACCESS_ARRAY* aa_load = (ACCESS_ARRAY*) WN_MAP_Get(LNO_Info_Map, 
      wn_array_load);
    ACCESS_ARRAY* aa_store = (ACCESS_ARRAY*) WN_MAP_Get(LNO_Info_Map, 
      wn_array_store);
    if (!(*aa_load == *aa_store))
      return FALSE; 
    return TRUE; 
			     ...
			     BOOL* if_mm_max) 
{
  ...
  /* if (a <> m) then m = a; */
    ...
    BOOL return_value = FALSE;
    if (Matching_Exprs(wn_expr_right, wn_expr_then)) {
      if (Matching_Load_Store(wn_expr_left, wn_st_then)) {
	mm_max = opr == OPR_LT || opr == OPR_LE;
	return_value = TRUE;
      }
    }
    else if (Matching_Exprs(wn_expr_left, wn_expr_then)) {
      if (Matching_Load_Store(wn_expr_right, wn_st_then)) {
	mm_max = opr == OPR_GT || opr == OPR_GE;
	return_value = TRUE;
      }
    }
...
#ifdef TARG_ST
  // FdF 20050211: Max_Wtype says that Max_Wtype(I4,U4) should be I64,
  // but, to avoid expansion, I4 is arbitrarily chosen. In fact, the
  // sign of the comparsion should be used (see DDTS MBTst20582).
  if (MTYPE_is_unsigned(WN_desc(WN_if_test(wn_if))) && type_cmp == MTYPE_I4)
    type_cmp = MTYPE_U4;
  Is_True(MTYPE_is_signed(WN_desc(WN_if_test(wn_if))) == MTYPE_is_signed(type_cmp), ("Incorrect type for MIN/MAX"));
#endif
