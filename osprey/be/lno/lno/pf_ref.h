...
public:
  ...
  ~PF_UGS ();
#ifdef TARG_ST
  PF_DESC *GetManualPFVec (PF_LEVEL level, PF_LOCLOOP locloop) { ComputePFVec(level, locloop); return &_pfdesc;}
#endif
  BOOL                  Add_Ref (WN* ref);
  void                  Build_Base_LGs ();
  PF_VOLUME             Volume (mINT16 depth);
...
#ifdef TARG_ST
  INT			NumREFs () { return _refs.Elements(); }
#endif
...
public:
  ...
  PF_VOLUME Volume (mINT16 depth);
#ifdef TARG_ST
  INT     NumUGSs () { return _ugs.Elements(); }
  PF_UGS *Get_UGS (INT i) { return _ugs.Bottom_nth(i); }
  PF_UGS *Get_UGS (WN *ref);
#endif
  void      Build_Base_LGs ();
  void      Find_Loc_Space (PF_LOCLOOP locloop);
  PF_SPLIT_VECTOR*   Find_Split_Vector ();
