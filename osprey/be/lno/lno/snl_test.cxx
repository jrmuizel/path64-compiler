                                              ...
                                              BOOL* changed)
{
  ...
  *changed = FALSE;
  ...
  if (snl_debug >= 3) {
    fprintf(TFile, "%s\t\t\t Transformation begins \n%s",
	    DBar, DBar);
  }
  ...
  if (snl_debug >= 3) {
    fprintf(TFile,"\t Interchange %s,\n", (reordering) ? "ON" : "OFF");
    fprintf(TFile,"\t Outer Unroll %s: ", (regstripping) ? "ON" : "OFF");
    for (i = outermost; i <= ni->Depth_Inner(); i++) {
      fprintf(TFile,"\n\t\t unroll[%d] = %d ", i-outermost, regstripsz[i-outermost]);
    }
    fprintf(TFile,";\n");
  }
		          ...
		          INT nloops)
{
  ...
  if (snl_debug >= 3) {
    fprintf(TFile, "%s\t\t Memory Accesses \n%s\n", DBar, DBar);
