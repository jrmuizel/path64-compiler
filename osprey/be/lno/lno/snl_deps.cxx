  ...
  _ci(&ni->Dostack())
{
  ...
  if (snl_debug >= 3) {
    fprintf(TFile,"-------------------------------------------------------\n");
    fprintf(TFile, "\t  Initializing the SNL_ANAL_INFO ... \n");
    fprintf(TFile,"-------------------------------------------------------\n");
  }
  ...
  for (LWN_ITER* wi = LWN_WALK_TreeIter(loop);
    ...
    if (OPCODE_is_load(op) || OPCODE_is_store(op) || OPCODE_is_call(op)) {
      ...
      // Arthur: I do not see how this is usefull ??
      //      if (OPCODE_is_load(op) || OPCODE_is_store(op) || OPCODE_is_call(op)) {
...
void SNL_ANAL_INFO::Enter_Deps(WN* wn, LEX_DEPTH ld)
{
  ...
  for (EINDEX16 e = Array_Dependence_Graph->Get_Out_Edge(v); e; e = enext) {
    ...
    if (snl_debug >= 3) {
      fprintf(TFile," snl_deps: enter_deps for v%d -> v%d: \n", v, v2);
    }
    ...
    if (snl_debug >= 3) {
      fprintf(TFile, "          insertion of 0x%p (lex depth %d):\n", 
		     wn, ld.Depth);
