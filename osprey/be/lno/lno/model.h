...
// FAB: keyword=LNOBRIDE
// For LAO, Benoit would like a more accurate dependance graph for the
// scheduler. For that we need to plug lno on. Either with -O3 or
// simply with -PHASE:lno=1 which set Run_lno to TRUE.
// It then runs Lnoptimizer which calls Phase_123.
// If within Phase_123, we didn't manage to count correctly the
// Resources, it returns TRUE (early_exit=TRUE) which have the effect
// of skipping the build of Current_Dep_Graph...
//
// To enable the build of Current_Dep_Graph by lno I added a call
// to Build_CG_Dependence_Graph and skipped Phase_123 which does
// not work yet.
...
/* ====================================================================
 *   ARRAY_REF_LIST -- list of array references in the SNL
 * ====================================================================
 */
...
public:
  ...
  ARRAY_REF_LIST(ARRAY_REF_LIST *orig, MEM_POOL *pool); 
  // It seems like it is nice to distinguish all these
  // different types of memory accesses -- Arthur
#ifdef TARG_ST
...
/* ====================================================================
 *   ARRAY_REF -- includes a stack of array references and a list
 *                of bad references
 * ====================================================================
 */
...
{
public:
  // Arthur: It seems usefull to distinguish all these different
