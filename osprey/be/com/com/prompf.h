...
class BE_EXPORTED_CLASS PROMPF_LINES { 
private: 
  STACK<INT> _low; 
  STACK<INT> _high; 
  MEM_POOL* _pool; 
public: 
  ...
  BE_EXPORTED_METHOD PROMPF_LINES(WN* wn_tree, MEM_POOL* pool); 
  INT Sections(); 
  INT Low(INT i);
  INT High(INT i);  
  void Print(FILE *fp); 
  BE_EXPORTED_METHOD INT Print_Compact(FILE *fp, BOOL print_brackets=TRUE); 
  BE_EXPORTED_METHOD void Add_Line(INT line); 
  BE_EXPORTED_METHOD void Add_Lines(WN* wn_tree);
}; 
...
public: 
  ...
  void Mark_F90_Lower(); 
  void Mark_Omp(); 
  BE_EXPORTED void Mark_Preopt(); 
  BE_EXPORTED void Mark_Prelno(); 
  BE_EXPORTED void Mark_Postlno(); 
  BE_EXPORTED void Elimination(INT old_loop); 
  void Undo_Elimination();
  BE_EXPORTED void Fusion(INT old_loops[], INT new_loop);
  void Undo_Fusion();
  BE_EXPORTED void Fission(INT old_loops[], PROMPF_LINES* old_lines[], INT new_loops[], 
    PROMPF_LINES* new_lines[],  INT nloops); 
  BE_EXPORTED void Distribution(INT old_loops[], PROMPF_LINES* old_lines[], 
    INT new_loops[], PROMPF_LINES* new_lines[],  INT nloops); 
  BE_EXPORTED void Interchange(INT old_loops[], INT new_loops[], INT nloops); 
  BE_EXPORTED void Pre_Peel(INT old_loops[], INT new_loops[], INT nloops); 
  void Undo_Pre_Peel();
  BE_EXPORTED void Post_Peel(INT old_loops[], INT new_loops[], INT nloops); 
  void Undo_Post_Peel();
  BE_EXPORTED void Mp_Tile(INT old_loop, INT new_loops[], INT nloops); 
  BE_EXPORTED void Dsm_Tile(INT old_loop, INT new_loops[], INT nloops); 
  BE_EXPORTED void Donest_Outer_Tile(INT old_loops[], INT new_loop, INT nloops);
  BE_EXPORTED void Donest_Middle_Tile(INT old_loop, INT new_loop); 
  BE_EXPORTED void Dsm_Local(INT new_loop, PROMPF_LINES* pl, char* index_name); 
  BE_EXPORTED void Dsm_Io(INT new_loop, INT linenum, char* index_name); 
  BE_EXPORTED void Single_Process(INT new_id, PROMPF_LINES* pl); 
  BE_EXPORTED void Mp_Version(INT old_loops[], INT new_loops[], PROMPF_ID_TYPE id_type[], 
    INT nloops); 
  BE_EXPORTED void Parallel_Region(INT old_loop, INT new_loop); 
  BE_EXPORTED void Hoist_Messy_Bounds(INT old_loop[], INT new_loop[], INT nloops); 
  BE_EXPORTED void Doacross_Sync(INT old_loop, INT new_loop); 
  BE_EXPORTED void Doacross_Outer_Tile(INT old_loop, INT new_loop); 
  BE_EXPORTED void Doacross_Inner_Tile(INT old_loop, INT new_loop); 
  BE_EXPORTED void Remove_Unity_Trip(INT old_loop); 
  BE_EXPORTED void Cache_Winddown(INT old_loops[], INT new_loops[], INT nloops); 
  BE_EXPORTED void Interleaved_Winddown(INT old_loops[], INT new_loops[], INT nloops); 
  BE_EXPORTED void General_Version(INT old_loops[], INT new_loops[], INT nloops); 
  BE_EXPORTED void Cache_Tile(INT old_loop, INT new_loop); 
  BE_EXPORTED void Register_Winddown(INT old_loops[], INT new_loops[], INT nloops); 
  BE_EXPORTED void Register_SStrip(INT old_loops[], INT new_loops[], INT nloops); 
  BE_EXPORTED void Register_Tile(INT loop); 
  BE_EXPORTED void Register_Startup(INT old_loops[], INT new_loops[], INT nloops); 
  BE_EXPORTED void Register_Shutdown(INT old_loops[], INT new_loops[], INT nloops); 
  BE_EXPORTED void Se_Tile(INT old_loop, INT new_loop); 
  BE_EXPORTED void Se_Cache_Tile(INT old_loop, INT new_loop); 
  BE_EXPORTED void Inner_Fission(INT old_loop, PROMPF_LINES* old_lines, INT new_loops[],
    PROMPF_LINES* new_lines[], INT nloops);
  BE_EXPORTED void Gather_Scatter(INT old_loop, PROMPF_LINES* old_lines, INT new_loops[],
    PROMPF_LINES* new_lines[], INT nloops);
  BE_EXPORTED void Vintr_Fission(INT old_loop, PROMPF_LINES* old_lines, INT new_loops[],
    PROMPF_LINES* new_lines[], INT nloops);
  BE_EXPORTED void Prefetch_Version(INT old_loops[], INT new_loops[], INT nloops); 
  ...
  BE_EXPORTED void Outer_Shackle(INT new_loop, PROMPF_LINES* pl, char* index_name);
  BE_EXPORTED void Inner_Shackle(INT old_loop, INT new_loop);
  BE_EXPORTED void Preopt_Create(INT new_loop, PROMPF_LINES* pl, char* index_name); 
  BE_EXPORTED void Clear();
  BE_EXPORTED void Save();
  BE_EXPORTED void Restore();
  BE_EXPORTED void Print(FILE* fp); 
  BE_EXPORTED INT Check(FILE* fp, WN* wn_func);
  BE_EXPORTED void Print_Compact(FILE* fp, PROMPF_TRANS_LOG ptl); 
};

BE_EXPORTED extern WN_MAP Prompf_Id_Map; 
BE_EXPORTED extern PROMPF_INFO* Prompf_Info; 
BE_EXPORTED extern MEM_POOL PROMPF_pool;

BE_EXPORTED extern PROMPF_ID_TYPE Prompf_Id_Type(WN* wn_ref, WN* wn_region, 
  BOOL* is_first=NULL); 

BE_EXPORTED extern void Prompf_Assign_Ids(WN* wn_old, WN* wn_new, STACK<WN*>* old_stack, 
