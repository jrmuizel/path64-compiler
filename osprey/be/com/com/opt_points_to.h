...
public:
  ...
  BE_EXPORTED void Meet_info_from_alias_class(const POINTS_TO *);

  // Merge two points to information
  BE_EXPORTED void Meet(const POINTS_TO *, ST *);
  ...
  BE_EXPORTED void Analyze_ST(ST *st, INT64 byte_ofst, INT64 byte_size, UINT8 bit_ofst, 
		  UINT8 bit_size, TY_IDX ty, BOOL has_equiv);

  BE_EXPORTED void Analyze_ST_as_base(ST *, INT64, TY_IDX);

  BE_EXPORTED void Analyze_WN_expr(WN *);
  ...
  BE_EXPORTED void Analyze_Lda_Base(WN *, const OPT_STAB &);
  BE_EXPORTED void Analyze_Lda_Base(WN *, const STAB_ADAPTER &);
  BE_EXPORTED void Lower_to_base(WN *);
  ...
  // Print (for debugging)
  BE_EXPORTED void Print(FILE *fp=stderr) const;
};
...
public:
  BE_EXPORTED void Prepend( POINTS_TO *pt, MEM_POOL *pool );
};
...
#ifdef Is_True_On
BE_EXPORTED extern void CHECK_POINTS_TO(POINTS_TO *);
#else
#define CHECK_POINTS_TO(pt)
#endif

//  Lower ST into the <base,ofst> form
BE_EXPORTED void Expand_ST_into_base_and_ofst(ST *st, INT64 st_ofst, ST ** base, INT64 *ofst);
...
BE_EXPORTED extern ST *Is_nested_call(const WN *wn);
...
#ifdef TARG_ST
      /* FdF 20050324: Initialize the points_to for the stack area
	 where actual parameters are copied. */
      else if (opr == OPR_PARM && WN_Parm_By_Value(wn)) {
	ST *actual_parm_st = New_ST (CURRENT_SYMTAB);
	static INT count;
	char name[64];
	sprintf (name, "_call_parm%d", count++);
	ST_Init (actual_parm_st, Save_Str (name), CLASS_VAR, SCLASS_AUTO, EXPORT_LOCAL, WN_ty(wn));
	Analyze_ST(actual_parm_st, 0, ST_size(actual_parm_st), 0, 0, ST_type(actual_parm_st),  TRUE); 
	Set_dedicated();
	return;
      }
#endif
