				      ...
				      BOOL s1_lex_before_s2, BOOL use_bounds)
{
  ...
  BOOL is_ivdep = FALSE;
  BOOL loop_Cray_Ivdep, loop_Liberal_Ivdep;
  ...
  if (_type == DEPV_ARRAY_ARRAY_GRAPH) {
    ...
    if ((common_nest == s1->Elements()) && 
	 Do_Loop_Is_Ivdep(s1->Top_nth(0))) {
      is_ivdep = TRUE;
#ifdef TARG_ST
      loop_Cray_Ivdep = ((Do_Loop_Is_Ivdep_default(s1->Top_nth(0)) && Cray_Ivdep) ||
			 Do_Loop_Is_Ivdep_vector(s1->Top_nth(0)));
      loop_Liberal_Ivdep = ((Do_Loop_Is_Ivdep_default(s1->Top_nth(0)) && Liberal_Ivdep) ||
			    Do_Loop_Is_Ivdep_liberal(s1->Top_nth(0)));
#else
      loop_Cray_Ivdep = Cray_Ivdep;
      loop_Liberal_Ivdep = Liberal_Ivdep;
#endif
    }
    for (INT i=0; i<common_nest; i++) {
  ...
  if (is_ivdep && !loop_Cray_Ivdep && !loop_Liberal_Ivdep && 
  ...
  if (WN_opcode(wn) == OPC_DO_LOOP) {
...
#ifdef TARG_ST
    // FdF: Use previous bug fix on volatile
	...
	if (WN_operator(array) == OPR_ADD) {
	  ...
	  if ((kid0 == OPR_ARRAY) && (kid1 == OPR_INTCONST)) {
	    ...
	    // FdF: Do as in the comment : "If we can't figure out the
	    // base pointer, we set it to zero".
#if 0 //def TARG_ST [CG]: bogus, lno except ARRAY or ADD(ARRAY,...)
	    // ErrMsgSrcpos(EC_LNO_Generic,Find_Line(array),"load expression ignored");
	    // fdump_tree(stderr, array);
	    array = NULL;
#else
	    Set_Bad_Mem(wn);
	    return 1;
...
#ifdef TARG_ST
	ST *st_base= array ? Get_ST_Base(WN_array_base(array)) : NULL;
#else
	WN *base = WN_array_base(array);
	...
	if (WN_operator(array) == OPR_ADD) {
	  ...
	  if ((kid0 == OPR_ARRAY) && (kid1 == OPR_INTCONST)) {
	    ...
	    // base pointer, we set it to zero".
#if 0 //def TARG_ST [CG]: bogus, lno except ARRAY or ADD(ARRAY,...)
	    //	    ErrMsgSrcpos(EC_LNO_Generic,Find_Line(array),"store expression ignored");
	    //fdump_tree(stderr, array);
	    array = NULL;
#else
	    Set_Bad_Mem(wn);
