...
void GOTO_TABLE::Dismantle(WN *bad, WN *parent)
{
  ...
  WN *block = WN_CreateBlock(); 
#ifdef TARG_ST
  // [CG]: Warning, must split the call to INSERT_BlockLast in two
  // as parameter evaluation ordewr is not guaranteed. Generates a
  // bug with gcc solaris -O2. (bug id 1-0-7-A/3)
  WN *tmp;
  WN_EXTRACT_FromBlock(parent, bad);
  tmp = lower_scf_non_recursive(block, bad, LOWER_SCF | LOWER_FREQUENCY_MAPS);
  WN_INSERT_BlockLast(block, tmp);
#else
  WN_EXTRACT_FromBlock(parent, bad);
