...
typedef enum {
  ...
  DIR_NEGEQ=6,
  DIR_STAR=7,
  // [HK] workaround for gcc-3.4.1 bug
  DUMMY = 8
} DIRECTION;
...
BE_EXPORTED extern DEP DEP_SetDistance(INT32 distance);
BE_EXPORTED extern void DEP_Lex_Pos_Decompose(DEP dep,MEM_POOL *pool,DEP **pos, 
    DEP **neg, BOOL keep_pos_equals,BOOL keep_neg_equals) ;

BE_EXPORTED extern DEP DEP_Lex_Pos_Compose(DEP *pos, DEP *neg, BOOL *pos_has_eq,
	BOOL *neg_has_eq);
...
BE_EXPORTED extern DEP DEP_SetDirection(DIRECTION direction);
BE_EXPORTED extern DEP DEP_UnionDirection(DEP dep,DIRECTION direction);
BE_EXPORTED extern DEP DEP_Negate(DEP dep);
BE_EXPORTED extern void  DIRECTION_Print(DIRECTION dir,FILE *fp);
BE_EXPORTED extern void  DEP_Print(const DEP dep,FILE *fp);
BE_EXPORTED extern void  DEP_PrintBound(const DEP dep,FILE *fp);
BE_EXPORTED extern DEPV *DEPV_Create(MEM_POOL *mem_pool,UINT8 num_dim);
BE_EXPORTED extern DEPV *DEPV_CreateStar(MEM_POOL *mem_pool,UINT8 num_dim);
BE_EXPORTED extern DEPV *DEPV_CreateEqual(MEM_POOL *mem_pool,UINT8 num_dim);
BE_EXPORTED extern DEPV *DEPV_Copy(MEM_POOL *mem_pool, DEPV *depv, UINT8 num_dim);
BE_EXPORTED extern void DEPV_Print(const DEPV *depv,FILE *fp, UINT8 num_dim);
BE_EXPORTED extern void DEPV_PrintBound(const DEPV *depv,FILE *fp, UINT8 num_dim);
