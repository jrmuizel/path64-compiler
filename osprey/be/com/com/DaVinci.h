...
class BE_EXPORTED_CLASS DaVinci_Callback {
public:
  BE_EXPORTED virtual void Node_Select(const INT n_ids, const NODE_ID id_array[]);
  BE_EXPORTED virtual void Edge_Select(const EDGE_ID& edge_id);
  BE_EXPORTED virtual void Menu_Select(const char *menu_id);
};
...
public:
  ...
  BE_EXPORTED NODE_TYPE& Name(const char *typ_name);
  BE_EXPORTED NODE_TYPE& Color(const char *rgb);
...
class BE_EXPORTED_CLASS EDGE_TYPE {
private:
...
public:
  ...
  BE_EXPORTED_METHOD EDGE_TYPE& Name(const char *typ_name);
  BE_EXPORTED_METHOD EDGE_TYPE& Color(const char *rgb);
  BE_EXPORTED_METHOD EDGE_TYPE& Pattern(EDGE_PATTERN pat) { _edge_pattern = pat; return *this; }
  BE_EXPORTED_METHOD EDGE_TYPE& Direction(EDGE_DIR dir)   { _edge_dir     = dir; return *this; }
};
...
typedef __GNUEXT::hash_map< const char *, Item_status,
                  __GNUEXT::hash<const char *>, Equal_obj > Item_info;
...
class BE_EXPORTED_CLASS DaVinci {
private:
  class IO {
  private:
    FILE  *_to_fp;
...
public:
...
#ifdef TARG_ST
    /* (cbr) now called uDraw in $UDG_HOME */
    bool is_enabled = ( getenv("UDG_HOME") != NULL );
    if ( ! is_enabled ) {
      static bool msg_given = false;
      if ( ! msg_given ) {
	DevWarn("daVinci not enabled; %s\n",
		"must set UDG_HOME and put uDrawGraph on path.");
#else
  ...
  BE_EXPORTED_METHOD DaVinci(MEM_POOL *m, FILE *trace_fp = NULL, bool usage_check = false);
  BE_EXPORTED_METHOD ~DaVinci();
  ...
  BE_EXPORTED_METHOD void Event_Loop(DaVinci_Callback *cb_hook);
  BE_EXPORTED_METHOD void Exit_Event_Loop();

  BE_EXPORTED_METHOD DA_ACK Title(const char *title);        // on top.
  BE_EXPORTED_METHOD DA_ACK Show_Status(const char *status); // lower right.
  ...
  BE_EXPORTED_METHOD void   Graph_Begin();
  BE_EXPORTED_METHOD void   Node_Begin(NODE_ID id, const char *label, const NODE_TYPE& node_type);
  BE_EXPORTED_METHOD void   Out_Edge(const EDGE_ID&   edge_id,
		  const EDGE_TYPE& edge_type,
		  const NODE_ID    dest_id);
  BE_EXPORTED_METHOD void   Node_End();
  BE_EXPORTED_METHOD DA_ACK Graph_End();

  BE_EXPORTED_METHOD DA_ACK Change_Attr(const NODE_ID     node_id,
		     const NODE_TYPE&  nt,
