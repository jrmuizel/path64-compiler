...
// [HK]
// class SLIST_NODE {
class BE_EXPORTED_CLASS SLIST_NODE {
friend class SLIST;
...
protected:
  BE_EXPORTED_METHOD SLIST_NODE(void)                       { _next = NULL; }
  BE_EXPORTED_METHOD ~SLIST_NODE(void)                      {}

  BE_EXPORTED_METHOD void        Insert_After(SLIST_NODE *nd)  { nd->_next = _next; _next = nd; }
  BE_EXPORTED_METHOD SLIST_NODE *Insert_Before(SLIST_NODE *nd) { nd->_next = this; return nd; }
  BE_EXPORTED_METHOD SLIST_NODE *Remove(SLIST_NODE *prev);
  BE_EXPORTED_METHOD void        Set_Next(SLIST_NODE *n)       { _next = n; }
  BE_EXPORTED_METHOD INT32       Len(void) const;
  BE_EXPORTED_METHOD INT         Pos(SLIST_NODE *) const;
public:
  BE_EXPORTED_METHOD SLIST_NODE *Next(void) const              { return _next;}
};

class BE_EXPORTED_CLASS SLIST {
private:
  SLIST_NODE *_head;
  SLIST_NODE *_tail;
...
public:
  BE_EXPORTED_METHOD SLIST(SLIST_NODE *list);
  BE_EXPORTED_METHOD ~SLIST(void)                 {}

  BE_EXPORTED_METHOD void Set_Head(SLIST_NODE *h)      { _head = h; }
  BE_EXPORTED_METHOD void Set_Tail(SLIST_NODE *t)      { _tail = t; }

  BE_EXPORTED_METHOD void        Init(SLIST_NODE *list);   // same as constructor
  BE_EXPORTED_METHOD void        Init_Head(SLIST_NODE *list)      { _head = list; _tail = NULL; }
  BE_EXPORTED_METHOD void        Clear(void)                      { _head = _tail = NULL; }

  BE_EXPORTED_METHOD BOOL        Append( SLIST_NODE *nd, SLIST_NODE *od);
  BE_EXPORTED_METHOD BOOL        Prepend( SLIST_NODE *nd, SLIST_NODE *od );
  BE_EXPORTED_METHOD void        Append_List(SLIST *new_list);
  BE_EXPORTED_METHOD void        Prepend_List(SLIST *new_list);
  BE_EXPORTED_METHOD SLIST_NODE *Remove_Headnode(void);
  BE_EXPORTED_METHOD SLIST_NODE *Remove(SLIST_NODE *prev, SLIST_NODE *cur);
  BE_EXPORTED_METHOD void        Remove_node(SLIST_NODE *slist_node);

  BE_EXPORTED_METHOD SLIST_NODE		*Head(void)            { return _head; }
  BE_EXPORTED_METHOD const SLIST_NODE	*Head(void) const      { return _head; }
  BE_EXPORTED_METHOD SLIST_NODE		*Tail(void)            { return _tail; }
  BE_EXPORTED_METHOD const SLIST_NODE	*Tail(void) const      { return _tail; }
  BE_EXPORTED_METHOD BOOL        Is_Empty(void) const             { return _head == NULL; }
  BE_EXPORTED_METHOD INT32       Len(void) const;
  BE_EXPORTED_METHOD INT         Pos(SLIST_NODE *nd) const        { return _head->Pos(nd); }

  BE_EXPORTED_METHOD void Append( SLIST_NODE *nd )
    {
      if (nd == NULL) return;
      if (_head == NULL)
	_head = _tail = nd;
  ...
  BE_EXPORTED_METHOD void Prepend( SLIST_NODE *nd )
    {
      if (nd == NULL) return;
...
class  BE_EXPORTED_CLASS SLIST_ITER {
private:
  SLIST_NODE  *_head;
  SLIST_NODE  *_cur;
  mINT16       _len;
...
public:
  ...
  BE_EXPORTED_METHOD INT32        Len(void);		// get the length of the list
...
class BE_EXPORTED_CLASS CHAIN_NODE {
friend class CHAIN;
...
protected:
  BE_EXPORTED_METHOD CHAIN_NODE(void)                       { _next = _prev = NULL; }
  BE_EXPORTED_METHOD ~CHAIN_NODE(void)                      {}

  BE_EXPORTED_METHOD CHAIN_NODE *Insert_After(CHAIN_NODE *nd);
  BE_EXPORTED_METHOD CHAIN_NODE *Insert_Before(CHAIN_NODE *nd);
  BE_EXPORTED_METHOD CHAIN_NODE *Remove(void);

  BE_EXPORTED_METHOD CHAIN_NODE *Next(void)                    { return _next;}
  BE_EXPORTED_METHOD const CHAIN_NODE *Next(void) const        { return _next;}
  BE_EXPORTED_METHOD CHAIN_NODE *Prev(void)                    { return _prev;}
  BE_EXPORTED_METHOD const CHAIN_NODE *Prev(void) const        { return _prev;}
  BE_EXPORTED_METHOD void        Set_Next(CHAIN_NODE *n)       { _next = n; }
  BE_EXPORTED_METHOD void        Set_Prev(CHAIN_NODE *n)       { _prev = n; }
};
...
public:
  BE_EXPORTED_METHOD CHAIN(CHAIN_NODE *nd)        { _head = _tail = nd; }
  BE_EXPORTED_METHOD ~CHAIN(void)                 {}

                                        // same as constructor
  BE_EXPORTED_METHOD void        Init(void)                { _head = _tail = NULL; }
  BE_EXPORTED_METHOD void        Init(CHAIN_NODE *nd)      { _head = _tail = nd; }
  BE_EXPORTED_METHOD void        Init(CHAIN *list)         
			       { _head = list->Head(); _tail = list->Tail();}
  BE_EXPORTED_METHOD void        Clear(void)               { _head = _tail = NULL; }

  BE_EXPORTED_METHOD void        Append( CHAIN_NODE *nd );
  BE_EXPORTED_METHOD void        Prepend( CHAIN_NODE *nd );
  BE_EXPORTED_METHOD void        Insert_After(CHAIN_NODE *nd, CHAIN_NODE *after_nd);
  BE_EXPORTED_METHOD void        Insert_Before(CHAIN_NODE *nd, CHAIN_NODE *before_nd);
  BOOL        Is_Member(CHAIN_NODE *nd) const;

  BE_EXPORTED_METHOD void        Append_List(CHAIN *new_list);
  BE_EXPORTED_METHOD void        Prepend_List(CHAIN *new_list);
  BE_EXPORTED_METHOD void        Remove(CHAIN_NODE *);
  BE_EXPORTED_METHOD CHAIN_NODE *Remove_Head(void);
  BE_EXPORTED_METHOD CHAIN_NODE *Remove_Tail(void);

  BE_EXPORTED_METHOD CHAIN_NODE       *Head(void)                 { return _head; }
  BE_EXPORTED_METHOD const CHAIN_NODE *Head(void) const           { return _head; }
  BE_EXPORTED_METHOD CHAIN_NODE       *Tail(void)                       { return _tail; }
  BE_EXPORTED_METHOD const CHAIN_NODE *Tail(void) const                 { return _tail; }
  BE_EXPORTED_METHOD BOOL        Is_Empty(void) const             { return _head == NULL; }
  BE_EXPORTED_METHOD INT32       Len(void) const;
...
class BE_EXPORTED_CLASS CHAIN_ITER {
private:
  CHAIN_NODE  *_cur;
  CHAIN       *_list;
  mINT16       _len;
...
public:
  BE_EXPORTED_METHOD CHAIN_ITER(CHAIN *sl)
		   { _list = sl; _cur=_list->Head(); 
					_len = -1; _idx = -1; }
  BE_EXPORTED_METHOD ~CHAIN_ITER(void) {}

  BE_EXPORTED_METHOD void         Init(CHAIN *sl) { _list = sl; _len=_idx=-1; _cur=_list->Head();}
  BE_EXPORTED_METHOD void         Clear(void) { _list = NULL; _cur = NULL; _len = -1; _idx = -1;}
  BE_EXPORTED_METHOD CHAIN_NODE  *First(void);  // get the first element, and reset the index.
  BE_EXPORTED_METHOD CHAIN_NODE  *Last(void);   // get the last element, and set index=len.
  BE_EXPORTED_METHOD CHAIN_NODE  *Next(void);   // get the next element, and incr. the pointer.
  BE_EXPORTED_METHOD CHAIN_NODE  *Prev(void);   // get the prev element, and decr. the pointer.
  BE_EXPORTED_METHOD CHAIN_NODE  *Nth(INT n);   // get the nth element
  BE_EXPORTED_METHOD CHAIN_NODE  *Last_Nth(INT n);// get the last nth element

  BE_EXPORTED_METHOD CHAIN       *List(void)            {return _list;} 
  BE_EXPORTED_METHOD CHAIN_NODE  *Peek_Next(void)       {return _cur->Next();} 
  BE_EXPORTED_METHOD CHAIN_NODE  *Cur(void)             {return _cur;}
  BE_EXPORTED_METHOD INT          Idx(void)             {return _idx;}
  BE_EXPORTED_METHOD INT32        Len(void);            // get the length of the list
  BE_EXPORTED_METHOD BOOL         Is_Empty(void)        { return _cur == NULL; }
  BE_EXPORTED_METHOD BOOL         Is_Empty_Reverse(void){ return _cur == NULL; }
};
