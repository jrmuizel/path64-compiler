...
public: 
  ...
  Input_File* ifl; 
  BE_EXPORTED INT Open_Read_File(char *file_name); 
  BE_EXPORTED void* Section_Address(Elf64_Word info);
  BE_EXPORTED INT Section_Size(Elf64_Word info);
  BE_EXPORTED void Close_Read_File(); 
    // The following methods are for specific sections. 
  BE_EXPORTED IPA_LNO_SUMMARY_PROCEDURE* Procedure(INT i);
  BE_EXPORTED INT Procedure_Count();
  BE_EXPORTED PROJECTED_REGION* Projected_Region(INT i);
  INT Projected_Region_Count();
  BE_EXPORTED PROJECTED_NODE* Projected_Node(INT i);
  INT Projected_Node_Count();
  BE_EXPORTED TERM* Term(INT i);
  INT Term_Count();
  BE_EXPORTED IVAR* Ivar(INT i);
  BE_EXPORTED INT Ivar_Count();
  BE_EXPORTED INT Add_Translated_Ivar(const IVAR& ivar); 
  BE_EXPORTED INT Add_Translated_Ivar_Unique(const IVAR& ivar); 
  BE_EXPORTED IPA_LNO_SUMMARY_FORMAL* Formal(INT i);
  INT Formal_Count();
  BE_EXPORTED IPA_LNO_SUMMARY_GLOBAL* Global(INT i);
  INT Global_Count();
  BE_EXPORTED SUMMARY_VALUE* Value(INT i);
  INT Value_Count();
  BE_EXPORTED SUMMARY_EXPR* Expr(INT i);
  INT Expr_Count();
}; 
...
public: 
  ...
  BE_EXPORTED void Open_Write_File(char *file_name); 
  BE_EXPORTED void Write_Section(Elf64_Word sh_info, char* name, void* buf, INT size);
  BE_EXPORTED INT Close_Write_File(); 
};
