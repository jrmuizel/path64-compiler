...
class forward_to_next_iterator : 
    // [HK]
#if __GNUC__ >= 3
   public std::iterator_traits<typename Container::iterator>
#else
...
protected:
   ...
   bool _is_dereferencable() const {return _idx >= 0 && _idx < _obj->size();}
    ...
    // [HK]
#if __GNUC__ >=3
   typename  forward_to_next_iterator<Container, Next>::reference _deref() const
#else
...
public:
   ...
   INT32      current_idx() const {return _idx;} // For "==" operator
    ...
    // [HK]
#if __GNUC__ >=3
   typename  forward_to_next_iterator<Container, Next>::reference  operator*() const 
#else
