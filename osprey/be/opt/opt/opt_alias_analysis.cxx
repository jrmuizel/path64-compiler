...
#ifdef TARG_ST
// [CG]
// For tracing in details the Compute_FSA algorithm use 
// Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG) (option -tt26:0x800).
// Available in debug mode only (Is_True_On).
//
#endif
...
void OPT_STAB::Simplify_Pointer_Ver(VER_ID ver, POINTS_TO *ai)
{
  ...
  if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
    fprintf(TFile, " [Simplify_Pointer_Ver(ver=%d aux=%d)\n", ver, aux);
  }
#endif
...
#if Is_True_On
    if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
      fprintf(TFile, "  ...points to already available:\n");
      fprintf(TFile, "  ...ver=%d aux=%d pt:", ver, aux);
      ai->Print(TFile);
      fprintf(TFile, " ]Simplify_Pointer_Ver\n");
    }
#endif
    CHECK_POINTS_TO(ai);
    return;
...
#if Is_True_On
	if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
	  fprintf(TFile, "  ...processing phi opnd=%d ver=%d:\n", i, opnd_vid);
	}
#endif
	if (opnd_pt == NULL) {  // if opnd is not analyzed, analyze it now.
#if Is_True_On
	  if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
	    fprintf(TFile, "  ...points to not available, simplifying it:\n");
	  }
#endif
	  pt2.Init();
...
#if Is_True_On
	if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
	  fprintf(TFile, "  ...phi opnd=%d ver=%d pt:", i, opnd_vid);
	  opnd_pt->Print(TFile);
	}
#endif		  
...
#if Is_True_On
	      if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
		fprintf(TFile, "  ...points to beeing processed and dominates, set to unknown:\n");
		fprintf(TFile, "  ...phi opnd=%d ver=%d pt:", i, opnd_vid);
		opnd_pt->Print(TFile);
	      }
#endif
...
#if Is_True_On
	if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
	  fprintf(TFile, "  ...meet phi opnd=%d with phi result=%d:\n", opnd_vid, ver);
	  if (opnd_pt->Expr_kind() == EXPR_IS_BEING_PROCESSED) {
	    fprintf(TFile, "  ...phi opnd=%d is being processed\n", opnd_vid);
	  }
	}
#endif	  
...
#if Is_True_On
	if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
	  fprintf(TFile, "  ...phi result after meet ver=%d pt:", ver);
...
#if Is_True_On
  if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
    fprintf(TFile, "  ...computed points to:\n");
    fprintf(TFile, "  ...ver=%d aux=%d pt:", ver, aux);
    ai->Print(TFile);
    fprintf(TFile, " ]Simplify_Pointer_Ver\n");
  }
...
#if Is_True_On
  if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
    fprintf(TFile, " [Analyze_Base_Flow_Sensitive():\n");
    fprintf(TFile, "  ...pt:");
    pt->Print(TFile);
    fprintf(TFile, "  ...wn:");
    fdump_tree_no_st(TFile, wn);
...
#if Is_True_On
  if (Get_Trace(TP_WOPT2, ALIAS_DUMP_FLAG)) {
    fprintf(TFile, "  ...points to after analyzis:");
    pt->Print(TFile);
    fprintf(TFile, " ]Analyze_Base_Flow_Sensitive()\n");
  }
#endif
