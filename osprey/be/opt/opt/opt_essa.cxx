				...
				EXP_OCCURS *use) const
{
  ...
  case CK_OP:
	...
	Is_Trace(Tracing(),
		 (TFile,"Same_e_version_phi_result: not modified\n"));
	return TRUE;
      }
	...
	return TRUE;
      ...
      Is_Trace(Tracing(),
		 (TFile,"Same_e_version_phi_result: Modified, not injured occurrence\n"));
      return FALSE;
    }
  case CK_VAR: // LPRE
				       ...
				       BOOL *injured) const
{
  ...
  *injured = FALSE;
  ...
  case CK_OP:
	...
	// all is well
	Is_Trace(Tracing(),
	     (TFile,"Same_e_version_real_occ_phi_opnd: not modified\n"));
	return TRUE;
      }
      // modified, so see if simply injured
      else if ( !Worklist()->Exclude_sr_cand() && 
	...
	*injured = TRUE;
	...
	return TRUE;
      ...
      // modified, and not injured
      Is_Trace(Tracing(),
	     (TFile,"Same_e_version_real_occ_phi_opnd: modified, not injured\n"));
      return FALSE;
    }
  case CK_VAR: // LPRE
...
ESSA::Process_delayed_rename(EXP_OCCURS *use, CODEREP *real_occ) const
{
  ...
  Is_Trace(Tracing(), (TFile,"<essa> Process_delayed_rename use:\n"));
  ...
  for (INT opnd_num = 0; opnd_num < exp_phi->Opnd_count(); opnd_num++) {
...
#if 0
      Is_Trace_cmd(Tracing(), fprintf(TFile,"<essa> opnd %d def: \n", opnd_num));
      ...
      case EXP_OCCURS::OCC_REAL_OCCUR:
	{
	  BOOL injured = FALSE;
	  if ( is_const_lpre ||
	      Same_e_version_real_occ_phi_opnd(def, 
                              use->Bb(), opnd_num, newcr, &injured) ) {
	    EXP_OCCURS *tmp_use = exp_phi->Pred(opnd_num);
...
ESSA::Rename(void)
{
  ...
  Is_Trace(Etable()->Tracing(),(TFile,
                   "====== ETABLE::Rename_expression ======\n"));
  Is_Trace_cmd(Etable()->Tracing(), Cur_e_expr()->Print(0, TFile));
#endif
  ...
  FOR_ALL_NODE(occur, exp_occ_iter, Init()) {
    ...
    //Is_Trace_cmd(Etable()->Tracing(), occur->Occurrence()->Print(2,TFile));
#endif
...
EXP_WORKLST::Rename_expression(ETABLE *etable)
{
  ...
  Is_Trace_cmd(etable->Tracing(), Exp()->Print(0, TFile));
