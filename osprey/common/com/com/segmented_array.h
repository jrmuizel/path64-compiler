...
// VTYPE_PTR is pointer to SEG_ARRAY::value_type,
...
template <class ARRAY_Ptr, class T, class VTYPE_PTR, class REF>
...
public:
  ...
  typedef VTYPE_PTR                 pointer;
  typedef REF                       reference;

private:
    
    ARRAY_Ptr segmented_array;
    VTYPE_PTR ptr;			// pointer to the current element
    VTYPE_PTR segment_last;		// ptr after the current segment
...
public:
    ...
    VTYPE_PTR Ptr () const		{ return ptr; }
    VTYPE_PTR operator->() const        { return ptr; }
    UINT Index () const {
