...
#if __GNUC__ >= 3
// [HK] added declaration of friend function as a template before refering to template specializations (not allowed anymore in gcc3-4-0)
...
public:
  ...
  // tree related "iterators"
    ...
    // [HK]
#if __GNUC__ >=3
   typename WN_TREE_ITER<POST_ORDER, WHIRL>::self &     
operator++()    { WN_TREE_next(); return *this;} // pre
   typename WN_TREE_ITER<POST_ORDER, WHIRL>::self       
operator++(INT) { typename WN_TREE_ITER<POST_ORDER, WHIRL>::self tmp =
*this; WN_TREE_next(); return tmp;}
#else
  self &      operator++()    { WN_TREE_next(); return *this;} // pre
  self        operator++(INT) { self tmp = *this; WN_TREE_next(); return tmp;}
#endif //  __GNUC__ >=3
