...
BE_EXPORTED extern char *FE_Type_Name(TY *ty);
#endif /* FRONT_END */

/* return the ty for a whirl expression */
BE_EXPORTED extern TY_IDX TY_Of_Expr (const WN *expr);

/* return the ty for a whirl expression accounting for SCLASS_FORMAL_REF */
BE_EXPORTED extern TY_IDX TY_Of_Parameter(WN *expr);
 ...
 * floating point.
 */
BE_EXPORTED extern BOOL Is_Float_Type (TY_IDX t);
...
BE_EXPORTED extern TY *Find_Unqualified_Type ( TY * );
...
BE_EXPORTED extern BOOL Equivalent_Types ( TY_IDX, TY_IDX, QUAL_CHECK );
BE_EXPORTED extern BOOL Similar_BE_Types ( TY_IDX, TY_IDX );
