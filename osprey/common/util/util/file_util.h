...
BE_EXPORTED extern BOOL Is_File ( const char *fname );

/* Determine whether two streams are associated with the same file: */
BE_EXPORTED extern BOOL Same_File (FILE *file1, FILE *file2);

/* Determine whether a filename has a given extension: */
BE_EXPORTED extern BOOL Has_Extension (
 ...
 * return.
 */
BE_EXPORTED extern BOOL Find_File (
 ...
 */
BE_EXPORTED extern char *Last_Pathname_Component ( char *pname );

/* Make an absolute path name from the file name,
 * which means no .. or . or // in the path. */
BE_EXPORTED extern char *Make_Absolute_Path (char *filename);

/* return getcwd or $PWD or . */
BE_EXPORTED extern char * Get_Current_Working_Directory (void);
