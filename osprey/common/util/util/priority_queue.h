...
***     void*   element
*** )
***     Restore the priority queue 'prq' following a change in the 
***     priority of 'element'.  It is an error if 'element' is not in
***     the priority queue.
***
***     If an 'index_fn' is provided when the heap is created, the element to
***     be deleted can be directly accessed in the heap, and the Update
***     operation takes O(log n) time.  If no 'index_fn' is provided, the
***     heap is first searched linearly for the element, and then it is
***     updated.  This takes O(nlog n) time.
***
***
*** void PRQ_Reset(
***     PRQ*    prq
