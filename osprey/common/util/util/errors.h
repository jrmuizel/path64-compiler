...
BE_EXPORTED extern void Abort_Compiler_Location (
  char* file_name,
  INT line_number
);
...
/*PRINTFLIKE1*/
BE_EXPORTED extern void DevWarn( const char* FormatString,... )
#ifdef __GNUC__
...
BE_EXPORTED extern void DevWarn_Toggle( void );
#pragma mips_frequency_hint NEVER DevWarn_Toggle

BE_EXPORTED extern BOOL Count_Limit_DevWarn( const char *const src_fname,
				 const UINT        src_line,
				 const UINT        limit );
...
BE_EXPORTED extern INT errno;

/* Control reporting by severity level: */
BE_EXPORTED extern INT Min_Error_Severity;
BE_EXPORTED extern INT Conformance_Level;
...
BE_EXPORTED extern void Set_Error_Tables(
...
BE_EXPORTED extern void Set_Error_File (
    const char *filename
);
BE_EXPORTED extern void Set_Error_Trace (
    FILE *stream
);

/* Notify error reporter of current source file name: */
BE_EXPORTED extern void Set_Error_Source (
    const char *filename
);

/* Notify error reporter of current source file line number: */
#define ERROR_LINE_UNKNOWN	0
BE_EXPORTED extern void Set_Error_Line (
    INT LineNo
);

/* Notify error reporter of current source position: */
#ifdef MONGOOSE_BE
BE_EXPORTED extern void Set_Error_Srcpos (
    SRCPOS SrcPos
);
#endif

/* Notify error reporter of current compiler phase: */
BE_EXPORTED extern void Set_Error_Phase (
    const char *phasename
);
BE_EXPORTED extern const char *Get_Error_Phase (void);

/* Determine whether compilation should terminate: */
BE_EXPORTED extern BOOL Get_Error_Count (	/* Return whether to stop now */
    INT *Error_Count,	/* Number of errors to this point */
    INT *Warning_Count	/* Number of warnings to this point */
);

/* Initialize the error handler: */
BE_EXPORTED extern void Init_Error_Handler ( INT Max_Allowed_Errors );

/* Initialize signal catching: */
BE_EXPORTED extern void Handle_Signals ( void );
...
BE_EXPORTED extern void Rag_Handle_Woff_Args(char	*wstring);

/* had any internal errors */
BE_EXPORTED extern BOOL Had_Internal_Error (void);
